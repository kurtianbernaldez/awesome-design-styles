---
name: Sunset Radio Synthwave
slug: sunset-radio
family: synthwave
description: "A broadcast-inspired interpretation emphasizing warm horizon bands and polished audio-like controls rather than an arcade grid."
tags:
  - synthwave
  - sunset-radio
  - complete-system
...
# Sunset Radio Synthwave

## Overview and Design Philosophy

A broadcast-inspired interpretation emphasizing warm horizon bands and polished audio-like controls rather than an arcade grid.
...
Center a 72px italic title above a long radio-like workspace. Features form two broad program panels. Keep generous horizontal bands and a clear dial-like control row.

The distinction from **Vector Arcade Synthwave** is structural: that interpretation follows this direction: Use a compact 4:5 opening and a 56px monospace heading. Workspace rows resemble a vector control board. Features stack with numbered rails and firm rectangular boundaries. Choose this variant when the composition above suits the content. A palette substitution alone does not establish either interpretation.

## Visual Character

Use deep plum, coral horizon bands, narrow magenta highlights, and dark opaque panels with rounded ends. Glow belongs to edges and selected controls only.
...
- Buttons are rounded illuminated keys. Tabs resemble station selectors; input fields are opaque readouts. Dialogs use one dark broadcast panel with a clear title.
...
Use abstract original horizon graphics. Audio metaphors must not imply playback controls where the actual action does something else.
...
Use 160ms color changes and no oscillating equalizer decoration.
...
- **Active / pressed:** Use 160ms color changes and no oscillating equalizer decoration.
...
.design .hero{display:block;padding:48px 0;background:linear-gradient(transparent 65%,#d36c5b22)}
.design .hero-copy{text-align:center;max-width:800px;margin:auto}.design .hero h1{font:700 italic 72px/1.05 system-ui,sans-serif;text-shadow:2px 2px #a3477d}
.design .hero-copy p{margin-inline:auto}.design .actions{justify-content:center}.design .project-card{max-width:840px;margin:40px auto 0;border:2px solid #bc789f;border-radius:36px;border-top:12px solid #cf7b69;box-shadow:0 0 18px #a3477d33}
.design .features{grid-template-columns:1fr 1fr;gap:32px}.design .feature-card:nth-child(n){border:1px solid #bc789f;border-radius:24px;box-shadow:none}
.design .stats{border-block:6px solid #cf7b69;padding:28px 0}.design .stats strong{font:600 italic 44px/1.2 system-ui,sans-serif}
.design .primary,.design .secondary{border-radius:99px}.design .demo-form{border-radius:24px;border:2px solid #bc789f}
.design .testimonial{border:0;font-style:italic}
