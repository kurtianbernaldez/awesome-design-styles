...
family: de-stijl
description: "A system of unequal orthogonal fields: heavy dark rails connect primary-color planes into one composition."
tags:
  - de-stijl
  - orthogonal-fields
  - complete-system
...
The page itself is the frame. Use an 8px outer boundary and adjoining hero fields in 1.25:1 proportion with no gutter. A large red cap sits above the yellow workspace. A blue statistics band spans the width. Features use unequal 2:1:1 rectangles.
...
- Use cream, red, yellow, and blue with black structural divisions.
- Let the control construction explain state and hierarchy.
- Do not use randomly positioned colored squares around an otherwise generic white card system.
...
Use cream, red, yellow, and blue with black structural divisions. No curves, diagonals, shadows, gradients, or floating cards. Every large color field must correspond to a real content region.
...
All controls are rectangular. Buttons contrast clearly with their containing field; editable inputs use an opaque neutral inset. Tabs use linear partitions. Menus and dialogs are orthogonal assemblies with clear focus separation.
...
Use original planar diagrams or simply cropped imagery only when required. Do not reproduce a famous painting as a page background.
...
Immediate state changes; position and geometry remain fixed.
...
- **Active / pressed:** Immediate state changes; position and geometry remain fixed.
...
Background reference: [MoMA: De Stijl exhibition catalogue](https://assets.moma.org/documents/moma_catalogue_1798_300159061.pdf). The rules in this document are an original interface interpretation, not a reproduction of a source artifact or a claim that this variant exhausts the family.
