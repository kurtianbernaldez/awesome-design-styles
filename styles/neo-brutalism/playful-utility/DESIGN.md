...
description: Friendly utility with thick outlines, small hard shadows, and clear modular cards.
tags:
  - neo-brutalism
  - playful-utility
  - complete-system
...
Friendly utility with thick outlines, small hard shadows, and clear modular cards. This is one original interpretation of the neo brutalism family, not a definitive rulebook for the entire aesthetic. It is intended for complete websites and application surfaces that benefit from this visual character. Its layout, material choices, and control conventions below define why it exists as a separate system.

Use balanced hero halves and equal three-column modules. Keep the header compact, headlines friendly, and all card actions aligned. Avoid the arbitrary rotation or deliberately irregular hierarchy of poster-driven systems.
...
- Keep geometry friendly and hit areas generous.
...
| `muted` | `#4d586d` | Secondary text on surface |
...
Rounded-feeling system sans is bold but never condensed. Keep short headings in sentence case. Use numeric indexes as friendly wayfinding rather than confrontational poster type.

| Role | Stack | Size / line height | Weight and tracking |
|---|---|---|---|
| Display / H1 | `system-ui, sans-serif` | fluid 36–64px / 1.08 | 800; -0.03em |
...
The outer container is at most 1120px wide, with 28px gutters and 24px gaps. Use CSS grid with `minmax(0, 1fr)` tracks so long content cannot enlarge the page. Use flex wrapping for action groups and navigation. Cards grow with content; do not enforce fixed text heights. Comparable pricing keeps the same benefit order and aligned actions even if decorative feature blocks have unequal spans.

## Surfaces, Borders, Radius, Shadows, and Depth

Use 10px card corners and 6px controls, each with a consistent 3px outline. Hard shadows belong to actionable or featured surfaces; form fields stay flat. This softer geometry and equal modular grid distinguish it from raw Brutalism.

Reference card boundary: 3px where a frame is used; reference corner radius: 10px. Surface shadow: `5px 5px 0 #242d40`. Controls generally use 10px corners unless their component treatment specifies a more precise shape. Content stays at layer 0, navigation at 10 when sticky, menus at 20, backdrop at 40, dialog at 50, and transient feedback at 60. Avoid creating stacking contexts on every card. Overlays must remain above all decorative treatments.

## Components

Primary buttons are yellow with dark text and a 3px hard shadow. Secondary actions use the cream surface with the same outline but no shadow. Checkboxes are large native controls with full clickable labels. Feature cards use one pale supporting fill; recommended pricing receives a written badge and heavier top boundary. Navigation links have a thick current underline. Tabs are outlined rounded rectangles with a yellow selected fill. Dialogs have the same 10px shape, opaque cream, and a small hard shadow.
...
Use simple original flat illustrations with bold outlines and restrained friendly shapes. Avoid copied mascots. Use framed 4:3 photos sparingly. Charts use solid dark-outlined bars, accurate axes, and explicit labels; do not turn data marks into decorative stickers.
...
Buttons press 2px toward their shadow in 90ms ease-out. Hover changes fill without bounce. Reduced motion swaps the shadow instantly without translating the control.
...
```css
.design-neo-brutalism-playful-utility {
  --canvas: #f7f0df;
  --surface: #fffbee;
  --ink: #242d40;
  --muted: #4d586d;
...
.design-neo-brutalism-playful-utility * { box-sizing: border-box; }
```
...
- Keep geometry friendly and hit areas generous.
- align repeated modules.
- explain status in words.
- Preserve the readable control and feedback behavior when adapting the visual composition.

## Don't

- Do not confuse playful visuals with random placement or add soft atmospheric shadows.
