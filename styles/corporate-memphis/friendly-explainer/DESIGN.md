...
description: "Illustration-led, approachable communication through abstract people, flat color, and generous plain typography."
tags:
  - corporate-memphis
  - friendly-explainer
  - complete-system
...
Use equal hero columns with a 44px gap. Keep the headline bold but conversational. Place a visible collaboration illustration above the workspace title. Feature cards are broad unoutlined pastel regions, not floating shadowed widgets.
...
- Use warm ivory, cobalt, coral, ochre, and quiet pastel section fills.
- Let the control construction explain state and hierarchy.
- Do not call a generic pastel UI Corporate Memphis when there is no illustration-led communication.
...
Use warm ivory, cobalt, coral, ochre, and quiet pastel section fills. Surfaces are flat with no cast shadow; 8px card corners are subordinate to the illustration. The testimonial is a soft asymmetric section block.
...
Use simple rounded rectangular actions and high-contrast labels. Tabs and forms are deliberately straightforward. Status chips stay distinct from illustrated decorative shapes. The UI itself must not require interpreting an illustration.
...
Use original abstract people collaborating, with economical silhouettes and nonliteral proportions. The preview includes an original generated collaboration scene; slight tonal variation is confined to the artwork, while UI surfaces remain flat. Never reproduce a particular brand’s characters.
...
Use friendly but brief 160ms feedback. No looping character animation or bobbing decorative shapes.
...
- **Active / pressed:** Use friendly but brief 160ms feedback. No looping character animation or bobbing decorative shapes.
...
Background reference: [BUCK: illustration system principles](https://buck.co/work/facebook-alegria). The rules in this document are an original interface interpretation, not a reproduction of a source artifact or a claim that this variant exhausts the family.
