---
name: Blog Magazine Web 2.0
slug: blog-magazine
family: web-2.0
description: "A publishing interpretation combining large editorial titles, soft gradient section headers, and spacious article modules rather than a dense social dashboard."
tags:
  - web-2.0
  - blog-magazine
  - complete-system
...
# Blog Magazine Web 2.0

## Overview and Design Philosophy

A publishing interpretation combining large editorial titles, soft gradient section headers, and spacious article modules rather than a dense social dashboard.
...
Use a 3:2 opening with a 68px serif title. Place the workspace in a sidebar-like module. Feature articles form unequal two-column regions with a prominent lead story.

The distinction from **Social Dashboard Web 2.0** is structural: that interpretation follows this direction: Use a compact 4:5 opening with a 46px sans heading. The workspace has a colored title strip and dense activity rows. Features stack as compact service modules. Choose this variant when the composition above suits the content. A palette substitution alone does not establish either interpretation.

## Visual Character

Use warm white paper, pale blue header gradients, fine gray boundaries, and small rounded corners. Keep body regions flat and reserve shine for action buttons.
...
- Actions use rounded glossy buttons; navigation resembles a magazine section bar. Tabs use a selected colored header. Forms and dialogs are calm white panels.
...
Use original editorial photography with captions and descriptive alternatives. Do not replace article titles with image text.
...
Use 160ms color changes and no automatic article carousel.
...
**Pointer policy: none.** Magazine reading does not benefit from a tracking gloss mark.
...
- **Active / pressed:** Use 160ms color changes and no automatic article carousel.
...
.design .hero{display:grid;grid-template-columns:3fr 2fr;gap:40px;align-items:start}.design .hero h1{font:700 68px/1.05 Georgia,serif;text-shadow:none}
.design .project-card{margin-top:48px;border:1px solid #9aafb8;border-radius:6px;background:#fff;border-top:20px solid #bdd8e4;box-shadow:0 3px 8px #3c657419}
.design .features{grid-template-columns:2fr 1fr;gap:28px}.design .feature-card:nth-child(n){border:1px solid #9aafb8;border-radius:6px;background:#fff;box-shadow:none}
.design .feature-card:first-child{grid-row:span 2;border-top:20px solid #bdd8e4}.design .feature-card h3{font:700 28px/1.3 Georgia,serif}
.design .stats{background:linear-gradient(#fff,#e2eff4);border-block:1px solid #9aafb8;padding:28px}.design .stats strong{font:700 42px/1.2 Georgia,serif}
.design .testimonial{text-align:left;font:italic 26px/1.6 Georgia,serif;border-left:5px solid #bdd8e4;padding:32px}
.design .testimonial blockquote{font:italic 26px/1.6 Georgia,serif}
.design .demo-form{border:1px solid #9aafb8;border-top:20px solid #bdd8e4;border-radius:6px;background:#fff}
