---
name: Friendly Explainer Corporate Memphis
slug: friendly-explainer
family: corporate-memphis
description: Approachable rounded modules and generous explanatory hierarchy with restrained flat color.
tags:
  - corporate-memphis
  - friendly-explainer
  - complete-system
...
# Friendly Explainer Corporate Memphis

## Overview and Design Philosophy

Approachable rounded modules and generous explanatory hierarchy with restrained flat color. This is one original interpretation of the corporate memphis family, not a definitive rulebook for the entire aesthetic. It is intended for complete websites and application surfaces that benefit from this visual character. Its layout, material choices, and control conventions below define why it exists as a separate system.

Use balanced hero halves and three equal explanation cards. Alternate explanation and proof with clear section headings. Pricing and forms remain regular and predictable. Keep every illustration secondary to the task it explains.
...
| Token | Value | Role |
|---|---|---|
| `canvas` | `#edf0f7` | Page environment; ink text |
| `surface` | `#ffffff` | Opaque reading and control plane |
| `ink` | `#33415c` | Main text on canvas or surface |
| `muted` | `#5c6b86` | Secondary text on surface |
| `accent` | `#566fa0` | Primary action and selected state |
| `on-accent` | `#ffffff` | Text on the accent fill |
| `line` | `#8392af` | Grouping rules |
| `control-line` | `#8392af` | Visible input and control boundaries |
...
Use friendly sans headings with open spacing and plain 17px body. Keep hierarchy strong enough to scan. Do not use childish lettering or excessively rounded novelty fonts.

| Role | Stack | Size / line height | Weight and tracking |
|---|---|---|---|
| Display / H1 | `system-ui, sans-serif` | fluid 36–60px / 1.08 | 700; -0.03em |
| H2 | `system-ui, sans-serif` | 34px / 1.2 | 700; -0.02em |
| H3 | `system-ui, sans-serif` | 23px / 1.3 | 650; normal |
...
Use 4/8/12/20/28/40/56/80px. Keep explanatory paragraphs close to their heading and separate steps with 28px gaps. Buttons remain compact enough to read as controls.
...
The outer container is at most 1160px wide, with 36px gutters and 28px gaps. Use CSS grid with `minmax(0, 1fr)` tracks so long content cannot enlarge the page. Use flex wrapping for action groups and navigation. Cards grow with content; do not enforce fixed text heights. Comparable pricing keeps the same benefit order and aligned actions even if decorative feature blocks have unequal spans.

## Surfaces, Borders, Radius, Shadows, and Depth

Use bright opaque panels, 16px corners, soft borders, and sparse pale colored fields. Illustration carries any expressive abstraction; text areas stay calm. Avoid a cloud of decorative blobs behind every paragraph.

Reference card boundary: 1px where a frame is used; reference corner radius: 16px. Surface shadow: `0 6px 18px #33415c12`. Controls generally use 12px corners unless their component treatment specifies a more precise shape. Content stays at layer 0, navigation at 10 when sticky, menus at 20, backdrop at 40, dialog at 50, and transient feedback at 60. Avoid creating stacking contexts on every card. Overlays must remain above all decorative treatments.

## Components

Primary buttons are blue with white labels and 8px corners. Secondary buttons use white and a visible outline. Inputs are conventional opaque fields with persistent labels and clear validation. Cards emphasize an icon or step label, heading, body, then action. Pricing recommendation is explicit. Navigation is a simple row; tabs use clear filled selection. Menus and dialogs use opaque white surfaces and restrained elevation.
...
Use original flat illustrations with varied, respectful human proportions when people are shown. Do not copy a company’s illustration style or use stereotypes. Charts remain factual with direct labels and accessible tables; decorative people must not become data units.
...
Charts remain factual with direct labels and accessible tables; decorative people must not become data units.
...
Use 160ms ease-out color changes and a short dialog fade. No bouncing characters or always-moving blobs. Reduced motion makes updates immediate.
...
```css
.design-corporate-memphis-friendly-explainer {
  --canvas: #edf0f7;
  --surface: #ffffff;
  --ink: #33415c;
  --muted: #5c6b86;
...
.design-corporate-memphis-friendly-explainer * { box-sizing: border-box; }
```
...
- make the hierarchy welcoming and clear.
- Preserve the readable control and feedback behavior when adapting the visual composition.

## Don't

- Do not add meaningless blobs, copied corporate characters, or oversimplified data graphics.
