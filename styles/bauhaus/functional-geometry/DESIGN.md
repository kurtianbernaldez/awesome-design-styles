---
name: Functional Geometry Bauhaus
slug: functional-geometry
family: bauhaus
description: Geometric composition and economical type arranged around clear functional relationships.
tags:
  - bauhaus
  - functional-geometry
  - complete-system
...
# Functional Geometry Bauhaus

## Overview and Design Philosophy

Geometric composition and economical type arranged around clear functional relationships. This is one original interpretation of the bauhaus family, not a definitive rulebook for the entire aesthetic. It is intended for complete websites and application surfaces that benefit from this visual character. Its layout, material choices, and control conventions below define why it exists as a separate system.

Use a 2:1 hero, a three-column modular feature field, and large numeric statistics aligned to its grid. Asymmetry comes from column spans. Circular decoration may mark a section index but must never replace text labels or interrupt reading.
...
- Give geometry a functional role.
...
| Token | Value | Role |
|---|---|---|
| `canvas` | `#f0eadd` | Page environment; ink text |
| `surface` | `#fff9ed` | Opaque reading and control plane |
| `ink` | `#292d31` | Main text on canvas or surface |
| `muted` | `#585d63` | Secondary text on surface |
| `accent` | `#9b342b` | Primary action and selected state |
| `on-accent` | `#ffffff` | Text on the accent fill |
| `line` | `#70757a` | Grouping rules |
| `control-line` | `#70757a` | Visible input and control boundaries |
...
Use a single geometric-feeling sans stack with 400/600/750 weights. Headings are short and flush left. Numbers may become large compositional anchors, but body remains 17px and unrotated.

| Role | Stack | Size / line height | Weight and tracking |
|---|---|---|---|
| Display / H1 | `system-ui, sans-serif` | fluid 36–68px / 1.08 | 750; -0.03em |
| H2 | `system-ui, sans-serif` | 38px / 1.2 | 750; -0.02em |
...
Use 4/8/16/24/32/48/80px. Repeated distances are visibly modular. Preserve equal padding within each role even when shapes differ.
...
The outer container is at most 1160px wide, with 32px gutters and 24px gaps. Use CSS grid with `minmax(0, 1fr)` tracks so long content cannot enlarge the page. Use flex wrapping for action groups and navigation. Cards grow with content; do not enforce fixed text heights. Comparable pricing keeps the same benefit order and aligned actions even if decorative feature blocks have unequal spans.

## Surfaces, Borders, Radius, Shadows, and Depth

Use flat opaque rectangles, strong horizontal rules, and occasional circles for nonessential indexes. One restrained red action and pale ochre or blue supporting fields are enough. No texture or shadow.
...
Primary buttons are red rectangular blocks, secondary buttons ink outlines. Inputs remain rectangular with visible dark boundaries; native radio circles are functional geometry. Cards use flat fields and a top rule rather than floating elevation. Pricing keeps equal columns. Navigation is a horizontal text register with a square selected marker. Tabs use a strong baseline; menus and modals are simple cream rectangles with a 2px frame. The close action is always a labeled control, not an abstract shape.
...
Use original geometric diagrams and simple 4:3 crops. Avoid copying famous compositions or furniture imagery. Icons use basic circles and strokes with text labels where ambiguous. Data visualization emphasizes proportion, accurate scales, and direct labels; color never replaces a legend or table alternative.
...
Use 100ms linear color changes. No spinning shapes or animated geometry for atmosphere. Reduced motion uses immediate state changes.
...
```css
.design-bauhaus-functional-geometry {
  --canvas: #f0eadd;
  --surface: #fff9ed;
  --ink: #292d31;
  --muted: #585d63;
...
.design-bauhaus-functional-geometry * { box-sizing: border-box; }
```
...
- Give geometry a functional role.
- make asymmetry orderly.
- keep labels literal.
- Preserve the readable control and feedback behavior when adapting the visual composition.

## Don't

- Do not scatter primary-color shapes or rotate essential text to signal the aesthetic.
