---
name: Crafted Workspace Japandi
slug: crafted-workspace
family: japandi
description: "A task-oriented interpretation using compact crafted compartments and disciplined alignment, balancing warmth with practical everyday density."
tags:
  - japandi
  - crafted-workspace
  - complete-system
...
# Crafted Workspace Japandi

## Overview and Design Philosophy

A task-oriented interpretation using compact crafted compartments and disciplined alignment, balancing warmth with practical everyday density.
...
Use a compact 4:5 opening and a 48px medium sans title. Workspace tasks align in clean rows; features stack with a small index rail. Keep 24px module gaps.

The distinction from **Gallery Living Japandi** is structural: that interpretation follows this direction: Center a 68px regular serif title above a broad low workspace. Features form two roomy gallery bays with a full-width lead entry. Maintain long pauses between sections. Choose this variant when the composition above suits the content. A palette substitution alone does not establish either interpretation.

## Visual Character

Use warm cream, muted olive, thin timber-colored seams, and 8px corners. Shallow compartments suggest joinery without imitating physical furniture controls.
...
- Buttons are firm modest rectangles. Tabs share a clean baseline. Fields are clearly outlined cream wells, and dialogs use a simple framed work surface.
...
| Token | Value | Role |
|---|---|---|
| `canvas` | `#eae6dc` | Page environment |
| `surface` | `#faf6ed` | Opaque reading surface |
| `ink` | `#3c4940` | Primary text on surface |
| `muted` | `#5c685e` | Secondary text on surface |
| `accent` | `#4e6a55` | Action and selection cue |
| `on-accent` | `#ffffff` | Text on filled accent |
| `line` | `#819185` | Grouping rules |
| `control-line` | `#819185` | Control boundary |
...
Use a 4px base with 8/12/16/24/32/48/64/96px intervals. The wide-screen section interval is 56px; the default module gap is 28px and outer gutter is 40px. Follow the role-specific interiors in the construction rules rather than forcing every region to have identical padding.
...
Use small original material details and functional diagrams. Decoration must not displace task information.
...
Use 140ms feedback with no passive elevation change.
...
**Pointer policy: none.** Warm order and restrained material details need no decorative tracking.
...
- **Active / pressed:** Use 140ms feedback with no passive elevation change.
...
.design .hero{ grid-template-columns:1fr 1fr; gap:64px; padding:72px 0; }

.design h1{ font-weight:400; line-height:1.15; letter-spacing:-.05em; }

.design .project-card{ background:#ddd5c5; border:0; border-left:8px solid #9e886c; border-radius:0; box-shadow:none; padding:32px; }

.design .project-card h2{ font-family:Georgia,serif; font-weight:400; }

.design .features{ gap:0; background:#e7e1d5; }

.design .feature-card{ border:0; border-right:1px solid #aea18d; background:transparent; border-radius:0; box-shadow:none; padding:32px; }

.design .feature-card:last-child{ border:0; }

.design .stats{ border-block:1px solid #aea18d; }

.design .testimonial{ border:0; padding:56px 15%; }

.design .primary{ border-radius:2px; }
...
.design .hero{display:grid;grid-template-columns:4fr 5fr;gap:32px;align-items:start;padding:36px 0}.design .hero h1{font:550 48px/1.12 system-ui,sans-serif}
.design .panel{background:#eee9dc;border:1px solid #9b927f;border-radius:8px;box-shadow:none;padding:24px}
.design .project-card{border-left:8px solid #9b8265}.design .features{grid-template-columns:1fr;gap:20px}
.design .feature-card:nth-child(n){display:grid;grid-template-columns:96px 1fr;gap:8px 20px}.design .feature-index{grid-row:1/4;font:13px/1.5 ui-monospace,monospace}.design .feature-card h3{margin:0;font-family:system-ui,sans-serif}
.design .stats{border:1px solid #9b927f;padding:24px;background:#eee9dc}.design .stats strong{font:550 32px/1.2 system-ui,sans-serif}
.design .testimonial{text-align:left;font:20px/1.6 system-ui,sans-serif;border-left:3px solid #9b8265;padding:24px}
.design .testimonial blockquote{font:20px/1.6 system-ui,sans-serif}.design .demo-form{border-left:8px solid #9b8265}
