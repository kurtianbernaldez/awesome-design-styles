---
name: Atomic Graphics Mid-Century
slug: atomic-graphics
family: mid-century-modern
description: "A print-and-pattern interpretation with playful atomic geometry, bold editorial blocks, and optimistic contrast rather than a quiet furnished interior."
tags:
  - mid-century-modern
  - atomic-graphics
  - complete-system
...
# Atomic Graphics Mid-Century

## Overview and Design Philosophy

A print-and-pattern interpretation with playful atomic geometry, bold editorial blocks, and optimistic contrast rather than a quiet furnished interior.
...
Use a centered 72px bold sans title with a broad workspace below. Features occupy unequal two-column blocks. Oversized statistics and small geometric index marks provide rhythm.

The distinction from **Woodland Editorial Mid-Century** is structural: that interpretation follows this direction: Use a 3:2 opening with a 62px regular serif title. Give the workspace generous interiors and a dark top rail. Feature stories stack in broad reading bands. Choose this variant when the composition above suits the content. A palette substitution alone does not establish either interpretation.

## Visual Character

Use warm cream, muted mustard, brick, and teal in flat fields. Abstract atomic linework appears only in margins. Corners remain mostly square with a few broad arcs.
...
- Actions are flat dark plaques with mildly rounded ends. Tabs are colored index labels. Fields remain plain cream; dialogs use a single framed informational block.
...
| Token | Value | Role |
|---|---|---|
| `canvas` | `#eee6d5` | Page environment |
| `surface` | `#fff7e8` | Opaque reading surface |
| `ink` | `#3b423b` | Primary text on surface |
| `muted` | `#464c3e` | Secondary text on surface |
| `accent` | `#805234` | Action and selection cue |
| `on-accent` | `#ffffff` | Text on filled accent |
| `line` | `#7e866e` | Grouping rules |
| `control-line` | `#7e866e` | Control boundary |
...
Use original geometric illustrations and mid-century-inspired object photography. Avoid recreating recognizable commercial posters.
...
Use 140ms color changes and no animated decorative orbitals.
...
**Pointer policy: none.** Warm proportions, materials, and print accents work best as a settled composition.
...
- **Active / pressed:** Use 140ms color changes and no animated decorative orbitals.
...
.design .hero{display:block;background:#e2b95d;padding:40px!important;margin:32px 0}.design .hero-copy{text-align:center;max-width:780px;margin:auto}
.design .hero h1{font:750 72px/1.03 system-ui,sans-serif}.design .hero-copy p{margin-inline:auto}.design .actions{justify-content:center}
.design .project-card{max-width:780px;margin:36px auto 0;background:#f8ecd3;border:0;border-left:20px solid #47716a;border-radius:0 40px 0 0;box-shadow:none}
.design .features{grid-template-columns:2fr 1fr;gap:24px}.design .feature-card:nth-child(n){background:#f8ecd3;border:0;border-radius:0;padding:32px}
.design .feature-card:first-child{grid-row:span 2;background:#d7e0c9;border-top:20px solid #ad654d}.design .feature-index{font-weight:800;font-size:20px}
.design .stats{background:#d7e0c9;padding:28px;border:0}.design .stats strong{font-size:52px}
.design .demo-form{border:0;border-left:20px solid #47716a;border-radius:0;background:#f8ecd3}.design .testimonial{border-block:6px solid #ad654d;padding:36px}
