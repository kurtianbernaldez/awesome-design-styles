---
name: Orbital Data Futurism
slug: orbital-data
family: futurism
description: "A calm telemetry interpretation built around precise modules, fine radial references, and tabular data instead of visual speed."
tags:
  - futurism
  - orbital-data
  - complete-system
...
# Orbital Data Futurism

## Overview and Design Philosophy

A calm telemetry interpretation built around precise modules, fine radial references, and tabular data instead of visual speed.
...
Use a compact 4:5 opening with a 48px medium sans title. Workspace and statistics carry the highest information density. Feature rows use narrow index rails and measured gutters.

The distinction from **Kinetic Type Futurism** is structural: that interpretation follows this direction: Use a broad 96px condensed italic headline above a long workspace. Feature sections form horizontal velocity bands. Keep body text upright and all controls aligned to the page grid. Choose this variant when the composition above suits the content. A palette substitution alone does not establish either interpretation.

## Visual Character

Use dark navy working planes, thin cool borders, and subtle circular reference lines in empty regions. Small status accents provide orientation without flooding surfaces with glow.
...
- Buttons are compact rectangular instruments. Selected tabs have a bright baseline and written label. Inputs use opaque dark wells; dialogs are flat telemetry panels with a strong title row.
...
Use labeled charts, original technical diagrams, and no decorative fake measurements. Radial graphics need a text equivalent when they encode data.
...
Use 120ms state transitions and discrete data updates; no perpetual scanning animation.
...
**Pointer policy: none.** Precision and readable telemetry are better served by direct control feedback.
...
- **Active / pressed:** Use 120ms state transitions and discrete data updates; no perpetual scanning animation.
...
.design .hero{display:grid;grid-template-columns:4fr 5fr;gap:28px;align-items:start;padding:32px 0}.design .hero h1{font:500 48px/1.1 system-ui,sans-serif;letter-spacing:-.03em}
.design .project-card{border:1px solid var(--control-line);border-radius:12px;box-shadow:none;clip-path:none;background:radial-gradient(circle at 100% 0,transparent 0 70px,#75bec51a 71px 72px,transparent 73px),var(--surface);padding:24px}
.design .project-rows>div{border-bottom:1px solid var(--line);padding:14px 0}.design .features{grid-template-columns:1fr;gap:16px}
.design .feature-card:nth-child(n){display:grid;grid-template-columns:100px 1fr;gap:8px 20px;clip-path:none;border:1px solid var(--line);border-radius:8px;padding:24px}
.design .feature-index{grid-row:1/4;font-family:ui-monospace,monospace}.design .feature-card h3{margin:0}
.design .stats{border:1px solid var(--line);border-radius:8px;padding:24px}.design .stats strong{font:500 32px/1.2 ui-monospace,monospace}
.design .testimonial{text-align:left;font-size:20px;border-left:2px solid var(--accent);padding:24px}
.design .testimonial blockquote{font-size:20px}.design .demo-form{clip-path:none;border-radius:8px;padding:24px}
