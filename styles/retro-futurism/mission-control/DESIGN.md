---
name: Mission Control Retro-Futurism
slug: mission-control
family: retro-futurism
description: "A command-room interpretation of an imagined analogue space program, organized around labeled banks, physical switches, and disciplined telemetry."
tags:
  - retro-futurism
  - mission-control
  - complete-system
...
# Mission Control Retro-Futurism

## Overview and Design Philosophy

A command-room interpretation of an imagined analogue space program, organized around labeled banks, physical switches, and disciplined telemetry.
...
Use a compact 4:5 opening with a 48px monospace title. The workspace is a bank of instruments. Feature sections form a single stacked operations log with serial labels.

The distinction from **Atomic Lounge Retro-Futurism** is structural: that interpretation follows this direction: Center a 68px serif title above a wide oval-edged workspace. Use generous two-column feature lounges and individual statistic medallions. Choose this variant when the composition above suits the content. A palette substitution alone does not establish either interpretation.

## Visual Character

Use warm gray metal, inset dark readouts, small amber indicators, and squared panel seams. Remove decorative orbital curves from reading areas.
...
- Buttons resemble labeled mechanical keys; selected tabs latch visibly. Inputs are inset readout fields with readable text. Dialogs use a framed instruction panel and an explicit cancel action.
...
| Token | Value | Role |
|---|---|---|
| `canvas` | `#e7dfcc` | Page environment |
| `surface` | `#fff3dc` | Opaque reading surface |
| `ink` | `#39444a` | Primary text on surface |
| `muted` | `#566368` | Secondary text on surface |
| `accent` | `#835332` | Action and selection cue |
| `on-accent` | `#ffffff` | Text on filled accent |
| `line` | `#7d898d` | Grouping rules |
| `control-line` | `#7d898d` | Control boundary |
...
Use functional diagrams with labels, no unexplained fake readings. Photography is optional and should show original equipment details.
...
Use immediate latch changes and no blinking status without a real urgent event.
...
**Pointer policy: none.** Instrument state should be conveyed by real controls rather than a decorative locator.
...
- **Active / pressed:** Use immediate latch changes and no blinking status without a real urgent event.
...
.design .hero{display:grid;grid-template-columns:4fr 5fr;gap:32px;align-items:start;padding:32px 0}.design .hero h1{font:700 48px/1.08 ui-monospace,monospace}
.design .panel{border-radius:4px;border:2px solid var(--control-line);box-shadow:inset 0 2px #ffffff66,0 4px #0003;padding:24px}
.design .project-rows{border:2px solid var(--control-line);padding:12px;background:var(--canvas)}
.design .project-rows>div{border-bottom:1px solid var(--line);font-family:ui-monospace,monospace;font-size:14px}
.design .features{grid-template-columns:1fr;gap:20px}.design .feature-card:nth-child(n){display:grid;grid-template-columns:110px 1fr;gap:8px 20px}
.design .feature-index{grid-row:1/4;font:700 13px/1.5 ui-monospace,monospace}.design .feature-card h3{margin:0}
.design .stats{border:2px solid var(--control-line);border-radius:4px;padding:24px}.design .stats strong{font:700 32px/1.2 ui-monospace,monospace}
.design :is(button,.badge){border-radius:3px}.design .testimonial{text-align:left;font:18px/1.6 ui-monospace,monospace;border:2px solid var(--line);padding:24px}
