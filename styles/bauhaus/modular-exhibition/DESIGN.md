---
name: Modular Exhibition Bauhaus
slug: modular-exhibition
family: bauhaus
description: "An exhibition-system interpretation where rectangular modules and purposeful primary-color zones organize a shared public space."
tags:
  - bauhaus
  - modular-exhibition
  - complete-system
...
# Modular Exhibition Bauhaus

## Overview and Design Philosophy

An exhibition-system interpretation where rectangular modules and purposeful primary-color zones organize a shared public space.
...
Center a 64px geometric title above a broad horizontal workspace. Align feature modules into a two-column exhibition wall with a full-width opening panel. Use a consistent 24px module gap.

The distinction from **Typographic Workshop Bauhaus** is structural: that interpretation follows this direction: Use an asymmetric opening with a 92px heavy sans heading on a narrow left measure. Set the workspace lower on the right. Feature rows have a large number rail and aligned explanatory text. Choose this variant when the composition above suits the content. A palette substitution alone does not establish either interpretation.

## Visual Character

Use paper-colored reading panels within red, yellow, and blue structural zones. A large circle marks the exhibit introduction; square corners and thin black rules hold the grid.
...
- Buttons are flat rectangular signage. Selected tabs occupy a dark module. Forms have an explicit framed station, and dialogs appear as an opaque informational panel.
...
Use original exhibit photography or geometric process diagrams. Keep captions outside image crops and make the modular grid visible without imagery.
...
Use a 140ms color transition with no tilted or bouncing modules.
...
- **Active / pressed:** Use a 140ms color transition with no tilted or bouncing modules.
...
.design .hero{display:block;background:#e8bf3c;padding:40px!important;margin:32px 0}.design .hero-copy{text-align:center;max-width:760px;margin:auto}
.design .hero h1{font:750 64px/1.05 system-ui,sans-serif}.design .hero-copy p{margin-inline:auto}.design .actions{justify-content:center}
.design .project-card{max-width:800px;margin:36px auto 0;border:2px solid;box-shadow:none;background:#f6f0dc;border-left:28px solid #294879}
.design .project-card::before,.design .project-card::after{display:none}.design .features{grid-template-columns:1fr 1fr;gap:24px}
.design .feature-card:nth-child(n){border:2px solid;background:#f6f0dc;border-top:24px solid #a62d27}.design .feature-card:first-child{grid-column:1/-1;border-top-color:#294879}
.design .feature-index{font-weight:800}.design .stats{padding:28px;border:2px solid;background:#f6f0dc}
.design .demo-form{border:2px solid;border-left:24px solid #294879}.design .testimonial{border-block:8px solid #e8bf3c;padding:36px}
