---
name: Block Editorial Neo-Brutalism
slug: block-editorial
family: neo-brutalism
description: "A publication-led variant where oversized typography and adjoining blocks take precedence over the friendly product-card rhythm."
tags:
  - neo-brutalism
  - block-editorial
  - complete-system
...
# Block Editorial Neo-Brutalism

## Overview and Design Philosophy

A publication-led variant where oversized typography and adjoining blocks take precedence over the friendly product-card rhythm.
...
Use a single broad headline above a horizontally organized workspace. Divide sections with thick black bars. Features form unequal 2:1:1 columns; the first is the lead story.

The distinction from **Utility Board Neo-Brutalism** is structural: that interpretation follows this direction: Use a compact 4:5 hero with a 46px heading and 24px gaps. Let the workspace carry the density. Feature rows form a full-width stack with a number column, explanation, and action. Choose this variant when the composition above suits the content. A palette substitution alone does not establish either interpretation.

## Visual Character

Keep the family’s hard offset and dark outlines, but remove rounded corners. Large yellow and lilac blocks act like printed sections rather than individual floating widgets.
...
- Use rectangular stamp-like actions, squared badges, and index tabs with a heavy selected border. Forms occupy a full framed column; placeholders remain understated.
...
Use a 4px base with 8/12/16/24/32/48/64/96px intervals. The wide-screen section interval is 72px; the default module gap is 24px and outer gutter is 28px. Follow the role-specific interiors in the construction rules rather than forcing every region to have identical padding.
...
Use original graphic editorial crops and large captions. Strong type should carry the design even when images are absent.
...
Use a 3px mechanical press with shadow removal. Avoid springy cards.
...
- **Active / pressed:** Use a 3px mechanical press with shadow removal. Avoid springy cards.
...
.design .hero{display:block;padding:36px 0}.design .hero-copy{border-bottom:8px solid;padding-bottom:24px}
.design .hero h1{font-size:clamp(54px,9vw,108px);font-weight:900;line-height:.96;text-transform:uppercase}
.design .project-card{margin:32px 0 0 20%;border-radius:0;background:#f5d3ba;box-shadow:8px 8px var(--ink)}
.design .features{grid-template-columns:2fr 1fr 1fr;gap:0}.design .feature-card:nth-child(n){border-radius:0;border-width:3px;box-shadow:none}
.design .feature-card:first-child{background:#e4c14a}.design .feature-index{font-size:20px;font-weight:800}
.design .stats{background:#c3b4f1;border:3px solid;padding:28px}.design .stats strong{font-size:52px}
.design :is(button,.badge){border-radius:0}.design .demo-nav{border-bottom:6px solid}
.design .testimonial{text-align:left;border:6px solid;padding:40px;background:#e4c14a}
.design .demo-form{border-radius:0;box-shadow:8px 8px var(--ink)}
