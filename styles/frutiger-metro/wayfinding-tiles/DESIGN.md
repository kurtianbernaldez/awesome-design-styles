---
name: Wayfinding Tiles Frutiger Metro
slug: wayfinding-tiles
family: frutiger-metro
description: "A navigation-led interpretation that turns flat urban geometry into a clear system of indexed destinations and modular information tiles."
tags:
  - frutiger-metro
  - wayfinding-tiles
  - complete-system
...
# Wayfinding Tiles Frutiger Metro

## Overview and Design Philosophy

A navigation-led interpretation that turns flat urban geometry into a clear system of indexed destinations and modular information tiles.
...
Use a compact 4:5 opening with a 48px title. The workspace is a route-like task list. Features stack as broad destination tiles with a stable index rail.

The distinction from **Urban Poster Frutiger Metro** is structural: that interpretation follows this direction: Use an 88px heavy title over a broad flat opening. Offset the workspace to one side. Feature regions form unequal blocks separated by thick neutral gutters. Choose this variant when the composition above suits the content. A palette substitution alone does not establish either interpretation.

## Visual Character

Use flat pale fields, dark teal boundaries, and saturated color only in index cells. Keep corners square and all depth absent.
...
- Buttons are rectangular destination signs. Selected tabs become a dark tile with a clear label. Form groups use aligned field columns; dialogs are one opaque information tile.
...
| Token | Value | Role |
|---|---|---|
| `canvas` | `#f1eddc` | Page environment |
| `surface` | `#fff9eb` | Opaque reading surface |
| `ink` | `#313b39` | Primary text on surface |
| `muted` | `#3f4e47` | Secondary text on surface |
| `accent` | `#3c6d52` | Action and selection cue |
| `on-accent` | `#ffffff` | Text on filled accent |
| `line` | `#7b8f81` | Grouping rules |
| `control-line` | `#7b8f81` | Control boundary |
...
Use simple original route diagrams and geometric icons. Directions and states must remain understandable in plain text.
...
Use a 100ms selection change with no sliding map-like background.
...
**Pointer policy: none.** Flat graphic rhythm and wayfinding remain clear without pointer-following geometry.
...
- **Active / pressed:** Use a 100ms selection change with no sliding map-like background.
...
.design .hero{ grid-template-columns:1fr 1fr; gap:36px; }

.design .hero::before{ content:''; position:absolute; top:18px; right:-100px; width:430px; height:430px; border:45px solid #df66a9; border-radius:50%; box-shadow:0 0 0 30px #8fcdd4,0 0 0 55px #2c344c; z-index:-1; }

.design h1{ font-weight:850; text-transform:lowercase; font-size:clamp(44px,7vw,84px); letter-spacing:-.06em; }

.design .project-card{ border:0; border-radius:0; background:#fbf8ee; box-shadow:none; border-left:12px solid #8fcdd4; }

.design .demo-nav{ border-bottom:12px solid #2c344c; }

.design .stats{ border:0; background:#2c344c; color:#fff; padding:28px; }

.design .stats>div>span{ color:#fff; }

.design .feature-card{ border:0; box-shadow:none; border-radius:0; border-top:14px solid #df66a9; }

.design .feature-card:nth-child(2){ border-color:#8fcdd4; }

.design .feature-card:nth-child(3){ border-color:#dbbb4c; }
...
.design .hero{display:grid;grid-template-columns:4fr 5fr;gap:28px;align-items:start;padding:32px 0}.design .hero h1{font:700 48px/1.08 system-ui,sans-serif}
.design .project-card{border:0;border-left:12px solid #2e6967;background:#e9ede4;border-radius:0;padding:24px}
.design .project-rows>div{border:0;border-bottom:2px solid #819d94;padding:16px 0}
.design .features{grid-template-columns:1fr;gap:16px}.design .feature-card:nth-child(n){display:grid;grid-template-columns:104px 1fr;gap:8px 24px;border:0;border-radius:0;background:#c6ddd5;padding:24px}
.design .feature-index{grid-row:1/4;background:#e4bc50;padding:16px;font-weight:800}.design .feature-card h3{margin:0}
.design .stats{background:#e9ede4;padding:24px;border-left:12px solid #2e6967}.design .stats strong{font-size:32px}
.design .testimonial{text-align:left;border-left:12px solid #e4bc50;padding:24px;font-size:20px}
.design .testimonial blockquote{font-size:20px}.design .demo-form{background:#e9ede4;border-radius:0;border:0;border-left:12px solid #2e6967}
