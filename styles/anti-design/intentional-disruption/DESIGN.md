...
family: anti-design
description: "Intentional disruption of polished conventions through mismatched editorial scale, offbeat alignment, and visible irregular framing."
tags:
  - anti-design
...
Use a 1.4:1 hero with a 96px serif headline. Inset the monospace lead by 18%; delay the workspace vertically by 72px. Feature columns follow 1:1.5:1 with a lowered center card and a small tilt on the third. Source order remains sensible.
...
- Use matte paper, a lime pasted eyebrow, lilac dashed workspace frame, and a mixture of solid and dashed region boundaries.
- Let the control construction explain state and hierarchy.
- Do not use arbitrary broken behavior, overlapping form labels, or an ordinary card grid with only a quirky font.
...
Use matte paper, a lime pasted eyebrow, lilac dashed workspace frame, and a mixture of solid and dashed region boundaries. The variation is deliberate and limited; there is still one stable grammar for actual controls.
...
Buttons, tabs, and form controls keep conventional semantics and consistent hit areas. Menus do not run away; dialogs are centered and dismissible. Disruption belongs to presentation, not to broken interactions.
...
Use original unexpected crops or awkward editorial juxtaposition if content needs imagery. Keep all essential text live and selectable.
...
Use immediate state changes. Static irregularity is enough; never make the reading order random between visits.
...
- **Active / pressed:** Use immediate state changes. Static irregularity is enough; never make the reading order random between visits.
