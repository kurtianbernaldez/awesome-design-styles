...
slug: chrome-pop
family: y2k
description: "Future-pop chrome and glossy plastic, with liquid silver surfaces and compressed italic display type."
tags:
  - y2k
  - chrome-pop
  - complete-system
...
Use a strong two-column opening and an italic 90px headline with tight tracking. The navigation is a long chrome capsule; the workspace alternates broad and tight corners. Keep a distinct inset header inside the reflective panel.
...
- Use a lavender environment with silver gradients that contain a sharp light/dark reflection transition.
- Let the control construction explain state and hierarchy.
- Do not replace chrome with generic purple glass, or use a rainbow gradient without a reflective light transition.
...
Use a lavender environment with silver gradients that contain a sharp light/dark reflection transition. Workspace fill moves from white to cool silver and back; controls use pink plastic gloss. A chrome surface needs an internal reflection band, not only a pale fill.
...
Primary and icon actions are glossy pills with a fine rim and lower lip. Inputs are readable matte insets. Feature panels have asymmetrical rounded corners; badges are compact tracked labels. Dialogs use restrained chrome framing around a calm interior.
...
Use original liquid-metal or translucent-plastic imagery when useful. Do not borrow brand gadgets, wordmarks, or proprietary futuristic fonts.
...
Use 140ms press and color feedback; reflections remain static. Reduced motion keeps material but removes travel.
...
**Pointer policy: page.** A moving chrome flare supports the expressive digital metallic environment.

### Chrome flare

A restrained metallic flare turns with the pointer over the chrome. This is optional decorative feedback, not a status indicator.

**Construction.** Use a compressed ellipse of alternating transparent and white conic highlights at 25% opacity. Its specular angle follows pointer direction. Keep the effect local and avoid a full-screen metallic wash. Unless a material-specific size above overrides it, use a 96px-wide region, height equal to width / 1, and a 1px base stroke. Sparse ink geometry defaults to 16% paint opacity; minimalist work uses 12%, brutalist work 20%, Art Deco 28%, and Wabi-Sabi 10%. Explicit recipe opacity takes precedence. Apply a further 70% opacity to the page-level geometry; glass uses its explicit light values. Use this document’s accent and ink tokens.
...
- **Active / pressed:** Use 140ms press and color feedback; reflections remain static. Reduced motion keeps material but removes travel.
...
Background reference: [CARI: Y2K Aesthetic](https://cari.institute/aesthetics/y2k-aesthetic). The rules in this document are an original interface interpretation, not a reproduction of a source artifact or a claim that this variant exhausts the family.
