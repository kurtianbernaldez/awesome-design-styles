---
name: Browser Collage Anti-Design
slug: browser-collage
family: anti-design
description: "A deliberately incongruous browser-native interpretation that juxtaposes plain controls, colored blocks, and oversized notices while preserving reliable task behavior."
tags:
  - anti-design
  - browser-collage
  - complete-system
...
# Browser Collage Anti-Design

## Overview and Design Philosophy

A deliberately incongruous browser-native interpretation that juxtaposes plain controls, colored blocks, and oversized notices while preserving reliable task behavior.
...
Use a compact 4:5 opening with a default-serif 60px title. The workspace has a stark outlined browser-like frame. Feature blocks vary their border treatments but share readable source order.

The distinction from **Typographic Rebellion Anti-Design** is structural: that interpretation follows this direction: Use a 112px uppercase headline across the page, followed by a small offset workspace. Features stack as oversized numbered statements with compact explanations. Keep line breaks responsive. Choose this variant when the composition above suits the content. A palette substitution alone does not establish either interpretation.

## Visual Character

Use plain white, sharp pale color blocks, mismatched solid/double borders, and no polished unified card system. Incongruity is deliberate, not accidental low contrast.
...
- Keep native-looking rectangular buttons and fields. Tabs are obvious underlined labels. Dialogs are plainly framed and accessible; surprise must never hide dismissal.
...
Use original image crops only where they add useful juxtaposition. Avoid visual pranks, deceptive controls, and meaningless cursor changes.
...
Use immediate feedback; no motion that moves targets away from users.
...
- **Active / pressed:** Use immediate feedback; no motion that moves targets away from users.
...
.design .hero{display:grid;grid-template-columns:4fr 5fr;gap:24px;align-items:start;padding:32px 0}.design .hero h1{font:700 60px/1.05 'Times New Roman',serif;text-transform:none}
.design .hero-copy{background:#e7ed9e;border:1px solid;padding:20px}.design .project-card{transform:none;border:8px double;border-radius:0;background:#fff;box-shadow:none;padding:24px}
.design .features{grid-template-columns:1fr 1fr;gap:24px}.design .feature-card:nth-child(n){transform:none;border:1px solid;border-radius:0;background:#eadcf0;box-shadow:none}
.design .feature-card:nth-child(2){border:6px double;background:#d6e9ef}.design .feature-card:nth-child(3){border:4px dashed;background:#e7ed9e}
.design .stats{border:1px solid;padding:24px;background:#fff}.design .stats strong{font:700 40px/1.2 'Times New Roman',serif}
.design :is(button,.badge){border-radius:0}.design .testimonial{text-align:left;font:italic 26px/1.5 'Times New Roman',serif;border:1px solid;padding:24px}
.design .testimonial blockquote{font:italic 26px/1.5 'Times New Roman',serif}
.design .demo-form{transform:none;border:3px solid;background:#fff;border-radius:0;box-shadow:none}
