---
name: Festival Poster Maximalism
slug: festival-poster
family: maximalism
description: "A public-facing, typographic interpretation that concentrates abundance into a loud poster opening instead of a collection of small ornamental frames."
tags:
  - maximalism
  - festival-poster
  - complete-system
...
# Festival Poster Maximalism

## Overview and Design Philosophy

A public-facing, typographic interpretation that concentrates abundance into a loud poster opening instead of a collection of small ornamental frames.
...
Center the opening headline at 96px above a broad workspace. Repeat oversized numbers, contrasting bands, and tightly grouped announcements. Keep supporting prose narrow within each colored region.

The distinction from **Cabinet of Curiosities** is structural: that interpretation follows this direction: Use an asymmetric 4:5 opening with a smaller 60px serif title. The workspace becomes an inset specimen sheet. Feature groups resemble differently sized catalogue compartments, with captions aligned inside their own frames. Choose this variant when the composition above suits the content. A palette substitution alone does not establish either interpretation.

## Visual Character

Use plum, butter, and coral as large flat fields. A double frame surrounds the opening; a repeated stripe appears only at section boundaries. Wide dark outlines hold the dense composition together.
...
- Actions are large outlined tickets, tabs form a row of caption strips, and inputs are plain writable slots inside the exuberant frame. Never give a passive label the same hard shadow as an actionable ticket.
...
| Token | Value | Role |
|---|---|---|
| `canvas` | `#f3dcb9` | Page environment |
| `surface` | `#fff2d9` | Opaque reading surface |
| `ink` | `#392638` | Primary text on surface |
| `muted` | `#634056` | Secondary text on surface |
| `accent` | `#843254` | Action and selection cue |
| `on-accent` | `#ffffff` | Text on filled accent |
| `line` | `#77536b` | Grouping rules |
| `control-line` | `#77536b` | Control boundary |
...
| Section heading | `ui-serif, Georgia, serif` | 40px / 1.2; 28px on phones |
...
Favor original festival-like typographic graphics and large captions. Use at most one focal image per region; abundance comes from composition, not a pile of unrelated stock images.
...
Use immediate ticket depression and a 120ms underline. Decorative bands remain still.
...
**Pointer policy: none.** The composition already contains abundant detail; a tracking ornament would compete with it.
...
- **Active / pressed:** Use immediate ticket depression and a 120ms underline. Decorative bands remain still.
...
.design body{ background:repeating-linear-gradient(90deg,#eee0b9 0 24px,#decda4 24px 48px); }

.design .demo-shell{ border-inline:12px double #843254; background:#f6dfb8; }

.design .demo-nav{ border-bottom:8px double; }

.design .hero{ gap:36px; padding:40px 0; background:radial-gradient(circle at 12px 12px,#84325422 2px,transparent 3px) 0 0/24px 24px; }

.design .hero-copy{ background:#f6dfb8; border-block:8px double; padding:28px 16px; }

.design h1{ font-size:clamp(46px,6.4vw,78px); font-style:italic; text-shadow:2px 2px #f2ba71; }

.design .project-card{ background:#eac1d4; border:8px double #843254; border-radius:50% 50% 12px 12px / 45px 45px 12px 12px; padding:40px 24px 24px; transform:rotate(2deg); box-shadow:9px 9px #67876b; }

.design .badge{ background:#f7d86f; border:2px solid; }

.design .features{ grid-template-columns:1.4fr 1fr 1fr; gap:20px; }

.design .feature-card{ border:6px double; background:#e2e6bf; }

.design .feature-card:nth-child(2){ background:#ecc3d2; transform:rotate(-2deg); }

.design .feature-card:nth-child(3){ background:#f6d278; transform:rotate(2deg); }

.design .testimonial{ border:10px double; background:#e6c4d7; padding:40px; }

.design .stats{ border:5px double; padding:24px; background:#dde1b6; }
...
.design .hero{display:block;border:8px double var(--ink);padding:32px!important;margin:28px 0;background:#f1b394}
.design .hero-copy{text-align:center;border:0;background:transparent;padding:8px}
.design .hero h1{font-family:Georgia,serif;font-size:clamp(48px,9vw,96px);font-style:normal;line-height:1;text-shadow:3px 3px #f7d86f}
.design .hero-copy p{margin-inline:auto}.design .actions{justify-content:center}
.design .project-card{max-width:800px;margin:40px auto 0;border:5px solid;border-radius:0;transform:none;box-shadow:10px 10px #843254;background:#f7d86f}
.design .project-rows{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:16px}
.design .project-rows>div{display:block;border-top:3px solid}
.design .features{grid-template-columns:1fr 1fr;gap:24px}.design .feature-card:first-child{grid-column:1/-1}
.design .feature-index{font-size:24px;font-weight:800}.design .feature-card:nth-child(n){transform:none;border:4px solid;border-radius:0}
.design .stats{background:#f1b394;border:6px double}.design .stats strong{font-size:64px}
.design .primary{border:3px solid var(--ink);box-shadow:5px 5px var(--ink);border-radius:0}
.design .demo-tabs{border-bottom:6px double}.design .demo-form{border:6px double;border-radius:0;background:#f7d86f}
