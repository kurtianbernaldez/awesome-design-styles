---
name: Kinetic Type Futurism
slug: kinetic-type
family: futurism
description: "A speed-oriented interpretation that expresses forward motion through slanted display type and directional bands rather than a dense science-fiction dashboard."
tags:
  - futurism
  - kinetic-type
  - complete-system
...
# Kinetic Type Futurism

## Overview and Design Philosophy

A speed-oriented interpretation that expresses forward motion through slanted display type and directional bands rather than a dense science-fiction dashboard.
...
Use a broad 96px condensed italic headline above a long workspace. Feature sections form horizontal velocity bands. Keep body text upright and all controls aligned to the page grid.

The distinction from **Orbital Data Futurism** is structural: that interpretation follows this direction: Use a compact 4:5 opening with a 48px medium sans title. Workspace and statistics carry the highest information density. Feature rows use narrow index rails and measured gutters. Choose this variant when the composition above suits the content. A palette substitution alone does not establish either interpretation.

## Visual Character

Use dark smooth planes, a single luminous edge, and clipped abstract corner accents. Directional geometry belongs to container edges; avoid a glowing outline around every word.
...
- Buttons are firm directional plaques with upright labels. Tabs use a continuous progress-like underline. Forms and dialogs retain opaque reading wells and obvious boundaries.
...
| Token | Value | Role |
|---|---|---|
| `canvas` | `#111e27` | Page environment |
| `surface` | `#1d303d` | Opaque reading surface |
| `ink` | `#eef6fa` | Primary text on surface |
| `muted` | `#b8cbd7` | Secondary text on surface |
| `accent` | `#9edbe6` | Action and selection cue |
| `on-accent` | `#16303a` | Text on filled accent |
| `line` | `#7995a7` | Grouping rules |
| `control-line` | `#7995a7` | Control boundary |
...
Use original motion studies or abstract directional diagrams. Do not use decorative speed lines behind paragraphs.
...
Use a 160ms 4px entry for an opened overlay only; reduced motion uses immediate appearance.
...
**Pointer policy: page.** A bounded directional streak expresses speed without moving the typography.

### Velocity streak

Three directional streaks track horizontal movement without moving the typography. This is optional decorative feedback, not a status indicator.

**Construction.** Stack three 2px accent rules in a 28px-high strip, with lengths of 100%, 70%, and 40%. Keep 30% opacity and sharp parallel alignment. Unless a material-specific size above overrides it, use a 144px-wide region, height equal to width / 1.45, and a 2px base stroke. Sparse ink geometry defaults to 16% paint opacity; minimalist work uses 12%, brutalist work 20%, Art Deco 28%, and Wabi-Sabi 10%. Explicit recipe opacity takes precedence. Apply a further 70% opacity to the page-level geometry; glass uses its explicit light values. Use this document’s accent and ink tokens.
...
- **Active / pressed:** Use a 160ms 4px entry for an opened overlay only; reduced motion uses immediate appearance.
...
.design .hero{display:block;padding:40px 0}.design .hero h1{font:700 italic 96px/.96 'Barlow Condensed',Impact,sans-serif;text-transform:uppercase;max-width:13ch}
.design .hero-copy{border-bottom:6px solid var(--accent);padding-bottom:28px}.design .project-card{max-width:850px;margin:36px 0 0 auto;border-radius:0;border-left:12px solid var(--accent);box-shadow:none;clip-path:none}
.design .features{grid-template-columns:1fr;gap:24px}.design .feature-card:nth-child(n){border-radius:0;border:0;border-top:2px solid var(--line);padding:28px 0;background:none;clip-path:none}
.design .feature-index{letter-spacing:.2em;font-weight:700}.design .stats strong{font:700 italic 56px/1 'Barlow Condensed',Impact,sans-serif}
.design .stats{border-block:2px solid var(--line);padding:28px 0}.design .primary{border-radius:0;border-left:8px solid var(--ink)}
.design .testimonial{text-align:left;border-left:8px solid var(--accent);padding:32px;font-size:28px}
.design .testimonial blockquote{font-size:28px}.design .demo-form{border-radius:0;border-top:6px solid var(--accent)}
