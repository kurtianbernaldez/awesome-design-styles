---
name: Layered Poster Grunge
slug: layered-poster
family: grunge
description: "A pasted-street-poster interpretation using broad overlapping paper regions and faded print color instead of a monochrome photocopied page."
tags:
  - grunge
  - layered-poster
  - complete-system
...
# Layered Poster Grunge

## Overview and Design Philosophy

A pasted-street-poster interpretation using broad overlapping paper regions and faded print color instead of a monochrome photocopied page.
...
Use a 3:2 opening with a 76px heavy serif title. Place the workspace against a larger backing-paper offset. Features form broad unequal panels with visible edge layering.

The distinction from **Xerox Zine Grunge** is structural: that interpretation follows this direction: Use an 88px condensed headline above an offset workspace. Feature content becomes unboxed zine columns with rough-looking rule separators. Keep body text clean and comfortably spaced. Choose this variant when the composition above suits the content. A palette substitution alone does not establish either interpretation.

## Visual Character

Use faded brick, mustard paper, graphite outlines, and hard misregistered offsets. Wear stays on outer edges; important content is never torn away or rotated.
...
- Actions are thick printed stickers. Tabs are separate paper strips. Inputs sit on untextured pale paper, and dialogs use one foreground poster sheet.
...
| Token | Value | Role |
|---|---|---|
| `canvas` | `#ddd6c5` | Page environment |
| `surface` | `#f7f0df` | Opaque reading surface |
| `ink` | `#3c352d` | Primary text on surface |
| `muted` | `#4d4238` | Secondary text on surface |
| `accent` | `#815137` | Action and selection cue |
| `on-accent` | `#ffffff` | Text on filled accent |
| `line` | `#857460` | Grouping rules |
| `control-line` | `#857460` | Control boundary |
...
Use original poster-like image crops and collage fragments with clear provenance. Do not borrow band logos, campaign copy, or recognizable commercial posters.
...
Use immediate press feedback; pasted sheets do not wobble.
...
- **Active / pressed:** Use immediate press feedback; pasted sheets do not wobble.
...
.design .hero{display:grid;grid-template-columns:3fr 2fr;gap:40px;align-items:start}.design .hero h1{font:800 76px/1.02 Georgia,serif;letter-spacing:-.05em}
.design .hero-copy{background:#d4b36c;padding:28px;border:3px solid}.design .project-card{margin-top:56px;border:3px solid;border-radius:0;transform:none;background:#eee1c4;box-shadow:12px 12px #ac6f59,15px 15px var(--ink)}
.design .features{grid-template-columns:1fr 1fr;gap:36px}.design .feature-card:nth-child(n){border:3px solid;border-radius:0;transform:none;background:#eee1c4;box-shadow:7px 7px #ac6f59}
.design .feature-card:first-child{grid-column:1/-1;background:#d4b36c}.design .feature-index{font-size:18px;font-weight:800}
.design .stats{background:#d4b36c;border:3px solid;padding:28px}.design .stats strong{font-family:Georgia,serif;font-size:48px}
.design .testimonial{text-align:left;border:0;border-left:12px solid #ac6f59;padding:32px}.design .demo-form{border:3px solid;border-radius:0;transform:none;background:#eee1c4;box-shadow:8px 8px #ac6f59}
