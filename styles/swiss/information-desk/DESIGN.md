---
name: Information Desk Swiss
slug: information-desk
family: swiss
description: "A dense service-information interpretation where small typography, tabular relationships, and consistent numbered rails support repeated tasks."
tags:
  - swiss
  - information-desk
  - complete-system
...
# Information Desk Swiss

## Overview and Design Philosophy

A dense service-information interpretation where small typography, tabular relationships, and consistent numbered rails support repeated tasks.
...
Use a compact 4:5 opening with a 44px heading. Give the workspace a clear table rhythm. Feature explanations stack in rows with a narrow fixed index column and flexible text.

The distinction from **Typographic Poster Swiss** is structural: that interpretation follows this direction: Set a 104px flush-left headline above a narrow right-aligned workspace. Large section numbers occupy their own vertical band. Text stays aligned to a small set of shared left edges. Choose this variant when the composition above suits the content. A palette substitution alone does not establish either interpretation.

## Visual Character

Use pale gray service zones, white working surfaces, and fine rules. Avoid poster-sized display type or ornamental color blocks. Color indicates a current location or action.
...
- Buttons have strict rectangular geometry, labels align consistently, and tabs resemble an index. Form groups are divided by rules rather than floating cards.
...
Use a 4px base with 8/12/16/24/32/48/64/96px intervals. The wide-screen section interval is 56px; the default module gap is 24px and outer gutter is 40px. Follow the role-specific interiors in the construction rules rather than forcing every region to have identical padding.
...
Use informational diagrams and restrained photographic crops. Charts should look like part of the same tabular system.
...
Use 100ms color feedback; data updates preserve column widths and reading position.
...
**Pointer policy: none.** The typographic grid and information hierarchy need uninterrupted reading space.
...
- **Active / pressed:** Use 100ms color feedback; data updates preserve column widths and reading position.
...
.design .hero{display:grid;grid-template-columns:4fr 5fr;gap:32px;padding:36px 0;align-items:start}.design .hero h1{font:700 44px/1.08 Arial,sans-serif}
.design .project-card{background:#edf0f1;border:1px solid #747b80;padding:24px}.design .project-rows>div{padding:16px 0;border-bottom:1px solid #a6adb0}
.design .features{grid-template-columns:1fr;gap:0}.design .feature-card:nth-child(n){display:grid;grid-template-columns:88px 1fr;gap:8px 20px;border:0;border-bottom:1px solid #a6adb0;padding:24px 0}
.design .feature-index{grid-row:1/4;font:700 14px/1.5 Arial,sans-serif}.design .feature-card h3{margin:0;font-size:22px}
.design .stats{background:#edf0f1;padding:24px;border:1px solid #747b80}.design .stats strong{font-size:32px}
.design .demo-tabs button{font-size:14px}.design .testimonial{text-align:left;font-size:20px;border-left:3px solid;padding:24px}
.design .testimonial blockquote{font-size:20px}
.design .demo-form{background:#edf0f1;border:1px solid #747b80;padding:24px}
