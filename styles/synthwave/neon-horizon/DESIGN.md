...
slug: neon-horizon
family: synthwave
description: Dark graphic planes with luminous edge accents and a confident, ordered display hierarchy.
tags:
  - synthwave
  - neon-horizon
  - complete-system
...
Dark graphic planes with luminous edge accents and a confident, ordered display hierarchy. This is one original interpretation of the synthwave family, not a definitive rulebook for the entire aesthetic. It is intended for complete websites and application surfaces that benefit from this visual character. Its layout, material choices, and control conventions below define why it exists as a separate system.

Use a centered hero introduction followed by a contained project panel and three equal feature cards. Broad horizontal sections create a horizon-like rhythm. Keep forms left aligned and pricing equal-width.
...
Use strong sans-serif display type and restrained monospaced metadata. Body is plain and opaque. Do not outline or glow long text; large headings should remain solid and readable.

| Role | Stack | Size / line height | Weight and tracking |
|---|---|---|---|
| Display / H1 | `system-ui, sans-serif` | fluid 36–72px / 1.08 | 800; -0.03em |
| H2 | `system-ui, sans-serif` | 38px / 1.2 | 800; -0.02em |
...
The outer container is at most 1200px wide, with 32px gutters and 28px gaps. Use CSS grid with `minmax(0, 1fr)` tracks so long content cannot enlarge the page. Use flex wrapping for action groups and navigation. Cards grow with content; do not enforce fixed text heights. Comparable pricing keeps the same benefit order and aligned actions even if decorative feature blocks have unequal spans.

## Surfaces, Borders, Radius, Shadows, and Depth

Use dark opaque planes with pink and cool-blue edge rails. A static deep gradient may surround the page, but panels stay solid. Light is an edge cue, never a blurred text effect or full-screen grid overlay.

Reference card boundary: 1px where a frame is used; reference corner radius: 2px. Surface shadow: `0 5px 0 #644879`. Controls generally use 2px corners unless their component treatment specifies a more precise shape. Content stays at layer 0, navigation at 10 when sticky, menus at 20, backdrop at 40, dialog at 50, and transient feedback at 60. Avoid creating stacking contexts on every card. Overlays must remain above all decorative treatments.

## Components

Primary buttons are pale pink with dark labels; secondary controls use dark surfaces and bright enough outlines. Inputs remain solid and calm. Cards have a single luminous-colored top edge without glow. Pricing uses a written recommendation and accent outline. Navigation uses a strong lower rule. Tabs invert clearly. Menus and dialogs are opaque dark panels with a fine contrasting frame and conventional actions.
...
Use original night or abstract landscape imagery in separate crops. Avoid copying music artwork, film titles, or iconic car silhouettes. Charts use bright solid strokes with direct labels and dash distinctions; keep axes accurate and provide table alternatives.
...
Use 140ms color changes. No pulsing neon, moving perspective grids, rapid color cycling, or animated sun bands. Reduced motion uses immediate updates.
...
.design-synthwave-neon-horizon * { box-sizing: border-box; }
```
...
- Keep luminous accents crisp.
- make the reading plane dark and opaque.
- maintain a broad stable rhythm.
- Preserve the readable control and feedback behavior when adapting the visual composition.

## Don't

- Do not glow body text or use endless animated scenery.
