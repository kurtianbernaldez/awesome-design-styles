---
name: Personal Homepage Web 1.0
slug: personal-homepage
family: web-1.0
description: "A personal-site interpretation with centered introductions, explicit section boxes, and cheerful plain HTML character rather than a sparse document index."
tags:
  - web-1.0
  - personal-homepage
  - complete-system
...
# Personal Homepage Web 1.0

## Overview and Design Philosophy

A personal-site interpretation with centered introductions, explicit section boxes, and cheerful plain HTML character rather than a sparse document index.
...
Center a 56px serif welcome heading above a broad bordered workspace. Arrange feature sections in two simple columns and retain visibly separate navigation links.

The distinction from **Hypertext Index Web 1.0** is structural: that interpretation follows this direction: Use a single left-aligned 44px serif heading and a narrow reading measure. Place the workspace below as a simple ruled list. Stack all feature entries as document sections. Choose this variant when the composition above suits the content. A palette substitution alone does not establish either interpretation.

## Visual Character

Use pale blue page color, white content boxes, dark blue rules, and simple double borders. Avoid modern floating cards, giant rounded corners, or gradient app shells.
...
- Buttons retain straightforward native-like bevels. Tabs are explicit section links with a selected underline. Forms and dialogs use plain boxes with conventional labels.
...
| Token | Value | Role |
|---|---|---|
| `canvas` | `#f1eee1` | Page environment |
| `surface` | `#fffbee` | Opaque reading surface |
| `ink` | `#30372d` | Primary text on surface |
| `muted` | `#606958` | Secondary text on surface |
| `accent` | `#344f99` | Action and selection cue |
| `on-accent` | `#ffffff` | Text on filled accent |
| `line` | `#7c8772` | Grouping rules |
| `control-line` | `#7c8772` | Control boundary |
...
| Section heading | `ui-serif, Georgia, serif` | 30px / 1.2; 28px on phones |
| Card heading | `ui-serif, Georgia, serif` | 23px / 1.3; variant overrides below take precedence |
...
Use original small decorative images with useful alt text where meaningful. Do not add blinking text, animated banners, or inaccessible image maps.
...
Use immediate native state feedback; decoration remains static.
...
**Pointer policy: page.** A restrained marquee references playful personal-web cursor decoration.

### Homepage marquee

A plain dashed selection frame steps across the homepage boxes. This is optional decorative feedback, not a status indicator.

**Construction.** Use a single 1px dashed ink rectangle at 40% opacity. Keep the dash phase fixed; do not animate marching ants or imply that real content is selected. Unless a material-specific size above overrides it, use a 144px-wide region, height equal to width / 1.45, and a 2px base stroke. Sparse ink geometry defaults to 16% paint opacity; minimalist work uses 12%, brutalist work 20%, Art Deco 28%, and Wabi-Sabi 10%. Explicit recipe opacity takes precedence. Apply a further 70% opacity to the page-level geometry; glass uses its explicit light values. Use this document’s accent and ink tokens.
...
- **Active / pressed:** Use immediate native state feedback; decoration remains static.
...
.design .demo-shell{ max-width:none; padding:12px 20px; }

.design .demo-nav{ display:block; min-height:0; padding:8px 0; border-bottom:2px groove; }

.design .demo-nav nav{ display:inline-flex; margin:8px 24px 8px 0; }

.design .demo-name{ display:block; font:700 24px/1.3 Georgia,serif; color:#222; }

.design .hero{ display:block; padding:24px 0; }

.design h1{ font:700 36px/1.1 'Times New Roman',serif; letter-spacing:0; }

.design h1 br{ display:contents; }

.design h1 br::after{ content:' '; }

.design .lead{ font:18px/1.5 'Times New Roman',serif; max-width:78ch; }

.design .eyebrow{ font-family:'Times New Roman',serif; letter-spacing:0; font-weight:400; }

.design :is(.panel,.project-card){ border:0; border-radius:0; box-shadow:none; background:transparent; padding:0; }

.design .project-card{ margin-top:24px; border-top:2px groove; max-width:700px; }

.design .project-card h2{ font-size:24px; margin-top:16px; }

.design .project-rows{ margin:8px 0; }

.design .project-rows>div{ border:0; padding:4px 0; }

.design .features{ display:block; }

.design .feature-card{ margin:24px 0; }

.design .feature-card h3{ font:700 24px/1.2 'Times New Roman',serif; }

.design .stats{ display:flex; gap:28px; border:2px groove; padding:16px; width:fit-content; max-width:100%; }

.design .stats strong{ font-size:24px; }

.design :is(button,.primary,.secondary){ background:#e6e6e6; color:#111; border:2px outset #eee; border-radius:0; font:14px Arial,sans-serif; }

.design button:active{ border-style:inset; }

.design a{ color:#243dad; text-decoration:underline; }

.design a:visited{ color:#71388b; }

.design .section{ margin-top:32px; }

.design .testimonial{ text-align:left; padding:16px 32px; border:0; }
...
.design body{background:#e5edf5}.design .hero{display:block;padding:32px 0}.design .hero-copy{text-align:center;max-width:760px;margin:auto}
.design .hero h1{font:700 56px/1.1 'Times New Roman',serif;color:#263d73}.design .hero-copy p{margin-inline:auto}.design .actions{justify-content:center}
.design .project-card{max-width:820px;margin:32px auto 0;border:5px double #53688c;background:#fff;border-radius:0;box-shadow:none;padding:24px}
.design .features{grid-template-columns:1fr 1fr;gap:24px}.design .feature-card:nth-child(n){border:2px solid #53688c;background:#fff;padding:24px}
.design .stats{border:5px double #53688c;background:#fff;padding:24px}.design .stats strong{font:700 36px/1.2 'Times New Roman',serif}
.design .demo-nav{justify-content:center;border-block:2px solid #53688c;padding:16px;gap:24px}
.design .testimonial{font:italic 24px/1.6 'Times New Roman',serif;border:2px solid #53688c;background:#fff;padding:24px}
.design .testimonial blockquote{font:italic 24px/1.6 'Times New Roman',serif}.design .demo-form{border:5px double #53688c;background:#fff;padding:24px}
