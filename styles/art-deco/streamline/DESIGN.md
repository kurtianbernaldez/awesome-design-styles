...
slug: streamline
family: art-deco
description: "A transport-inspired interpretation emphasizing long horizontal bands, rounded ends, and polished restraint instead of stepped theatrical ornament."
tags:
  - art-deco
  - streamline
  - complete-system
...
A transport-inspired interpretation emphasizing long horizontal bands, rounded ends, and polished restraint instead of stepped theatrical ornament.
...
Use a broad 3:2 opening with a 64px sans title. Stretch the workspace into a horizontal cabin-like region. Align feature strips along continuous horizontal rules.

The distinction from **Ornamental Marquee Art Deco** is structural: that interpretation follows this direction: Center an 80px uppercase title inside a double-framed opening. Place a wide workspace below it. Use mirrored two-column features with a full-width lead panel. Choose this variant when the composition above suits the content. A palette substitution alone does not establish either interpretation.

## Visual Character

Use dark teal, pale metallic cream, rounded terminal corners, and repeated horizontal pinstripes. Replace sharp fan ornaments with parallel bands; metallic highlights stay narrow.
...
- Actions are long capsules with double outlines. Tabs share a continuous rail. Forms are clean inset regions; dialogs use one rounded metal-edged panel.
...
| Token | Value | Role |
|---|---|---|
| `canvas` | `#1f292b` | Page environment |
| `surface` | `#293638` | Opaque reading surface |
| `ink` | `#f3ead5` | Primary text on surface |
| `muted` | `#cbbf9f` | Secondary text on surface |
| `accent` | `#dfc18a` | Action and selection cue |
| `on-accent` | `#292d2b` | Text on filled accent |
| `line` | `#9c9177` | Grouping rules |
| `control-line` | `#9c9177` | Control boundary |
...
Use a 4px base with 8/12/16/24/32/48/64/96px intervals. The wide-screen section interval is 96px; the default module gap is 32px and outer gutter is 40px. Follow the role-specific interiors in the construction rules rather than forcing every region to have identical padding.
...
Use original architectural or travel imagery with horizontal crops. Avoid brand emblems or decorative pseudo-historical badges.
...
Use a smooth 180ms color change; no sweeping chrome animation.
...
**Pointer policy: surface.** Light can travel along polished trim while the page composition remains still.

### Polished speedlines

Three narrow metallic lines glide horizontally like a polished trim reflection. This is decorative material feedback, not a status indicator.
...
- **Active / pressed:** Use a smooth 180ms color change; no sweeping chrome animation.
...
.design .hero{display:grid;grid-template-columns:3fr 2fr;gap:40px}.design .hero h1{font:500 64px/1.08 system-ui,sans-serif;letter-spacing:.02em;text-transform:none}
.design .panel{border:3px double var(--line);border-radius:48px 8px 48px 8px;background:var(--surface);box-shadow:none}
.design .panel::before,.design .panel::after{display:none}.design .project-card{border-top:9px double var(--line);padding:36px}
.design .features{grid-template-columns:1fr;gap:24px}.design .feature-card:nth-child(n){padding:28px 40px;border-inline:0;border-radius:0;border-block:3px double}
.design .primary,.design .secondary{border-radius:999px;padding-inline:28px}.design .demo-nav{border-block:6px double var(--line)}
.design .stats{border-block:6px double var(--line);padding:28px}.design .stats strong{font-family:system-ui,sans-serif;font-size:40px}
.design .demo-tabs{border-bottom:6px double}.design .testimonial{border-inline:0;border-block:3px double;padding:40px}
.design dialog{border-radius:36px 8px;border:3px double var(--line)}
.design body{background:repeating-linear-gradient(0deg,transparent 0 48px,#c8aa6b0a 48px 50px),#202d30}.design .project-card{outline:0}.design .hero-copy :is(p,.actions){margin-inline:0;justify-content:flex-start}
