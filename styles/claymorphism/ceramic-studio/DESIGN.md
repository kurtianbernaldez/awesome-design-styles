---
name: Ceramic Studio Claymorphism
slug: ceramic-studio
family: claymorphism
description: "A quieter sculptural interpretation that favors broad matte vessels and restrained type over colorful toy-like pieces."
tags:
  - claymorphism
  - ceramic-studio
  - complete-system
...
# Ceramic Studio Claymorphism

## Overview and Design Philosophy

A quieter sculptural interpretation that favors broad matte vessels and restrained type over colorful toy-like pieces.
...
Use a 3:2 opening with a regular 66px serif title. The workspace becomes a wide shallow vessel. Feature groups use one generous column with a sculpted outer edge, leaving broad space between reading groups.

The distinction from **Toy Workshop Claymorphism** is structural: that interpretation follows this direction: Use a 1:1 opening with a heavy 64px rounded sans title. The workspace is assembled from visibly separate colored task pieces. Features become a compact board with large numbered tiles. Choose this variant when the composition above suits the content. A palette substitution alone does not establish either interpretation.

## Visual Character

Use warm porcelain and clay-like matte surfaces with paired inner shading and a soft cast shadow. Corners are large and asymmetrical, suggesting molded objects rather than machine-perfect pill buttons.
...
- Actions are low, broad molded ovals. Inputs are smooth inset wells. Tabs share one quiet slab; selected state uses a shallow recess. Menus and dialogs use the same ceramic volume with no glossy highlight band.
...
| Token | Value | Role |
|---|---|---|
| `canvas` | `#e6dccf` | Page environment |
| `surface` | `#f0e4d5` | Opaque reading surface |
| `ink` | `#463b34` | Primary text on surface |
| `muted` | `#65554a` | Secondary text on surface |
| `accent` | `#775141` | Action and selection cue |
| `on-accent` | `#ffffff` | Text on filled accent |
| `line` | `#b2a08b` | Grouping rules |
| `control-line` | `#897361` | Control boundary |
...
|---|---|---|
| Display | `Georgia,serif`; use the specific heading override in the construction rules | Follow the opening size above; 36px on small phones |
| Section heading | `Georgia,serif` | 34px / 1.2; 28px on phones |
| Card heading | `Georgia,serif` | 24px / 1.3; variant overrides below take precedence |
...
Use original ceramic material photography or simple matte objects if needed. No shiny plastic or illustrated stickers.
...
Use 180ms color feedback and an immediate small compression. Keep the resting objects still.
...
- **Active / pressed:** Use 180ms color feedback and an immediate small compression. Keep the resting objects still.
...
.design body{background:#e6dccf}.design .hero{display:grid;grid-template-columns:3fr 2fr;gap:56px;padding:72px 0}
.design .hero h1{font:400 66px/1.1 Georgia,serif;letter-spacing:-.04em}
.design .panel{background:#f0e4d5;box-shadow:inset 8px 8px 18px #fff9,inset -10px -10px 20px #bfa98c66,12px 18px 28px #8c755833;border-radius:48px 28px 40px 24px}
.design .project-card{background:#ead7c2;padding:36px}
.design .features{grid-template-columns:1fr;gap:36px}.design .feature-card:nth-child(n){display:grid;grid-template-columns:130px 1fr;gap:8px 28px;background:#f0e4d5!important;padding:36px}
.design .feature-index{grid-row:1/4}.design .feature-card h3{margin:0;font-family:Georgia,serif;font-weight:400}
.design .stats{border:0;gap:32px}.design .stats strong{font-family:Georgia,serif;font-weight:400;font-size:44px}
.design .primary,.design .secondary{border-radius:50%/25%;box-shadow:inset 3px 4px 8px #fff4,inset -3px -4px 8px #573f3333,0 5px 9px #573f3322}
.design .demo-tabs{background:#ead7c2;border:0;border-radius:24px;padding:8px}
.design .demo-tabs button[aria-selected=true]{background:#f0e4d5;box-shadow:inset 3px 3px 7px #bfa98c88;border-radius:20px}
.design .testimonial{font-family:Georgia,serif;font-style:italic;text-align:left;border:0}
