...
description: Bright optimistic surfaces with gentle gloss, clear blues, and approachable rounded controls.
tags:
  - frutiger-aero
  - clear-horizon
  - complete-system
...
Bright optimistic surfaces with gentle gloss, clear blues, and approachable rounded controls. This is one original interpretation of the frutiger aero family, not a definitive rulebook for the entire aesthetic. It is intended for complete websites and application surfaces that benefit from this visual character. Its layout, material choices, and control conventions below define why it exists as a separate system.

Use balanced hero halves and three equal features. The page opens brightly and keeps a stable horizontal rhythm. Let a contained project pane carry the glossy edge; reading sections and forms remain calm.
...
Use open sans-serif headings with medium weight and a friendly 17px body. Avoid ultralight text on bright backgrounds. Metadata stays readable and secondary, not microscopic.

| Role | Stack | Size / line height | Weight and tracking |
|---|---|---|---|
| Display / H1 | `system-ui, sans-serif` | fluid 36–58px / 1.08 | 650; -0.03em |
| H2 | `system-ui, sans-serif` | 32px / 1.2 | 650; -0.02em |
...
The outer container is at most 1140px wide, with 36px gutters and 28px gaps. Use CSS grid with `minmax(0, 1fr)` tracks so long content cannot enlarge the page. Use flex wrapping for action groups and navigation. Cards grow with content; do not enforce fixed text heights. Comparable pricing keeps the same benefit order and aligned actions even if decorative feature blocks have unequal spans.

## Surfaces, Borders, Radius, Shadows, and Depth

Use bright opaque blue-white panels, modest 18px corners, and soft top highlights. A static sky-like blue wash may surround the panels. No transparent text planes, lens flare, or uncontrolled scenic backdrop beneath forms.

Reference card boundary: 1px where a frame is used; reference corner radius: 18px. Surface shadow: `inset 0 2px 0 #ffffff,0 8px 20px #37647c22`. Controls generally use 12px corners unless their component treatment specifies a more precise shape. Content stays at layer 0, navigation at 10 when sticky, menus at 20, backdrop at 40, dialog at 50, and transient feedback at 60. Avoid creating stacking contexts on every card. Overlays must remain above all decorative treatments.

## Components

Primary buttons are deep blue with white labels and a subtle upper highlight. Secondary buttons are bright outlined surfaces. Inputs have solid blue-gray boundaries and visible labels. Cards use gentle gloss only at the top edge. Pricing recommendation uses a text badge and stronger border. Navigation is a bright rounded strip; selected tabs are blue. Menus and dialogs use opaque near-white surfaces with a soft shadow and a visible outline.
...
Original nature or water imagery can sit in separate media frames, with descriptive alt text and captions. Avoid stock globe/swoosh combinations that mimic a specific identity. Icons use friendly clear strokes. Charts use solid marks on white, direct labels, and patterns as needed; supply table alternatives.
...
Use 160ms ease-out color changes and a short dialog fade. Backgrounds remain static. No drifting bubbles, ripples behind controls, or animated highlights. Reduced motion makes states immediate.
...
```css
.design-frutiger-aero-clear-horizon {
  --canvas: #dcedf4;
  --surface: #f8feff;
  --ink: #28495a;
  --muted: #526f7b;
...
.design-frutiger-aero-clear-horizon * { box-sizing: border-box; }
```
...
- use clear blue actions sparingly.
- Preserve the readable control and feedback behavior when adapting the visual composition.

## Don't

- Do not put white text on pale gloss or simulate a branded operating system.
