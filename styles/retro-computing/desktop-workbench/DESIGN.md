---
name: Desktop Workbench Retro-Computing
slug: desktop-workbench
family: retro-computing
description: Explicit window frames, inset fields, and concise utility labels with modern accessible behavior.
tags:
  - retro-computing
  - desktop-workbench
  - complete-system
...
# Desktop Workbench Retro-Computing

## Overview and Design Philosophy

Explicit window frames, inset fields, and concise utility labels with modern accessible behavior. This is one original interpretation of the retro computing family, not a definitive rulebook for the entire aesthetic. It is intended for complete websites and application surfaces that benefit from this visual character. Its layout, material choices, and control conventions below define why it exists as a separate system.

Use a 3:2 hero and two-column workbench modules. Keep windows aligned in the document rather than freely draggable. Task reading order is stable; no overlapping panels or simulated desktop clutter.
...
| Token | Value | Role |
|---|---|---|
| `canvas` | `#bcc9c8` | Page environment; ink text |
| `surface` | `#e1e6df` | Opaque reading and control plane |
| `ink` | `#283b3a` | Main text on canvas or surface |
| `muted` | `#526762` | Secondary text on surface |
| `accent` | `#355f6c` | Primary action and selected state |
| `on-accent` | `#ffffff` | Text on the accent fill |
| `line` | `#718580` | Grouping rules |
| `control-line` | `#718580` | Visible input and control boundaries |
...
Use monospaced headings and utility labels with sans long-form body. Keep body 16px and captions at least 13px. Do not shrink typography to historical pixel sizes or force a bitmap font.

| Role | Stack | Size / line height | Weight and tracking |
|---|---|---|---|
| Display / H1 | `ui-monospace, monospace` | fluid 36–48px / 1.08 | 700; -0.03em |
| H2 | `ui-monospace, monospace` | 28px / 1.2 | 700; -0.02em |
| H3 | `ui-monospace, monospace` | 20px / 1.3 | 650; normal |
| Body | `system-ui, sans-serif` | 16px / 1.65 | 400; normal |
...
Use 4/8/12/16/20/32/48/56px. Window interiors are compact but never below 16px. Controls retain 44px touch targets and wrap for long labels.

Use 8px from label to field, 4–8px from field to help, 20px between form fields, and 24–32px between task groups. Standard controls have 12px vertical and 20px horizontal padding with a minimum 48px height. Card padding is 20px unless the surface guidance gives a stronger role-specific value. Separate unrelated page sections by 56px at wide widths. Convert these reference pixels to rem in implementations that respect user text settings.

## Layout and Grid

Use a 3:2 hero and two-column workbench modules. Keep windows aligned in the document rather than freely draggable. Task reading order is stable; no overlapping panels or simulated desktop clutter.

The outer container is at most 1120px wide, with 24px gutters and 20px gaps. Use CSS grid with `minmax(0, 1fr)` tracks so long content cannot enlarge the page. Use flex wrapping for action groups and navigation. Cards grow with content; do not enforce fixed text heights. Comparable pricing keeps the same benefit order and aligned actions even if decorative feature blocks have unequal spans.

## Surfaces, Borders, Radius, Shadows, and Depth

Use opaque muted desktop fields and simple light/dark edge pairs. Window headers are generic, not copied operating-system chrome. Insets clarify fields; all controls also have visible borders and labels.

Reference card boundary: 2px where a frame is used; reference corner radius: 0px. Surface shadow: `inset 2px 2px 0 #ffffff,inset -2px -2px 0 #82948f,3px 3px 0 #82948f`. Controls generally use 0px corners unless their component treatment specifies a more precise shape. Content stays at layer 0, navigation at 10 when sticky, menus at 20, backdrop at 40, dialog at 50, and transient feedback at 60. Avoid creating stacking contexts on every card. Overlays must remain above all decorative treatments.

## Components

Primary buttons are dark teal with white labels and a raised edge. Secondary buttons use the pale window surface. Inputs are inset with strong boundaries. Cards have a generic title strip and clean content area. Pricing remains a normal comparison. Navigation uses labeled text controls. Selected tabs connect visually to the panel. Menus and dialogs use clear borders, visible Close/Cancel actions, Escape behavior, and native focus handling.
...
Use original simple pixel-like icons or clear line icons, with accessible names. Avoid copied system icons, startup screens, and software logos. Charts are flat, labeled, and accurate with data tables. Never render meaningful text into tiny raster images.
...
Charts are flat, labeled, and accurate with data tables. Never render meaningful text into tiny raster images.
...
Use immediate states. No blinking cursors, startup sequence, fake loading delay, or system sounds. Reduced motion is identical to the default.
...
```css
.design-retro-computing-desktop-workbench {
  --canvas: #bcc9c8;
  --surface: #e1e6df;
  --ink: #283b3a;
  --muted: #526762;
...
.design-retro-computing-desktop-workbench * { box-sizing: border-box; }
```
...
- Keep the workbench functional.
- enlarge historical affordances for modern access.
- use generic original chrome.
- Preserve the readable control and feedback behavior when adapting the visual composition.

## Don't

- Do not clone an operating system or make users manage overlapping windows.
