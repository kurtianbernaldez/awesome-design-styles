---
name: Nature Portal Frutiger Aero
slug: nature-portal
family: frutiger-aero
description: "A panoramic ecological-technology interpretation where an optimistic landscape anchors a spacious public portal instead of a compact control shell."
tags:
  - frutiger-aero
  - nature-portal
  - complete-system
...
# Nature Portal Frutiger Aero

## Overview and Design Philosophy

A panoramic ecological-technology interpretation where an optimistic landscape anchors a spacious public portal instead of a compact control shell.
...
Center a 68px heading above a broad workspace and large landscape strip. Use spacious two-column features, a full-width first feature, and wide transparent section dividers.

The distinction from **Aquatic Workspace Frutiger Aero** is structural: that interpretation follows this direction: Use a compact 4:5 opening with a 56px sans heading. The workspace is a rounded clear-blue housing with a panoramic water-like image strip. Features form tidy two-column glassy modules. Choose this variant when the composition above suits the content. A palette substitution alone does not establish either interpretation.

## Visual Character

Use bright sky, fresh green, white glossy panels, and rounded transparent edges. Landscape provides the environmental atmosphere while text rests on white surfaces.
...
- Primary actions are glossy green capsules. Tabs use a rounded green selection. Forms and dialogs are clean white panels with a soft sky-colored edge.
...
Use the original nature artwork as a panoramic scene with a stable crop. Pair every meaningful environmental claim with text; scenery is not evidence.
...
Use 200ms overlay fades and no animated background landscape.
...
- **Active / pressed:** Use 200ms overlay fades and no animated background landscape.
...
.design .hero{display:block;padding:48px 0}.design .hero-copy{text-align:center;max-width:780px;margin:auto}.design .hero h1{font:600 68px/1.08 system-ui,sans-serif}
.design .hero-copy p{margin-inline:auto}.design .actions{justify-content:center}.design .project-card{max-width:900px;margin:40px auto 0;border-radius:32px;background:#ffffffed;border:2px solid #8ebba0;padding:32px}
.design .style-artwork{height:260px;border-radius:24px;background-position:center 60%}.design .features{grid-template-columns:1fr 1fr;gap:32px}
.design .feature-card:nth-child(n){border-radius:28px;background:#f8fff7;border:2px solid #8ebba0}.design .feature-card:first-child{grid-column:1/-1}
.design .primary,.design .primary:hover:not(:disabled){background:linear-gradient(#56873b,#315f31);color:white;border-color:#315f31;box-shadow:inset 0 2px #a5ce86,0 3px 7px #315f3144}
.design .stats{border-radius:28px;padding:32px;background:#eff9e9;border:2px solid #8ebba0}.design .demo-form{background:#f8fff7;border:2px solid #8ebba0;border-radius:28px}
.design .testimonial{border:0;font-size:26px}
.design .testimonial blockquote{font-size:26px}
