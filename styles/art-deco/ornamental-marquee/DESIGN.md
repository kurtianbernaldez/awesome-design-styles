---
name: Ornamental Marquee Art Deco
slug: ornamental-marquee
family: art-deco
description: "A theatrical interpretation using centered monumental type, stepped frames, and symmetrical marquee bands to announce a special occasion."
tags:
  - art-deco
  - ornamental-marquee
  - complete-system
...
# Ornamental Marquee Art Deco

## Overview and Design Philosophy

A theatrical interpretation using centered monumental type, stepped frames, and symmetrical marquee bands to announce a special occasion.
...
Center an 80px uppercase title inside a double-framed opening. Place a wide workspace below it. Use mirrored two-column features with a full-width lead panel.

The distinction from **Streamline Art Deco** is structural: that interpretation follows this direction: Use a broad 3:2 opening with a 64px sans title. Stretch the workspace into a horizontal cabin-like region. Align feature strips along continuous horizontal rules. Choose this variant when the composition above suits the content. A palette substitution alone does not establish either interpretation.

## Visual Character

Dark jewel surfaces, cream-gold lines, stepped corner accents, and fan-like abstract linework frame content. Ornament belongs at the perimeter, leaving text on a calm dark field.
...
- Primary actions resemble framed admission plaques. Selected tabs have paired gold rules. Fields retain plain opaque interiors; dialogs use a symmetrical title and stepped outer frame.
...
Use original architectural details or high-contrast still-life photographs. Keep all identity and event copy original.
...
Use a 180ms fade for overlays; marquee bands never blink.
...
- **Active / pressed:** Use a 180ms fade for overlays; marquee bands never blink.
...
.design .hero{display:block;border:6px double var(--line);padding:40px!important;margin:40px 0;background:repeating-linear-gradient(45deg,transparent 0 30px,#c4ab6810 30px 31px)}
.design .hero-copy{text-align:center;max-width:780px;margin:auto}.design .hero h1{font:400 80px/1.02 Georgia,serif;text-transform:uppercase;letter-spacing:.07em}
.design .hero-copy p{margin-inline:auto}.design .actions{justify-content:center}.design .project-card{max-width:760px;margin:40px auto 0;border:8px double var(--line);border-radius:0;box-shadow:inset 0 0 0 8px var(--canvas)}
.design .features{grid-template-columns:1fr 1fr;gap:32px}.design .feature-card:first-child{grid-column:1/-1}
.design .feature-card:nth-child(n){border:6px double var(--line);text-align:center;padding:36px}.design .feature-index{letter-spacing:.2em}
.design .stats{border:6px double var(--line);padding:32px;text-align:center}.design .stats strong{font-size:52px}
.design .demo-tabs{justify-content:center;border-block:3px double var(--line);padding:12px}
.design .demo-form{border:6px double var(--line)}.design .testimonial{font-style:italic;border-block:8px double var(--line)}
