---
name: Handheld RPG Pixel Art
slug: handheld-rpg
family: pixel-art
description: "A handheld role-playing interface interpretation using limited olive tones, framed dialogue regions, and compact inventory-like modules."
tags:
  - pixel-art
  - handheld-rpg
  - complete-system
...
# Handheld RPG Pixel Art

## Overview and Design Philosophy

A handheld role-playing interface interpretation using limited olive tones, framed dialogue regions, and compact inventory-like modules.
...
Use a compact 4:5 opening with a 40px pixel heading. Workspace tasks form inventory rows. Features stack as dialogue panels with a narrow index slot.

The distinction from **Arcade Scoreboard Pixel Art** is structural: that interpretation follows this direction: Center a 52px pixel title above a broad scoreboard-like workspace. Features use two large framed columns with a full-width lead stage. Increase spacing around pixel typography. Choose this variant when the composition above suits the content. A palette substitution alone does not establish either interpretation.

## Visual Character

Use four principal olive values, hard stepped outlines, and integer-sized spacing. Avoid smooth gradients, antialiased decorative curves, or tiny unreadable bitmap body text.
...
- Buttons are framed game-menu commands with explicit labels. Tabs use an inverse selected cell. Inputs and dialogs resemble clear dialogue boxes with visible focus.
...
| Token | Value | Role |
|---|---|---|
| `canvas` | `#bbc39a` | Page environment |
| `surface` | `#dce3b9` | Opaque reading surface |
| `ink` | `#303b2a` | Primary text on surface |
| `muted` | `#414e39` | Secondary text on surface |
| `accent` | `#46503d` | Action and selection cue |
| `on-accent` | `#f2f5da` | Text on filled accent |
| `line` | `#7a8663` | Grouping rules |
| `control-line` | `#687552` | Control boundary |
| `error` | `#303b2a` | Error text with written explanation |
| `success` | `#303b2a` | Success text with written confirmation |
| `warning` | `#303b2a` | Warning text with written next step |
...
|---|---|---|
| Display | `Silkscreen,ui-monospace,monospace`; use the specific heading override in the construction rules | Follow the opening size above; 36px on small phones |
| Section heading | `Silkscreen,ui-monospace,monospace` | 32px / 1.2; 28px on phones |
| Card heading | `Silkscreen,ui-monospace,monospace` | 22px / 1.3; variant overrides below take precedence |
...
Use a flexible container around 1152px, with minmax(0,1fr) tracks and content-sized cards. Keep navigation, opening, workspace, statistics, features, testimonial, pricing, contact, and footer in a coherent source order. A visually offset panel must not change the keyboard order. Allow action rows to wrap and avoid fixed heights around text.
...
Use original pixel illustrations aligned to a consistent logical pixel grid. Essential text stays HTML; use a readable system monospace for long paragraphs.
...
Use immediate stepped state changes with no flashing selection or simulated typewriter text.
...
A stepped inventory-cursor shape jumps between 16px cells. This is optional decorative feedback, not a status indicator.

**Construction.** Build a 20 × 28px right-pointing stepped caret with three square stair steps. Use 40% opacity and no rounded edges, blur, or pulse. Preserve the native cursor and actual editing caret. Unless a material-specific size above overrides it, use a 48px-wide region, height equal to width / 1.45, and a 4px base stroke. Sparse ink geometry defaults to 16% paint opacity; minimalist work uses 12%, brutalist work 20%, Art Deco 28%, and Wabi-Sabi 10%. Explicit recipe opacity takes precedence. Apply a further 70% opacity to the page-level geometry; glass uses its explicit light values. Use this document’s accent and ink tokens.

**Movement and coverage.** Snap both coordinates to a 16px logical grid, then clamp them to the viewport. Use immediate steps with no interpolation or rotation. Position the page-level decoration in a fixed, viewport-sized layer: it follows the mouse through the whole page, including margins, the hero, footer, and the gaps between columns. Never clip this layer to a card or grid column.
...
- **Active / pressed:** Use immediate stepped state changes with no flashing selection or simulated typewriter text.
...
.design body{background:#bbc39a}.design .hero{display:grid;grid-template-columns:4fr 5fr;gap:32px;align-items:start}.design .hero h1{font:400 40px/1.2 Silkscreen,monospace;text-shadow:none}
.design .panel{background:#dce3b9;border:4px solid #46503d;border-radius:0;box-shadow:4px 4px #7a8663;padding:24px}
...
.design .feature-index{grid-row:1/4;font:12px/1.5 Silkscreen,monospace}.design .feature-card h3{margin:0}
.design .stats{border:4px solid #46503d;background:#dce3b9;padding:24px}.design .stats strong{font:400 28px/1.4 Silkscreen,monospace}
.design .primary,.design .primary:hover:not(:disabled){background:#46503d;color:#f2f5da;border:4px solid #46503d;box-shadow:4px 4px #7a8663}
.design .demo-tabs button[aria-selected=true]{background:#46503d;color:#f2f5da}.design .testimonial{text-align:left;font:18px/1.7 ui-monospace,monospace;border:4px solid #46503d;padding:24px}
