---
name: Technical Manual Industrial
slug: technical-manual
family: industrial
description: "A documentation-led interpretation using numbered engineering sheets, measured rules, and restrained annotations instead of a dark machine enclosure."
tags:
  - industrial
  - technical-manual
  - complete-system
...
# Technical Manual Industrial

## Overview and Design Philosophy

A documentation-led interpretation using numbered engineering sheets, measured rules, and restrained annotations instead of a dark machine enclosure.
...
Use a compact 4:5 opening, 48px condensed heading, and tabular workspace. Stack feature entries with an 88px drawing-number rail. Keep all prose upright and left aligned.

The distinction from **Assembly Line Industrial** is structural: that interpretation follows this direction: Use a broad 72px condensed heading above a full-width workstation. Features form equal station columns; statistics appear as individual readout banks. Choose this variant when the composition above suits the content. A palette substitution alone does not establish either interpretation.

## Visual Character

Use off-white drawing paper, graphite borders, and muted safety-yellow index strips. No screws or metallic gradients are necessary; precision and assembly notation carry the industrial identity.
...
- Actions are squared work-order stamps. Tabs are numbered sheet labels, fields have firm outlines, and dialogs read as one technical instruction sheet.
...
| Token | Value | Role |
|---|---|---|
| `canvas` | `#e6e5df` | Page environment |
| `surface` | `#f6f5ee` | Opaque reading surface |
| `ink` | `#303630` | Primary text on surface |
| `muted` | `#586058` | Secondary text on surface |
| `accent` | `#414e45` | Action and selection cue |
| `on-accent` | `#ffffff` | Text on filled accent |
| `line` | `#9b9e94` | Grouping rules |
| `control-line` | `#747870` | Control boundary |
...
Use original labeled engineering diagrams. Avoid decorative measurements that could be mistaken for actual dimensions.
...
Use immediate state changes and stable table columns.
...
- **Active / pressed:** Use immediate state changes and stable table columns.
...
.design body{background:#e6e5df}.design .demo-shell{background:none}.design .hero{display:grid;grid-template-columns:4fr 5fr;gap:32px;align-items:start}.design .hero h1{font:700 48px/1 'Barlow Condensed',Impact,sans-serif}
.design .panel{background:#f6f5ee;border:1px solid #747870;border-radius:0;box-shadow:none;padding:24px}.design .panel::before,.design .panel::after{display:none}
.design .project-card{border-top:14px solid #d0b44e}.design .features{grid-template-columns:1fr;gap:0}
.design .feature-card:nth-child(n){display:grid;grid-template-columns:88px 1fr;gap:8px 20px;border:0;border-top:2px solid;padding:24px 0;background:none}
.design .feature-index{grid-row:1/4;font-family:ui-monospace,monospace}.design .feature-card h3{margin:0}
.design .stats{background:#f6f5ee;border:1px solid;padding:24px}.design .stats strong{font-family:ui-monospace,monospace;font-size:32px}
.design .testimonial{text-align:left;border:1px solid;padding:24px;font-size:20px}
.design .testimonial blockquote{font-size:20px}.design .demo-nav{border-bottom:3px solid}
.design .demo-form{border-top:14px solid #d0b44e}
...
.design .card-top{background:transparent;border:0;margin:0 0 24px;padding:0}.design .demo-nav{border-image:none}
