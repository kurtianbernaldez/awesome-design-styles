---
name: Typographic Workshop Bauhaus
slug: typographic-workshop
family: bauhaus
description: "A type-first workshop interpretation: composition is built from rules, numbers, and asymmetric typesetting rather than a collection of primary-color objects."
tags:
  - bauhaus
...
# Typographic Workshop Bauhaus

## Overview and Design Philosophy

A type-first workshop interpretation: composition is built from rules, numbers, and asymmetric typesetting rather than a collection of primary-color objects.
...
Use an asymmetric opening with a 92px heavy sans heading on a narrow left measure. Set the workspace lower on the right. Feature rows have a large number rail and aligned explanatory text.

The distinction from **Modular Exhibition Bauhaus** is structural: that interpretation follows this direction: Center a 64px geometric title above a broad horizontal workspace. Align feature modules into a two-column exhibition wall with a full-width opening panel. Use a consistent 24px module gap. Choose this variant when the composition above suits the content. A palette substitution alone does not establish either interpretation.

## Visual Character

Warm paper, black rules, and a single red organizing rail carry the construction. Geometric color is concentrated at section edges; ordinary reading areas stay unpainted.
...
- Use flat rectangular actions and underlined navigation. Tabs sit on one strong baseline. Fields are ruled boxes; dialogs are unornamented rectangular sheets.
...
| Token | Value | Role |
|---|---|---|
| `canvas` | `#f0eadd` | Page environment |
| `surface` | `#fff9ed` | Opaque reading surface |
| `ink` | `#292d31` | Primary text on surface |
| `muted` | `#494e54` | Secondary text on surface |
| `accent` | `#9b342b` | Action and selection cue |
| `on-accent` | `#ffffff` | Text on filled accent |
| `line` | `#70757a` | Grouping rules |
| `control-line` | `#70757a` | Control boundary |
...
| Section heading | `system-ui, sans-serif` | 38px / 1.2; 28px on phones |
...
Use typographic specimens and functional diagrams. Circles are indexing devices, not decoration placed behind every heading.
...
Use immediate rule and color changes; maintain the static printed composition.
...
**Pointer policy: none.** Fixed geometric relationships and typographic order are the visual subject.
...
- **Active / pressed:** Use immediate rule and color changes; maintain the static printed composition.
...
.design .hero{ grid-template-columns:1.25fr 1fr; align-items:stretch; gap:0; padding:0; margin:32px 0; }

.design .hero-copy{ padding:40px 32px 40px 0; border-top:12px solid #292d31; }

.design h1{ font-size:clamp(44px,6.3vw,78px); font-weight:800; letter-spacing:-.06em; text-transform:lowercase; }

.design .project-card{ background:#e9bd34; border:0; border-top:12px solid #9b342b; border-radius:0; box-shadow:none; }

.design .project-card h2{ position:relative; padding-left:64px; }

.design .project-card h2::before{ content:''; width:48px; height:48px; border-radius:50%; position:absolute; left:0; top:0; background:#284d7f; }

.design .feature-card{ border:0; border-top:10px solid #9b342b; border-radius:0; background:transparent; padding-inline:0; }

.design .feature-card:nth-child(2){ border-color:#284d7f; }

.design .feature-card:nth-child(3){ border-color:#d1a215; }

.design .feature-index{ width:60px; height:60px; display:grid; place-items:center; border:2px solid; border-radius:50%; font-size:10px; }

.design .stats{ border-block:6px solid; }

.design .icon-button{ border-radius:50%; }
...
.design .hero{display:grid;grid-template-columns:5fr 4fr;align-items:start;gap:48px}.design .hero h1{font:800 92px/.95 system-ui,sans-serif;letter-spacing:-.07em}
.design .hero-copy{border-left:12px solid #a62d27;padding-left:24px}.design .project-card{margin-top:80px;border:0;border-top:8px solid var(--ink);background:none;box-shadow:none}
.design .project-card::before,.design .project-card::after{display:none}.design .features{grid-template-columns:1fr;gap:0}
.design .feature-card:nth-child(n){display:grid;grid-template-columns:100px 1fr;gap:8px 24px;border:0;border-top:2px solid;padding:28px 0;background:none}
.design .feature-index{grid-row:1/4;font-size:22px;font-weight:800}.design .feature-card h3{margin:0}
.design .stats{border-top:8px solid;background:none;padding:28px 0}.design .stats strong{font-size:56px}
.design .demo-tabs{border-bottom:3px solid}.design .testimonial{text-align:left;font-size:28px;border-left:12px solid #a62d27;padding:32px}
.design .testimonial blockquote{font-size:28px}
.design .demo-form{border:0;border-top:8px solid;background:none}
.design .feature-card:nth-child(n){transform:none}
.design .feature-index{width:auto;height:auto;display:block;border:0;border-radius:0;font-size:20px}.design .project-card h2{padding-left:0}.design .project-card h2::before{display:none}
