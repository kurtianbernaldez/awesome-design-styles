...
family: futurism
description: Crisp technical surfaces and directional hierarchy without decorative complexity.
tags:
  - futurism
  - precision-interface
  - complete-system
...
Crisp technical surfaces and directional hierarchy without decorative complexity. This is one original interpretation of the futurism family, not a definitive rulebook for the entire aesthetic. It is intended for complete websites and application surfaces that benefit from this visual character. Its layout, material choices, and control conventions below define why it exists as a separate system.

Use a 7:5 hero and three evenly spaced feature panels. Support scanning with consistent labels and aligned numeric readouts. Directional accents belong to section boundaries and do not imply a hidden workflow.
...
- Keep technical detail truthful.
...
Use precise sans headings and monospaced labels for technical metadata only. Keep body 17px with generous leading. Do not use unreadable stencil alphabets, all-caps paragraphs, or fabricated glyphs.

| Role | Stack | Size / line height | Weight and tracking |
|---|---|---|---|
| Display / H1 | `system-ui, sans-serif` | fluid 36–66px / 1.08 | 650; -0.03em |
| H2 | `system-ui, sans-serif` | 34px / 1.2 | 650; -0.02em |
...
Use opaque blue-black panels, cool edge rules, and almost-square geometry. A 4px start rail may indicate current context. No glass blur, lens flare, animated scanlines, or glow on body text.

Reference card boundary: 1px where a frame is used; reference corner radius: 2px. Surface shadow: `none`. Controls generally use 2px corners unless their component treatment specifies a more precise shape. Content stays at layer 0, navigation at 10 when sticky, menus at 20, backdrop at 40, dialog at 50, and transient feedback at 60. Avoid creating stacking contexts on every card. Overlays must remain above all decorative treatments.

## Components

Primary actions use pale cyan with dark labels; secondary buttons use solid dark panels and a visible boundary. Inputs are opaque and never styled as holograms. Cards organize label, title, data, then action. Pricing uses clear tabular values. Navigation current state uses a cyan rail and underline. Tabs are thin-edged segments. Menus and dialogs are opaque technical panels with an explicit close control and normal text order.
...
Use original technical diagrams or carefully cropped product imagery. Avoid simulated targeting reticles over people or decorative fake measurements. Icons use crisp 1.5px strokes. Charts have solid high-contrast lines, directly labeled series, and a table alternative. Unknown values are written as unavailable rather than invented telemetry.
...
Use 120ms ease-out state changes and a 140ms dialog fade. No scanning beams, constant HUD animation, or glitch transitions. Reduced motion removes fades and shows textual progress.
...
```css
.design-futurism-precision-interface {
  --canvas: #111e27;
  --surface: #1d303d;
  --ink: #eef6fa;
  --muted: #b8cbd7;
...
.design-futurism-precision-interface * { box-sizing: border-box; }
```
...
- Keep technical detail truthful.
- prioritize task hierarchy.
- use opaque readable planes.
- Preserve the readable control and feedback behavior when adapting the visual composition.

## Don't

- Do not add fake telemetry, neon body glow, or hard-to-decipher science-fiction fonts.
