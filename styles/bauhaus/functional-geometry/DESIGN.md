...
family: bauhaus
description: "Functional geometric composition where circles, bars, and primary color organize the interface."
tags:
  - bauhaus
  - functional-geometry
  - complete-system
...
Join a 1.25:1 hero into two structural regions. A thick dark bar caps the text while a red bar caps an ochre workspace. Pair a large lowercase sans headline with a single blue circular anchor. Features use red, blue, and yellow top bars.
...
- Use matte paper, square boundaries, no shadows, and measured geometric shapes.
- Let the control construction explain state and hierarchy.
- Do not reduce Bauhaus to colored dots scattered around a conventional card layout.
...
Use matte paper, square boundaries, no shadows, and measured geometric shapes. Primary colors occupy useful regions instead of becoming random confetti. Circles mark small tools and indexes; rectangles contain reading content.
...
Keep buttons flat and direct. Circular icon tools sit alongside rectangular text actions. Form inputs use square, precise boundaries. Tabs identify one active segment through a stable bar; menus continue the rectangular grammar.
...
Use only a few original geometric constructions, functional diagrams, or clear photographs. Geometry must relate to hierarchy; do not paste a historical poster into the page.
...
Immediate state changes or 120ms color feedback. Static composition provides the rhythm.
...
- **Active / pressed:** Immediate state changes or 120ms color feedback. Static composition provides the rhythm.
...
Background reference: [MoMA: Bauhaus](https://www.moma.org/calendar/exhibitions/303). The rules in this document are an original interface interpretation, not a reproduction of a source artifact or a claim that this variant exhausts the family.
