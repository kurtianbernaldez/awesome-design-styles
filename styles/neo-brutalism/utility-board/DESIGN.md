---
name: Utility Board Neo-Brutalism
slug: utility-board
family: neo-brutalism
description: "A task-dense board interpretation that uses hard-offset modular controls to support operational work rather than a poster-sized pitch."
tags:
  - neo-brutalism
  - utility-board
  - complete-system
...
# Utility Board Neo-Brutalism

## Overview and Design Philosophy

A task-dense board interpretation that uses hard-offset modular controls to support operational work rather than a poster-sized pitch.
...
Use a compact 4:5 hero with a 46px heading and 24px gaps. Let the workspace carry the density. Feature rows form a full-width stack with a number column, explanation, and action.

The distinction from **Block Editorial Neo-Brutalism** is structural: that interpretation follows this direction: Use a single broad headline above a horizontally organized workspace. Divide sections with thick black bars. Features form unequal 2:1:1 columns; the first is the lead story. Choose this variant when the composition above suits the content. A palette substitution alone does not establish either interpretation.

## Visual Character

Use flat cream boards and lightly colored task regions, 3px dark boundaries, and 4px hard shadows. Corners are only 6px; spacing makes the boards practical, not bulky.
...
- Buttons behave like small mechanical keys. Selected tabs become yellow index keys. Form fields stay 48px high, tool toggles use a dark inset marker, and badges are compact labeled counters.
...
| Token | Value | Role |
|---|---|---|
| `canvas` | `#f7f0df` | Page environment |
| `surface` | `#fffbee` | Opaque reading surface |
| `ink` | `#242d40` | Primary text on surface |
| `muted` | `#3f495b` | Secondary text on surface |
| `accent` | `#e4c14a` | Action and selection cue |
| `on-accent` | `#242d40` | Text on filled accent |
| `line` | `#242d40` | Grouping rules |
| `control-line` | `#242d40` | Control boundary |
...
Use functional symbols, numbered steps, and compact diagrams. Illustration is optional and must not displace the task controls.
...
Use immediate or 100ms press feedback and no passive-card lift.
...
- **Active / pressed:** Use immediate or 100ms press feedback and no passive-card lift.
...
.design .hero{display:grid;grid-template-columns:4fr 5fr;gap:24px;padding:32px 0;align-items:start}
.design .hero h1{font-size:46px;line-height:1.1}.design .project-card{background:#fffbee;border-radius:6px;box-shadow:4px 4px var(--ink);padding:24px}
.design .project-rows>div{background:#c8e7da;border:2px solid;margin:8px 0;padding:12px}
.design .project-rows>div:nth-child(2){background:#f5d3ba}
.design .features{grid-template-columns:1fr;gap:16px}.design .feature-card:nth-child(n){display:grid;grid-template-columns:110px 1fr;gap:8px 20px;border-radius:6px;box-shadow:4px 4px var(--ink);padding:20px}
.design .feature-card .feature-index{grid-row:1/4;font-size:14px;font-weight:700}.design .feature-card h3{margin:0}
.design .stats{border:3px solid;padding:20px;background:#fffbee}.design .stats strong{font:700 32px/1.2 ui-monospace,monospace}
.design .demo-tabs button[aria-selected=true]{box-shadow:none;border-style:inset}
.design .testimonial{text-align:left;padding:28px;border:3px solid;background:#c8e7da}
.design .demo-form{border-radius:6px;box-shadow:4px 4px var(--ink)}
