---
name: Monochrome Desktop Retro Computing
slug: monochrome-desktop
family: retro-computing
description: "A monochrome graphical-desktop interpretation using stippled backgrounds, stark title bars, and outlined windows rather than colorful late-era bevels."
tags:
  - retro-computing
  - monochrome-desktop
  - complete-system
...
# Monochrome Desktop Retro Computing

## Overview and Design Philosophy

A monochrome graphical-desktop interpretation using stippled backgrounds, stark title bars, and outlined windows rather than colorful late-era bevels.
...
Use a 1:1 opening with a 48px system-serif title. Workspace and features become sharply framed monochrome windows arranged in a consistent two-column desktop.

The distinction from **Phosphor Terminal Retro Computing** is structural: that interpretation follows this direction: Use a compact 4:5 opening and a 44px monospace title. Workspace and features form line-separated terminal sections with explicit labels and no floating cards. Choose this variant when the composition above suits the content. A palette substitution alone does not establish either interpretation.

## Visual Character

Use white window interiors, black rules, a pale stippled canvas, and hard one-pixel offsets. No color coding, gloss, or soft shadows.
...
- Buttons are rounded black-outlined keys with inverse pressed states. Tabs use outlined file labels. Inputs are white rectangles; dialogs are double-framed foreground windows.
...
| Token | Value | Role |
|---|---|---|
| `canvas` | `#dddddd` | Page environment |
| `surface` | `#ffffff` | Opaque reading surface |
| `ink` | `#222222` | Primary text on surface |
| `muted` | `#595959` | Secondary text on surface |
| `accent` | `#222222` | Action and selection cue |
| `on-accent` | `#ffffff` | Text on filled accent |
| `line` | `#777777` | Grouping rules |
| `control-line` | `#777777` | Control boundary |
| `error` | `#222222` | Error text with written explanation |
| `success` | `#222222` | Success text with written confirmation |
| `warning` | `#222222` | Warning text with written next step |
...
Use original monochrome pixel icons only where meaningful. Preserve live text and do not copy proprietary desktop icon sets.
...
Use immediate inverse states with no window zoom animation.
...
**Pointer policy: none.** A decorative selection box or terminal cell could be mistaken for an actual application state.
...
- **Active / pressed:** Use immediate inverse states with no window zoom animation.
...
.design body{background:radial-gradient(#aaa 1px,transparent 1px) 0 0/4px 4px #ddd}.design .hero{display:grid;grid-template-columns:1fr 1fr;gap:32px;align-items:start}
.design .hero h1{font:700 48px/1.1 Georgia,serif}.design .panel{background:#fff;border:2px solid #222;border-radius:0;box-shadow:4px 4px #222;padding:28px}
.design .project-card{border-top:24px solid #222}.design .panel::before,.design .panel::after{display:none}.design .features{grid-template-columns:1fr 1fr;gap:28px}
.design .feature-card:nth-child(n){border-top:16px solid #222}.design .stats{background:#fff;border:2px solid #222;padding:24px}
.design .stats strong{font:700 36px/1.2 Georgia,serif}.design .primary,.design .primary:hover:not(:disabled){background:#fff;color:#222;border:2px solid #222;box-shadow:2px 2px #222;border-radius:8px}
.design .demo-tabs button[aria-selected=true]{background:#222;color:#fff;box-shadow:none}.design .testimonial{text-align:left;background:#fff;border:2px solid #222;padding:28px;font:italic 24px/1.6 Georgia,serif}
.design .testimonial blockquote{font:italic 24px/1.6 Georgia,serif}
.design .demo-form{border-top:24px solid #222}
