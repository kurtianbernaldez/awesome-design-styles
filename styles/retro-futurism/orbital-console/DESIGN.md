...
**Pointer policy: surface.** A local instrument reflection belongs to the physical console.

### Atomic orbit

Two thin elliptical orbit lines follow the pointer over the console. This is decorative material feedback, not a status indicator.

**Construction.** Draw a fine accent ellipse with a faint second ring 8px beyond it. Compress its height to 60% and incline it around −25 degrees, adding only the bounded pointer angle. Do not run an automatic orbit. Use this document’s accent and ink tokens. Unless the material recipe specifies another size, use a 96px-wide region with height equal to width / 1 and a 1px base stroke.
