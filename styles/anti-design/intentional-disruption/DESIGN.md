---
name: Intentional Disruption Anti-Design
slug: intentional-disruption
family: anti-design
description: Unexpected scale and asymmetric emphasis that preserve a clear, accessible route through the interface.
tags:
  - anti-design
...
# Intentional Disruption Anti-Design

## Overview and Design Philosophy

Unexpected scale and asymmetric emphasis that preserve a clear, accessible route through the interface. This is one original interpretation of the anti design family, not a definitive rulebook for the entire aesthetic. It is intended for complete websites and application surfaces that benefit from this visual character. Its layout, material choices, and control conventions below define why it exists as a separate system.

Use a 3:1 hero at wide widths with a deliberately small supporting pane, then invert emphasis through a 1:2:1 feature grid. Keep source order logical. Visual surprise comes from proportion and contrast, not disorientation or inaccessible positioning.
...
- Challenge composition while preserving usability.
...
| Token | Value | Role |
|---|---|---|
| `canvas` | `#f4ead8` | Page environment; ink text |
| `surface` | `#fff9ec` | Opaque reading and control plane |
| `ink` | `#392d48` | Main text on canvas or surface |
| `muted` | `#675878` | Secondary text on surface |
| `accent` | `#d1c46a` | Primary action and selected state |
| `on-accent` | `#392d48` | Text on the accent fill |
| `line` | `#827190` | Grouping rules |
| `control-line` | `#827190` | Visible input and control boundaries |
...
Mix a large serif headline with blunt sans utility text and monospaced indexes. Use scale contrasts deliberately. Never distort, rotate, or obscure essential instructions, and never make a label look disabled for irony.

| Role | Stack | Size / line height | Weight and tracking |
|---|---|---|---|
| Display / H1 | `ui-serif, Georgia, serif` | fluid 36–86px / 1.08 | 500; -0.03em |
| H2 | `ui-serif, Georgia, serif` | 40px / 1.2 | 500; -0.02em |
...
Use 4/8/16/24/32/48/72/96px. Uneven section gaps are allowed to stress a change in meaning, but related controls retain conventional 8px/16px relationships.
...
The outer container is at most 1200px wide, with 28px gutters and 32px gaps. Use CSS grid with `minmax(0, 1fr)` tracks so long content cannot enlarge the page. Use flex wrapping for action groups and navigation. Cards grow with content; do not enforce fixed text heights. Comparable pricing keeps the same benefit order and aligned actions even if decorative feature blocks have unequal spans.

## Surfaces, Borders, Radius, Shadows, and Depth

Use flat fields, strong visible edges, and one unexpected oversized frame. Unequal emphasis is deliberate; there are no fake browser errors, deceptive links, or hidden hit targets. Essential controls remain conventional.

Reference card boundary: 2px where a frame is used; reference corner radius: 0px. Surface shadow: `none`. Controls generally use 0px corners unless their component treatment specifies a more precise shape. Content stays at layer 0, navigation at 10 when sticky, menus at 20, backdrop at 40, dialog at 50, and transient feedback at 60. Avoid creating stacking contexts on every card. Overlays must remain above all decorative treatments.

## Components

Primary buttons are pale olive with dark labels and a clear 2px boundary. Secondary buttons are conventional outlined rectangles. Forms are the most predictable region: plain labels, fields, help, and error recovery. Cards can differ in emphasis but never disguise interactivity. Pricing remains equal-width and factual. Navigation uses plainly named links; selected tabs are strongly underlined. Menus and dialogs abandon disruptive scale and use standard readable geometry.
...
Original unconventional crops and juxtaposed imagery are welcome when meaning remains clear. Never use misleading icons or copied provocative branding. Charts must remain accurate, with standard axes and table alternatives; surprise is not a reason to manipulate visual evidence.
...
Charts must remain accurate, with standard axes and table alternatives; surprise is not a reason to manipulate visual evidence.
...
Use immediate states. No evasive buttons, moving click targets, scroll hijacking, or motion-triggered reveals. Reduced motion is identical to the default.
...
```css
.design-anti-design-intentional-disruption {
  --canvas: #f4ead8;
  --surface: #fff9ec;
  --ink: #392d48;
  --muted: #675878;
...
.design-anti-design-intentional-disruption * { box-sizing: border-box; }
```
...
- Challenge composition while preserving usability.
- give each surprise a purpose.
- keep forms conventional.
- Preserve the readable control and feedback behavior when adapting the visual composition.

## Don't

- Do not use deceptive controls, unreadable overlaps, or broken navigation as an aesthetic.
