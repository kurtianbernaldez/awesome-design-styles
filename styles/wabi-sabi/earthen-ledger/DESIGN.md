---
name: Earthen Ledger Wabi-Sabi
slug: earthen-ledger
family: wabi-sabi
description: "A practical material-led interpretation with low earthen compartments and orderly ledger entries, retaining quiet irregularity within daily working tools."
tags:
  - wabi-sabi
  - earthen-ledger
  - complete-system
...
# Earthen Ledger Wabi-Sabi

## Overview and Design Philosophy

A practical material-led interpretation with low earthen compartments and orderly ledger entries, retaining quiet irregularity within daily working tools.
...
Use a compact 4:5 opening with a 50px serif title. Workspace rows form a measured ledger. Features use two shallow compartments with generous internal space.

The distinction from **Ink and Paper Wabi-Sabi** is structural: that interpretation follows this direction: Use a 3:2 opening with a 68px regular serif title. Leave a broad blank margin and stack feature essays as unboxed reading bands. Preserve asymmetry without random placement. Choose this variant when the composition above suits the content. A palette substitution alone does not establish either interpretation.

## Visual Character

Use clay-gray, chalk, warm umber, uneven corner radii, and a faint grounded shadow. Surfaces resemble simple handmade vessels, without glossy highlights or artificial distress.
...
- Buttons are restrained rounded plaques. Tabs use a simple selected rule. Fields are pale inset areas, and dialogs use one low earthen slab.
...
Use a 4px base with 8/12/16/24/32/48/64/96px intervals. The wide-screen section interval is 64px; the default module gap is 32px and outer gutter is 40px. Follow the role-specific interiors in the construction rules rather than forcing every region to have identical padding.
...
Use original ceramic or natural-material details in small deliberate crops. Imperfection belongs to material edges, not text alignment or controls.
...
Use 160ms color changes; surfaces do not animate or deform.
...
- **Active / pressed:** Use 160ms color changes; surfaces do not animate or deform.
...
.design .hero{display:grid;grid-template-columns:4fr 5fr;gap:36px;align-items:start}.design .hero h1{font:400 50px/1.15 Georgia,serif}
.design .panel{background:#e5dfd0;border:1px solid #928573;border-radius:22px 8px 16px 10px;box-shadow:0 5px 10px #554a3512;padding:28px}
.design .project-rows>div{border-bottom:1px solid #a69b87;padding:16px 0}.design .features{grid-template-columns:1fr 1fr;gap:32px}
.design .feature-card h3{font:400 26px/1.3 Georgia,serif}.design .stats{border:0;gap:28px;background:none}
.design .stats>div{border-bottom:2px solid #928573;padding:24px}.design .stats strong{font:400 36px/1.2 Georgia,serif}
.design .testimonial{text-align:left;border-left:3px solid #928573;padding:28px;font:italic 22px/1.7 Georgia,serif}
.design .testimonial blockquote{font:italic 22px/1.7 Georgia,serif}
.design .primary,.design .secondary{border-radius:12px 6px}.design .demo-form{background:#e5dfd0}
