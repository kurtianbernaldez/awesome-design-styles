---
name: Vienna Geometry Art Nouveau
slug: vienna-geometry
family: art-nouveau
description: "An original geometric Secession-inspired interpretation balancing ornamental rhythm with square frames, measured repetition, and upright typography."
tags:
  - art-nouveau
  - vienna-geometry
  - complete-system
...
# Vienna Geometry Art Nouveau

## Overview and Design Philosophy

An original geometric Secession-inspired interpretation balancing ornamental rhythm with square frames, measured repetition, and upright typography.
...
Use a centered 60px upright serif heading and a broad workspace below. Features form a precise two-column wall. Small repeated squares act as separators and indexes.

The distinction from **Illustrated Folio Art Nouveau** is structural: that interpretation follows this direction: Use a 3:2 opening with a 72px italic serif title. Place the workspace inside a tall folio frame. Feature entries become spacious full-width reading leaves. Choose this variant when the composition above suits the content. A palette substitution alone does not establish either interpretation.

## Visual Character

Use ivory fields, dark olive outlines, muted gold bands, and checker-like abstract edging. Replace loose whiplash curves with disciplined rectangular ornament.
...
- Actions are upright framed plaques with square corners. Tabs use repeated vertical markers. Forms are measured rectangular grids; dialogs repeat the same double frame.
...
Use original decorative architectural detail or geometric illustration. Repeated ornaments remain outside text and never imply an interactive checkerboard.
...
Use immediate selection and a 140ms color transition; geometry stays fixed.
...
- **Active / pressed:** Use immediate selection and a 140ms color transition; geometry stays fixed.
...
.design .hero{display:block;border-block:12px solid #b79a59;padding:40px 0;margin:28px 0}.design .hero-copy{text-align:center;max-width:760px;margin:auto}
.design .hero h1{font:400 60px/1.1 Georgia,serif;letter-spacing:.08em;text-transform:uppercase}.design .hero-copy p{margin-inline:auto}.design .actions{justify-content:center}
.design .project-card{max-width:780px;margin:40px auto 0;border:5px double #515b3a;border-radius:0;background:#f7efd8;box-shadow:none}
.design .panel::before,.design .panel::after{display:none}.design .features{grid-template-columns:1fr 1fr;gap:28px}
.design .feature-card:nth-child(n){border:5px double #515b3a;border-radius:0;background:#f7efd8;border-top:20px solid #b79a59}
.design .feature-index{letter-spacing:.15em}.design :is(button,input,select,textarea,.badge){border-radius:0}
.design .stats{border:5px double #515b3a;padding:28px}.design .demo-tabs{border-block:3px double #515b3a;padding:12px}
.design .testimonial{border-block:12px solid #b79a59;padding:36px}.design .demo-form{border:5px double #515b3a;border-radius:0}
.design .hero-copy{border:0;border-radius:0}.design .project-card h2{font-style:normal}.design .testimonial{border-radius:0}
