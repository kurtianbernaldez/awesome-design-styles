---
name: Typographic Rebellion Anti-Design
slug: typographic-rebellion
family: anti-design
description: "A typography-led interpretation that disrupts conventional hierarchy through enormous type and abrupt scale changes rather than mismatched browser widgets."
tags:
  - anti-design
  - typographic-rebellion
  - complete-system
---

# Typographic Rebellion Anti-Design

## Overview and Design Philosophy

A typography-led interpretation that disrupts conventional hierarchy through enormous type and abrupt scale changes rather than mismatched browser widgets.
...
Use a 112px uppercase headline across the page, followed by a small offset workspace. Features stack as oversized numbered statements with compact explanations. Keep line breaks responsive.

The distinction from **Browser Collage Anti-Design** is structural: that interpretation follows this direction: Use a compact 4:5 opening with a default-serif 60px title. The workspace has a stark outlined browser-like frame. Feature blocks vary their border treatments but share readable source order. Choose this variant when the composition above suits the content. A palette substitution alone does not establish either interpretation.

## Visual Character

Use mostly monochrome paper, one acid-color rail, and strong underlines. Remove ordinary card boxes so unexpected type scale becomes the dominant structure.
...
- Actions are blunt rectangular labels. Tabs use heavy underlines. Forms and dialogs remain plainly organized with stable labels and strong focus outlines.
...
| Token | Value | Role |
|---|---|---|
| `canvas` | `#f4ead8` | Page environment |
| `surface` | `#fff9ec` | Opaque reading surface |
| `ink` | `#392d48` | Primary text on surface |
| `muted` | `#5d4f6c` | Secondary text on surface |
| `accent` | `#d1c46a` | Action and selection cue |
| `on-accent` | `#392d48` | Text on filled accent |
| `line` | `#827190` | Grouping rules |
| `control-line` | `#827190` | Control boundary |
...
Use a 4px base with 8/12/16/24/32/48/64/96px intervals. The wide-screen section interval is 96px; the default module gap is 32px and outer gutter is 28px. Follow the role-specific interiors in the construction rules rather than forcing every region to have identical padding.
...
Use original typographic compositions; imagery is optional. Never turn essential prose into decorative unreadable lettering.
...
Use immediate selection; hierarchy shifts occur by composition, never by animating text size during reading.
...
**Pointer policy: none.** The composition supplies the intentional disruption; the native pointer stays predictable.
...
- **Active / pressed:** Use immediate selection; hierarchy shifts occur by composition, never by animating text size during reading.
...
.design .hero{ grid-template-columns:1.4fr 1fr; align-items:start; gap:28px; padding:32px 0; }

.design h1{ font:500 clamp(50px,8vw,96px)/.96 Georgia,serif; letter-spacing:-.07em; }

.design .hero-copy>.eyebrow{ font-family:monospace; background:#e1ed73; border:1px solid; display:inline-block; padding:5px; transform:rotate(-5deg); }

.design .lead{ margin-left:18%; max-width:29ch; font-family:monospace; font-size:16px; }

.design .project-card{ border:2px dashed; border-radius:50% 0 0 0 / 35px 0 0 0; margin-top:72px; box-shadow:none; background:#e4c5e8; }

.design .project-card h2{ font-family:monospace; font-size:28px; }

.design .features{ grid-template-columns:1fr 1.5fr 1fr; align-items:start; }

.design .feature-card{ border:1px solid; box-shadow:none; }

.design .feature-card:nth-child(2){ margin-top:48px; background:#e1ed73; }

.design .feature-card:nth-child(3){ border:2px dashed; transform:rotate(3deg); }

.design .stats{ border-block:1px solid; }

.design .stats strong{ font-family:Georgia,serif; font-style:italic; font-weight:400; }
...
.design .hero{display:block;padding:28px 0}.design .hero h1{font:900 112px/.9 Arial,sans-serif;text-transform:uppercase;letter-spacing:-.075em;max-width:11ch}
.design .hero-copy{border-bottom:18px solid #ced76d;padding-bottom:28px}.design .project-card{margin:40px 0 0 42%;transform:none;border:1px solid;border-radius:0;background:var(--surface);box-shadow:none;padding:24px}
.design .features{grid-template-columns:1fr;gap:40px}.design .feature-card:nth-child(n){transform:none;border:0;border-top:4px solid;border-radius:0;background:none;padding:28px 0;box-shadow:none}
.design .feature-card h3{font:800 42px/1.05 Arial,sans-serif;letter-spacing:-.05em}.design .feature-index{font:700 20px/1.2 ui-monospace,monospace}
.design .stats strong{font:800 64px/1 Arial,sans-serif}.design .stats{border-block:4px solid;padding:28px 0}
.design .testimonial{text-align:left;font:700 34px/1.2 Arial,sans-serif;border:0;padding:36px 0}
.design .testimonial blockquote{font:700 34px/1.2 Arial,sans-serif}
.design .demo-form{transform:none;border:0;border-top:18px solid #ced76d;border-radius:0;background:none;box-shadow:none;padding:32px 0}
