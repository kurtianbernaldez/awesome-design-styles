---
name: Translucent Hardware Y2K
slug: translucent-hardware
family: y2k
description: "A consumer-electronics interpretation of the turn-of-the-millennium aesthetic: rounded translucent plastic, molded seams, and small technical labels."
tags:
  - y2k
...
# Translucent Hardware Y2K

## Overview and Design Philosophy

A consumer-electronics interpretation of the turn-of-the-millennium aesthetic: rounded translucent plastic, molded seams, and small technical labels.
...
Use a compact 4:5 opening with a 56px rounded sans heading. Group the workspace into one molded shell. Features use two roomy compartments rather than flyer-like layers.

The distinction from **Digital Flyer Y2K** is structural: that interpretation follows this direction: Use an 104px compressed uppercase headline across a broad opening. Offset the workspace below it. Features form unequal columns with bold serial strips and sharp corners. Choose this variant when the composition above suits the content. A palette substitution alone does not establish either interpretation.

## Visual Character

Use pale aqua plastic, silver seams, glossy narrow highlights, and rounded translucent edges. Keep text on an opaque inner panel so the plastic shell never harms legibility.
...
- Buttons are domed plastic keys. Tabs resemble molded dividers; input wells sit inside the shell. Dialogs use a single rounded housing with a visible close control.
...
| Token | Value | Role |
|---|---|---|
| `canvas` | `#d7e6ec` | Page environment |
| `surface` | `#eef7f7` | Opaque reading surface |
| `ink` | `#263e4a` | Primary text on surface |
| `muted` | `#4d6570` | Secondary text on surface |
| `accent` | `#42627e` | Action and selection cue |
| `on-accent` | `#ffffff` | Text on filled accent |
| `line` | `#92a9b3` | Grouping rules |
| `control-line` | `#718b97` | Control boundary |
...
Use original abstract hardware details or product-like still life. No borrowed device logos, branded interfaces, or proprietary casing silhouettes.
...
Use 160ms highlight changes and immediate pressed bevels.
...
**Pointer policy: surface.** The molded housing catches light locally; the surrounding content stays quiet.

### Plastic glint

A rounded clear-plastic glint follows the pointer inside the molded housing. This is decorative material feedback, not a status indicator.

**Construction.** Draw a 3px double circular accent rim, with a restrained white radial glint near its upper-left quarter. Use 30% overall paint opacity and no distortion of text. Use this document’s accent and ink tokens. Unless the material recipe specifies another size, use a 144px-wide region with height equal to width / 1.45 and a 2px base stroke.
...
- **Active / pressed:** Use 160ms highlight changes and immediate pressed bevels.
...
.design .hero{display:grid;grid-template-columns:4fr 5fr;gap:36px;align-items:start}.design .hero h1{font:650 56px/1.05 system-ui,sans-serif;letter-spacing:-.05em;text-shadow:none}
.design .panel{background:linear-gradient(135deg,#f5ffff,#c7e1e5);border:6px solid #97bbc4;border-radius:44px;box-shadow:inset 0 3px #fff,0 8px 18px #45677944;padding:32px}
.design .project-card::before,.design .project-card::after{display:none}.design .project-rows{background:#eef7f7;border:1px solid #7e9ca6;border-radius:20px;padding:16px}
.design .features{grid-template-columns:1fr 1fr;gap:28px}.design .feature-card:nth-child(n){border-radius:36px;border-width:4px}
.design .stats{border:4px solid #97bbc4;border-radius:36px;padding:28px;background:#d9ecee}.design .stats strong{font:600 38px/1.1 system-ui,sans-serif}
.design .primary,.design .secondary{border-radius:99px}.design .demo-tabs button{border-radius:18px 18px 0 0}
.design .testimonial{border:0;font-size:24px;text-shadow:none}
.design .testimonial blockquote{font-size:24px}.design .demo-form{border-radius:36px}
