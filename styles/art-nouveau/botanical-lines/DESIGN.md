---
name: Botanical Lines Art Nouveau
slug: botanical-lines
family: art-nouveau
description: Flowing framed compositions with organic curves, literary typography, and calm readable interiors.
tags:
  - art-nouveau
  - botanical-lines
  - complete-system
...
# Botanical Lines Art Nouveau

## Overview and Design Philosophy

Flowing framed compositions with organic curves, literary typography, and calm readable interiors. This is one original interpretation of the art nouveau family, not a definitive rulebook for the entire aesthetic. It is intended for complete websites and application surfaces that benefit from this visual character. Its layout, material choices, and control conventions below define why it exists as a separate system.

Use a 3:2 hero with a tall project pane and two-column features. Frame major sections as flowing but stable blocks. Keep body left aligned and place curved ornament in outer margins. Never force text to wrap around a narrow decorative silhouette.
...
- Keep organic framing outside readable content.
...
| Token | Value | Role |
|---|---|---|
| `canvas` | `#eee8d7` | Page environment; ink text |
| `surface` | `#fbf5e7` | Opaque reading and control plane |
| `ink` | `#354635` | Main text on canvas or surface |
| `muted` | `#5a6a50` | Secondary text on surface |
| `accent` | `#526b3d` | Primary action and selected state |
| `on-accent` | `#ffffff` | Text on the accent fill |
| `line` | `#7b8967` | Grouping rules |
| `control-line` | `#7b8967` | Visible input and control boundaries |
...
Use flowing serif display headings with ordinary sans-serif controls. Italics are permitted for short quotations, not navigation. Keep body at 17px with 1.7 line height and avoid decorative type for small labels.

| Role | Stack | Size / line height | Weight and tracking |
|---|---|---|---|
| Display / H1 | `ui-serif, Georgia, serif` | fluid 36–70px / 1.08 | 400; -0.03em |
| H2 | `ui-serif, Georgia, serif` | 38px / 1.2 | 400; -0.02em |
| H3 | `ui-serif, Georgia, serif` | 26px / 1.3 | 650; normal |
...
Use 4/8/12/20/28/40/64/96px. Large curved frames need 32px interiors. Leave quiet space around botanical decoration and keep captions close to images.

Use 8px from label to field, 4–8px from field to help, 20px between form fields, and 24–32px between task groups. Standard controls have 12px vertical and 20px horizontal padding with a minimum 48px height. Card padding is 28px unless the surface guidance gives a stronger role-specific value. Separate unrelated page sections by 96px at wide widths. Convert these reference pixels to rem in implementations that respect user text settings.

## Layout and Grid

Use a 3:2 hero with a tall project pane and two-column features. Frame major sections as flowing but stable blocks. Keep body left aligned and place curved ornament in outer margins. Never force text to wrap around a narrow decorative silhouette.

The outer container is at most 1140px wide, with 40px gutters and 28px gaps. Use CSS grid with `minmax(0, 1fr)` tracks so long content cannot enlarge the page. Use flex wrapping for action groups and navigation. Cards grow with content; do not enforce fixed text heights. Comparable pricing keeps the same benefit order and aligned actions even if decorative feature blocks have unequal spans.

## Surfaces, Borders, Radius, Shadows, and Depth

Use cream paper planes with muted green outlines and asymmetric 28px/4px corner pairs. Organic framing is a container treatment; control hit areas stay rectangular. Decorative curves must not intrude into text or imply unimplemented interactions.

Reference card boundary: 1px where a frame is used; reference corner radius: 28px. Surface shadow: `none`. Controls generally use 12px corners unless their component treatment specifies a more precise shape. Content stays at layer 0, navigation at 10 when sticky, menus at 20, backdrop at 40, dialog at 50, and transient feedback at 60. Avoid creating stacking contexts on every card. Overlays must remain above all decorative treatments.

## Components

Primary actions use deep green, modest 8px corners, and clear sans labels. Inputs stay plain cream with visible boundaries. Cards have one organic corner pair and roomy interiors; pricing remains aligned and rectangular enough to compare. Navigation uses a fine ruled row and underlined current location. Tabs use a curved end only at the group boundary. Menus and dialogs are opaque cream with restrained border curves; decorative corners do not change focus order.
...
Use original botanical line work and carefully cropped natural photography. Avoid copied historical posters or proprietary ornamental fonts. Informative imagery needs descriptive alt text. Charts remain rectangular with clear axes; leaf shapes must not replace accurate bars or line markers. Provide a table alternative.
...
Charts remain rectangular with clear axes; leaf shapes must not replace accurate bars or line markers. Provide a table alternative.
...
Color changes use 180ms ease-out. No growing vines, drawing animation behind text, or continuous drifting leaves. Reduced motion makes the same compositions static.
...
```css
.design-art-nouveau-botanical-lines {
  --canvas: #eee8d7;
  --surface: #fbf5e7;
  --ink: #354635;
  --muted: #5a6a50;
...
.design-art-nouveau-botanical-lines * { box-sizing: border-box; }
```
...
- Keep organic framing outside readable content.
- pair expressive headings with plain controls.
- use original line work.
- Preserve the readable control and feedback behavior when adapting the visual composition.

## Don't

- Do not decorate every input or let curves create narrow text columns.
