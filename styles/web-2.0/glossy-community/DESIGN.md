...
family: web-2.0
description: Rounded community-oriented panels, clear gradients, and friendly task-focused hierarchy.
tags:
  - web-2.0
  - glossy-community
  - complete-system
...
Rounded community-oriented panels, clear gradients, and friendly task-focused hierarchy. This is one original interpretation of the web 2.0 family, not a definitive rulebook for the entire aesthetic. It is intended for complete websites and application surfaces that benefit from this visual character. Its layout, material choices, and control conventions below define why it exists as a separate system.

Use balanced hero halves and three equal feature panels. Maintain a simple community-product flow: introduction, benefits, social proof, plans, form. Avoid oversized badges or decorative ribbons that cover pricing.
...
Use clear sans-serif headings and generous 17px body. Keep labels bold enough to read on glossy controls. Avoid oversized reflected wordmarks and faux handwritten accents.

| Role | Stack | Size / line height | Weight and tracking |
|---|---|---|---|
| Display / H1 | `system-ui, sans-serif` | fluid 36–56px / 1.08 | 700; -0.03em |
| H2 | `system-ui, sans-serif` | 32px / 1.2 | 700; -0.02em |
...
Use opaque white panels, 12px corners, and soft shallow shadows. Secondary controls may use a white-to-pale-blue gradient with dark text. Primary buttons remain a stable deep blue; top highlights are small and decorative.

Reference card boundary: 1px where a frame is used; reference corner radius: 12px. Surface shadow: `0 4px 12px #30485c1f`. Controls generally use 12px corners unless their component treatment specifies a more precise shape. Content stays at layer 0, navigation at 10 when sticky, menus at 20, backdrop at 40, dialog at 50, and transient feedback at 60. Avoid creating stacking contexts on every card. Overlays must remain above all decorative treatments.

## Components

Primary actions are deep blue with white labels, 8px corners, and a slight top highlight. Secondary buttons have pale gloss and visible borders. Inputs remain opaque white. Cards have modest rounding and clear title/body/action order. Pricing recommendation is a written badge within the card. Navigation uses a rounded pale bar, selected tabs are solid blue, and menus/dialogs are opaque white with visible boundaries and limited shadow.
...
Use original friendly illustrations or straightforward screenshots with readable crops. Avoid copied badges, social-network logos, and reflection effects on meaningful imagery. Charts use solid marks and readable legends on white, with table alternatives.
...
Use 140ms ease-out color changes and a short dialog fade. No bouncing badges, animated gloss, or auto-advancing testimonial carousels. Reduced motion makes all changes immediate.
...
```css
.design-web-2.0-glossy-community {
  --canvas: #e8eff6;
  --surface: #ffffff;
  --ink: #30485c;
  --muted: #5c7286;
...
.design-web-2.0-glossy-community * { box-sizing: border-box; }
```
...
- make community content concrete.
- Preserve the readable control and feedback behavior when adapting the visual composition.

## Don't

- Do not add fake awards, huge decorative badges, or reflected text.
