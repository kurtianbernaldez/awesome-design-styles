---
name: Ink and Paper Wabi-Sabi
slug: ink-and-paper
family: wabi-sabi
description: "A calligraphic editorial interpretation where imperfect ink-like edges and generous paper space carry quiet character instead of sculptural earthy surfaces."
tags:
  - wabi-sabi
  - ink-and-paper
  - complete-system
...
# Ink and Paper Wabi-Sabi

## Overview and Design Philosophy

A calligraphic editorial interpretation where imperfect ink-like edges and generous paper space carry quiet character instead of sculptural earthy surfaces.
...
Use a 3:2 opening with a 68px regular serif title. Leave a broad blank margin and stack feature essays as unboxed reading bands. Preserve asymmetry without random placement.

The distinction from **Earthen Ledger Wabi-Sabi** is structural: that interpretation follows this direction: Use a compact 4:5 opening with a 50px serif title. Workspace rows form a measured ledger. Features use two shallow compartments with generous internal space. Choose this variant when the composition above suits the content. A palette substitution alone does not establish either interpretation.

## Visual Character

Use warm paper, charcoal ink, irregular-looking double rules, and no card shadows. Texture is limited to outer margins; content remains clean and high contrast.
...
- Actions are modest ink-colored rectangles. Tabs are underlined text, fields have simple graphite boundaries, and dialogs are plain paper sheets.
...
| Token | Value | Role |
|---|---|---|
| `canvas` | `#e9e3d7` | Page environment |
| `surface` | `#f5f0e5` | Opaque reading surface |
| `ink` | `#47483c` | Primary text on surface |
| `muted` | `#5b5b4c` | Secondary text on surface |
| `accent` | `#62684a` | Action and selection cue |
| `on-accent` | `#ffffff` | Text on filled accent |
| `line` | `#898a73` | Grouping rules |
| `control-line` | `#898a73` | Control boundary |
...
| Section heading | `ui-serif, Georgia, serif` | 34px / 1.2; 28px on phones |
...
Use a 4px base with 8/12/16/24/32/48/64/96px intervals. The wide-screen section interval is 104px; the default module gap is 32px and outer gutter is 40px. Follow the role-specific interiors in the construction rules rather than forcing every region to have identical padding.
...
Use original ink studies or quiet material photographs. Do not imitate sacred calligraphy or use unreadable glyphs as decoration.
...
Use immediate underlines and 180ms color transitions; no drifting paper animation.
...
**Pointer policy: none.** Quiet material irregularity and unhurried reading call for stillness.
...
- **Active / pressed:** Use immediate underlines and 180ms color transitions; no drifting paper animation.
...
.design body{ background:radial-gradient(ellipse at 20% 0,#d6cbb888,transparent 45%),#eae5da; }

.design .demo-nav{ border:0; padding-block:28px; }

.design .hero{ grid-template-columns:1.2fr 1fr; gap:80px; align-items:start; padding:80px 0; }

.design h1{ font:400 clamp(42px,5.8vw,68px)/1.22 Georgia,serif; letter-spacing:-.045em; }

.design .project-card{ margin-top:64px; background:#ded7c7; border:0; border-radius:3px 12px 2px 7px; box-shadow:none; padding:32px; }

.design .project-card h2{ font-weight:400; font-size:28px; }

.design .panel:not(.project-card){ background:transparent; border:0; border-top:1px solid #a79c88; border-radius:0; box-shadow:none; padding:24px 0; }

.design .features{ grid-template-columns:1.4fr 1fr 1fr; gap:44px; }

.design .feature-card:nth-child(2){ margin-top:32px; }

.design .feature-card:nth-child(3){ margin-top:64px; }

.design .stats{ border:0; gap:40px; }

.design .stats>div{ border:0; }

.design .stats strong{ font-weight:400; }

.design .testimonial{ text-align:left; border:0; padding:32px 0; max-width:640px; }

.design .icon-button{ border:0; background:transparent; }
...
.design .hero{display:grid;grid-template-columns:3fr 2fr;gap:56px;align-items:start}.design .hero h1{font:400 68px/1.12 Georgia,serif;letter-spacing:-.04em}
.design .project-card{border:0;border-block:3px double #837b6b;border-radius:0;background:none;box-shadow:none;padding:32px 0}
.design .features{grid-template-columns:1fr;gap:48px}.design .feature-card:nth-child(n){border:0;border-top:3px double #837b6b;border-radius:0;background:none;box-shadow:none;padding:32px 0}
.design .feature-card h3{font:400 30px/1.3 Georgia,serif}.design .feature-index{font:italic 16px/1.5 Georgia,serif}
.design .stats{border-block:3px double #837b6b;padding:32px 0;background:none}.design .stats strong{font:400 44px/1.2 Georgia,serif}
.design .testimonial{text-align:left;font:italic 28px/1.7 Georgia,serif;border:0;padding:40px 0;max-width:680px}
.design .testimonial blockquote{font:italic 28px/1.7 Georgia,serif}
.design .demo-form{border:0;border-top:3px double #837b6b;border-radius:0;background:none;padding:32px 0;box-shadow:none}
