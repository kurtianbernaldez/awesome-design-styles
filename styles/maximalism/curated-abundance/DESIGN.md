...
family: maximalism
description: Richly layered editorial blocks, contrasting type, and abundant detail organized around a clear focal point.
tags:
  - maximalism
  - curated-abundance
  - complete-system
...
Richly layered editorial blocks, contrasting type, and abundant detail organized around a clear focal point. This is one original interpretation of the maximalism family, not a definitive rulebook for the entire aesthetic. It is intended for complete websites and application surfaces that benefit from this visual character. Its layout, material choices, and control conventions below define why it exists as a separate system.

Use a 7:5 hero and three unequal feature spans of 2:1:1. Follow an abundant opening with a calm full-width quote. A section has one dominant headline and at most two supporting ornament zones. Never overlap text or scramble source order.
...
Display serifs anchor the page; plain system sans handles controls. Use italic quotes and small monospaced catalog labels as two secondary voices. Do not introduce a fourth type family.

| Role | Stack | Size / line height | Weight and tracking |
|---|---|---|---|
| Display / H1 | `ui-serif, Georgia, serif` | fluid 36–72px / 1.08 | 700; -0.03em |
| H2 | `ui-serif, Georgia, serif` | 40px / 1.2 | 700; -0.02em |
...
The outer container is at most 1160px wide, with 32px gutters and 28px gaps. Use CSS grid with `minmax(0, 1fr)` tracks so long content cannot enlarge the page. Use flex wrapping for action groups and navigation. Cards grow with content; do not enforce fixed text heights. Comparable pricing keeps the same benefit order and aligned actions even if decorative feature blocks have unequal spans.

## Surfaces, Borders, Radius, Shadows, and Depth

Surface blocks may have double outlines, striped margins, or flat secondary fields. Keep the central reading area opaque. Use a different framing rhythm for hero, feature collection, and quotation rather than repeating one decorated card.

Reference card boundary: 2px where a frame is used; reference corner radius: 4px. Surface shadow: `0 8px 0 #392638`. Controls generally use 4px corners unless their component treatment specifies a more precise shape. Content stays at layer 0, navigation at 10 when sticky, menus at 20, backdrop at 40, dialog at 50, and transient feedback at 60. Avoid creating stacking contexts on every card. Overlays must remain above all decorative treatments.

## Components

Primary actions are plum, square-ended plaques with a 2px frame and clear white labels. Forms live on the quiet cream surface with complete rectangular boundaries. Feature cards get alternating pale apricot and blush fields, but pricing retains equal widths and matching action positions. Navigation is a strong ruled masthead; menus are plain opaque lists. Tabs use a bold lower rule rather than another patterned enclosure. The modal is the calmest frame on the page: a double outline, normal body copy, and one decisive action.
...
Curate one original illustration or photograph per major section, with a caption and a consistent editorial crop. Supporting patterns belong in margins. Charts use plain cream plotting areas, dark axes, directly labeled series, and pattern-coded alternatives; ornament must never distort data.
...
Color changes use 160ms ease-out. Keep ornament static; no automatic carousel, animated collage, or accumulating scroll reveals. Dialogs fade without moving.
...
```css
.design-maximalism-curated-abundance {
  --canvas: #f3dcb9;
  --surface: #fff2d9;
  --ink: #392638;
  --muted: #6b465c;
...
.design-maximalism-curated-abundance * { box-sizing: border-box; }
```
...
- Cluster detail deliberately.
- alternate dense and quiet sections.
- preserve one reading path.
- Preserve the readable control and feedback behavior when adapting the visual composition.

## Don't

- Do not pattern every surface, overlap captions, or use five unrelated font styles.
