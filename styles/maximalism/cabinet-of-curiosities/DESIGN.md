...
family: maximalism
description: "An intimate collector’s interpretation: many carefully framed objects and annotations rather than the scale and urgency of a public poster."
tags:
  - maximalism
...
An intimate collector’s interpretation: many carefully framed objects and annotations rather than the scale and urgency of a public poster.
...
Use an asymmetric 4:5 opening with a smaller 60px serif title. The workspace becomes an inset specimen sheet. Feature groups resemble differently sized catalogue compartments, with captions aligned inside their own frames.

The distinction from **Festival Poster Maximalism** is structural: that interpretation follows this direction: Center the opening headline at 96px above a broad workspace. Repeat oversized numbers, contrasting bands, and tightly grouped announcements. Keep supporting prose narrow within each colored region. Choose this variant when the composition above suits the content. A palette substitution alone does not establish either interpretation.

## Visual Character

Combine dark plum shelves, cream specimen paper, fine double borders, and green dividers. Pattern is confined to shelf margins. There is no need to tilt the content itself.
...
- Use compact brass-colored rectangular actions, narrow index tabs, and full-width labeled inputs. Menus resemble an index card; dialogs use the specimen-paper surface with a precise outer frame.
...
| `muted` | `#6b465c` | Secondary text on surface |
...
Use original still-life or collection photography with explicit captions. Each image is treated as an item worth studying, not a background texture.
...
Use a restrained 160ms color change; opening a menu does not scatter the surrounding compartments.
...
- **Active / pressed:** Use a restrained 160ms color change; opening a menu does not scatter the surrounding compartments.
...
.design .demo-shell{border-inline:14px solid #843254;background:#eadcc7}
.design .hero{display:grid;grid-template-columns:4fr 5fr;gap:40px;background:none;align-items:start;padding:48px 0}
.design .hero-copy{border-block:3px double;padding:28px 0;background:none}
.design .hero h1{font-size:60px;font-weight:400;font-style:normal;text-shadow:none}
.design .project-card{background:#fff2d9;border:8px double #77536b;border-radius:2px;transform:none;box-shadow:inset 0 0 0 8px #e6d5bc;padding:40px}
.design .features{grid-template-columns:1fr 1.5fr 1fr;gap:16px}.design .feature-card:nth-child(n){border:5px double;transform:none;background:#fff2d9}
.design .feature-card:nth-child(2){background:#e0e3c7;border-top:24px solid #843254}
.design .feature-card:nth-child(3){background:#f0d3c8;border-bottom:24px solid #843254}
.design .feature-index{letter-spacing:.12em;border-bottom:1px solid;padding-bottom:12px}
.design .stats{border:0;background:#e0e3c7;padding:24px}.design .stats strong{font-family:Georgia,serif;font-weight:400}
.design .testimonial{background:#fff2d9;border:8px double;font-style:italic}
.design .testimonial blockquote{font-style:italic}
.design .demo-tabs{border-bottom:3px double}.design .demo-tabs button[aria-selected=true]{background:#fff2d9;border:1px solid}
.design .primary{border-radius:2px}.design .demo-form{border:5px double;background:#fff2d9}
