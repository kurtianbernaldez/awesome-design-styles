---
name: Curated Maximalism
slug: curated-abundance
family: maximalism
description: "Abundant but curated layering: patterns, varied frames, rich typography, and several complementary visual rhythms."
tags:
  - maximalism
  - curated-abundance
  - complete-system
...
Use a compact 7:5 opening framed by patterned margins. A large italic serif headline sits between double rules. The arched workspace tilts 2 degrees; unequal feature columns use staggered frames. Maintain a clear headline-to-action reading order amid the abundance.
...
- Combine cream stripes, plum double borders, patterned intervals, a pink arched panel, and a green hard offset.
- Let the control construction explain state and hierarchy.
- Do not replace abundance with three pastel cards or allow patterns to destroy the hierarchy of a working form.
...
| Token | Value | Role |
|---|---|---|
| `canvas` | `#f3dcb9` | Page environment; ink text |
| `surface` | `#fff2d9` | Opaque reading and control plane |
| `ink` | `#392638` | Main text on canvas or surface |
| `muted` | `#6b465c` | Secondary text on surface |
| `accent` | `#843254` | Primary action and selected state |
| `on-accent` | `#ffffff` | Text on the accent fill |
| `line` | `#77536b` | Grouping rules |
| `control-line` | `#77536b` | Visible input and control boundaries |
...
Combine cream stripes, plum double borders, patterned intervals, a pink arched panel, and a green hard offset. Feature surfaces alternate muted green, pink, and ochre. Keep actual reading areas solid so pattern is perceived around content rather than through it.
...
Use 8/12/20/28/40/64/88px. Alternate compressed metadata with generous reading blocks; detail must cluster around a main subject.
...
Controls remain consistent within their local group even while section frames differ. Primary actions use plum; badges use yellow with an ink outline. Use explicit labels and retain normal hit boxes; decorative rotation belongs to the surrounding frame.
...
Use eclectic original editorial art with deliberate relationships in scale and color. Repetition of a few motifs ties the layers together. Avoid unrelated stock imagery simply to fill gaps.
...
Charts use plain cream plotting areas, dark axes, directly labeled series, and pattern-coded alternatives; ornament must never distort data.
...
Keep the composition static. Use 140ms underlines and color changes on actual controls; abundance does not require constant motion.
...
- **Active / pressed:** Keep the composition static. Use 140ms underlines and color changes on actual controls; abundance does not require constant motion.
...
Background reference: [Shapero Modern: More is More](https://shaperomodern.com/exhibitions/18-maximalism-more-is-more/). The rules in this document are an original interface interpretation, not a reproduction of a source artifact or a claim that this variant exhausts the family.
