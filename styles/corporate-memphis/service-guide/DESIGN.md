---
name: Service Guide Corporate Memphis
slug: service-guide
family: corporate-memphis
description: "A practical service interpretation using friendly flat illustration as orientation within a disciplined task flow rather than an expansive brand story."
tags:
  - corporate-memphis
  - service-guide
  - complete-system
...
# Service Guide Corporate Memphis

## Overview and Design Philosophy

A practical service interpretation using friendly flat illustration as orientation within a disciplined task flow rather than an expansive brand story.
...
Use a compact 4:5 opening with a 48px bold title. Keep a small illustration strip in the workspace. Feature steps stack with colored number cells and aligned descriptions.

The distinction from **Community Story Corporate Memphis** is structural: that interpretation follows this direction: Center a 68px rounded sans title above a broad illustrated workspace. Features form spacious two-column chapters with a full-width opening story. Choose this variant when the composition above suits the content. A palette substitution alone does not establish either interpretation.

## Visual Character

Use flat cream surfaces, muted coral and teal index blocks, and modest 12px corners. Avoid decorative floating blobs behind every paragraph.
...
- Buttons are clear rounded rectangles. Tabs label service stages. Fields are grouped with persistent help, and dialogs use a compact friendly task panel.
...
| Token | Value | Role |
|---|---|---|
| `canvas` | `#edf0f7` | Page environment |
| `surface` | `#ffffff` | Opaque reading surface |
| `ink` | `#33415c` | Primary text on surface |
| `muted` | `#56637d` | Secondary text on surface |
| `accent` | `#566fa0` | Action and selection cue |
| `on-accent` | `#ffffff` | Text on filled accent |
| `line` | `#8392af` | Grouping rules |
| `control-line` | `#8392af` | Control boundary |
...
Use a 4px base with 8/12/16/24/32/48/64/96px intervals. The wide-screen section interval is 56px; the default module gap is 28px and outer gutter is 36px. Follow the role-specific interiors in the construction rules rather than forcing every region to have identical padding.
...
Use the original illustration in a small contextual crop. Functional steps and errors must remain complete without imagery.
...
Use 140ms color feedback and stable step transitions without moving unrelated form content.
...
**Pointer policy: none.** Illustration and plain service controls provide sufficient guidance and personality.
...
- **Active / pressed:** Use 140ms color feedback and stable step transitions without moving unrelated form content.
...
.design {
  ...
  --dialog-padding: 32px;
  --backdrop: #33415c99;
  --dialog-shadow: 0 6px 18px #33415c12;
  --stat: 36px;
  --scheme: light;
}
...
.design .hero{ grid-template-columns:1fr 1fr; gap:44px; }

.design h1{ font-weight:750; letter-spacing:-.045em; }

.design .project-card{ border:0; background:#f6eddd; border-radius:0; box-shadow:none; padding:24px; }

.design .style-artwork{ display:block; height:190px; background:url('/assets/corporate-memphis.png') center/cover; margin:12px -14px 0; }

.design .feature-card{ border:0; background:#edf0e6; border-radius:8px; box-shadow:none; }

.design .feature-card:nth-child(2){ background:#f8e1d9; }

.design .feature-card:nth-child(3){ background:#e6e4f5; }

.design .feature-index{ padding:14px; border-radius:50%; align-self:start; background:#d6d3ed; color:var(--ink); }

.design .stats{ border:0; background:#edf0e6; padding:28px; }

.design .testimonial{ border:0; background:#f6eddd; border-radius:40px 0; }
...
.design .hero{display:grid;grid-template-columns:4fr 5fr;gap:32px;align-items:start;padding:32px 0}.design .hero h1{font:750 48px/1.1 system-ui,sans-serif}
.design .panel{border:1px solid var(--control-line);border-radius:12px;background:#f4eee3;box-shadow:none;padding:24px}
.design .style-artwork{height:110px;border-radius:8px;background-position:center 45%}.design .features{grid-template-columns:1fr;gap:20px}
.design .feature-card:nth-child(n){display:grid;grid-template-columns:104px 1fr;gap:8px 20px}.design .feature-index{grid-row:1/4;background:#e8c2b1;border-radius:8px;padding:16px;font-weight:700}.design .feature-card h3{margin:0}
.design .stats{border:1px solid var(--control-line);border-radius:12px;padding:24px;background:#dce7db}.design .stats strong{font-size:32px}
.design .testimonial{text-align:left;border-left:6px solid #d5a894;padding:24px;font-size:20px}
.design .testimonial blockquote{font-size:20px}.design .demo-form{border-radius:12px}
