...
slug: night-terminal
family: cyberpunk
description: High-contrast dark panels, sharp signal accents, and dense but legible technical hierarchy.
tags:
  - cyberpunk
  - night-terminal
  - complete-system
...
High-contrast dark panels, sharp signal accents, and dense but legible technical hierarchy. This is one original interpretation of the cyberpunk family, not a definitive rulebook for the entire aesthetic. It is intended for complete websites and application surfaces that benefit from this visual character. Its layout, material choices, and control conventions below define why it exists as a separate system.

Use a 7:5 hero and three technical feature cells. Align readouts and metadata. Keep a readable task spine even when decorative rail positions vary. Separate caution and error messages from nonsemantic neon accents.
...
- Use signal accents to clarify state.
...
| `accent` | `#e4c66b` | Primary action and selected state |
| `on-accent` | `#262331` | Text on the accent fill |
...
Use monospaced headings and metadata with plain sans-serif body for longer reading. Keep uppercase labels short. Never replace Latin text with fabricated symbols or use glitch effects on information.

| Role | Stack | Size / line height | Weight and tracking |
|---|---|---|---|
| Display / H1 | `ui-monospace, monospace` | fluid 36–64px / 1.08 | 750; -0.03em |
| H2 | `ui-monospace, monospace` | 34px / 1.2 | 750; -0.02em |
| H3 | `ui-monospace, monospace` | 23px / 1.3 | 650; normal |
...
The outer container is at most 1200px wide, with 28px gutters and 24px gaps. Use CSS grid with `minmax(0, 1fr)` tracks so long content cannot enlarge the page. Use flex wrapping for action groups and navigation. Cards grow with content; do not enforce fixed text heights. Comparable pricing keeps the same benefit order and aligned actions even if decorative feature blocks have unequal spans.

## Surfaces, Borders, Radius, Shadows, and Depth

Use opaque near-black panels, pale gold actions, and restrained magenta edge accents. One 3px signal rail is enough for a group. No text glow, glass blur, or animated scanline overlay. Corners stay square.
...
Primary buttons use pale gold with dark labels; secondary controls are outlined dark panels. Inputs are opaque with bright enough boundaries. Cards have a short technical index and one strong edge. Pricing is explicit and factual. Navigation uses a signal underline, tabs invert when selected, and menus/dialogs are solid dark sheets with a bright boundary. No action is represented only by an icon or fake keyboard shortcut.
...
Use original night photography or abstract circuitry only in separate media regions. Avoid brand-like faction emblems and copied game interfaces. Icons use sharp 2px strokes. Charts use direct labels, solid/dashed series, and accessible tables. Decorative telemetry must not masquerade as actual metrics.
...
Use 80ms linear color changes. Absolutely no glitch flashing, strobing error states, moving scanlines, or text distortion. Reduced motion changes colors immediately.
...
```css
.design-cyberpunk-night-terminal {
  --canvas: #151721;
  --surface: #232636;
  --ink: #f0eef8;
  --muted: #bebdd2;
...
.design-cyberpunk-night-terminal * { box-sizing: border-box; }
```
...
- distinguish decoration from real data.
- Preserve the readable control and feedback behavior when adapting the visual composition.

## Don't

- Do not add flashing glitches, fake alerts, or unreadable microtext.
