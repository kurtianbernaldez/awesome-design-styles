---
name: Geometric Toybox Memphis
slug: geometric-toybox
family: memphis
description: "A modular toybox interpretation where repeated geometric keys and friendly rounded containers make pattern feel like an organized kit."
tags:
  - memphis
  - geometric-toybox
  - complete-system
...
# Geometric Toybox Memphis

## Overview and Design Philosophy

A modular toybox interpretation where repeated geometric keys and friendly rounded containers make pattern feel like an organized kit.
...
Center a 64px heavy title above a broad workspace. Use a two-column grid of equally weighted feature boxes. Repeat one circle, one stripe, and one checker motif at a controlled scale.

The distinction from **Editorial Collage Memphis** is structural: that interpretation follows this direction: Use a 3:2 opening with an 84px serif title. Offset the workspace slightly below the title. Features use an unequal two-column arrangement and oversized caption strips. Choose this variant when the composition above suits the content. A palette substitution alone does not establish either interpretation.

## Visual Character

Use pastel yellow, mint, and coral compartments with black outlines and large rounded-square corners. Patterns stay in borders and index tiles, leaving content interiors quiet.
...
- Buttons are outlined rounded keys with a small hard offset. Tabs are separate colored keys. Form controls are plain white slots inside patterned containers.
...
Use abstract original geometric illustrations. Pattern must remain decorative, with no meaning that depends on distinguishing a tiny shape.
...
Use 120ms key depression and no animated background patterns.
...
- **Active / pressed:** Use 120ms key depression and no animated background patterns.
...
.design .hero{display:block;padding:40px 0}.design .hero-copy{text-align:center;max-width:740px;margin:auto}.design .hero h1{font:850 64px/1.05 system-ui,sans-serif}
.design .hero-copy p{margin-inline:auto}.design .actions{justify-content:center}.design .project-card{max-width:800px;margin:36px auto 0;transform:none;border:3px solid;border-radius:32px;background:#f5e6ad;box-shadow:7px 7px var(--ink)}
.design .features{grid-template-columns:1fr 1fr;gap:32px}.design .feature-card:nth-child(n){transform:none;border:3px solid;border-radius:28px;background:#d5e9e3;box-shadow:6px 6px var(--ink)}
.design .feature-card:nth-child(2){background:#f2c9bb}.design .feature-card:nth-child(3){background:#e3d6f1}
.design .feature-index{display:inline-block;padding:12px;background:repeating-linear-gradient(90deg,#fff9 0 8px,transparent 8px 16px);border:2px solid;border-radius:50%}
.design .demo-tabs button{border:2px solid;border-radius:16px;background:#f5e6ad}.design .demo-tabs{border:0;gap:12px}
.design .stats{border:3px solid;border-radius:28px;padding:28px;background:#e3d6f1}.design .demo-form{border:3px solid;border-radius:28px;background:#d5e9e3}
