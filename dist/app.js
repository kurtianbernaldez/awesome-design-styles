...
function home() {
  ...
  main.innerHTML = `<section class="intro"><p class="eyebrow">THE OPEN DESIGN COLLECTION <span>VOL. 01 / ${families.flatMap(f=>f.variants).length} SYSTEMS</span></p><div class="intro-row"><h1>Find your<br>visual language<span class="accent-dot">.</span></h1><div class="intro-note"><span class="small-rule"></span><p>A style is a starting point.<br>Find the interpretation that feels right.</p><p class="subtle">Compare the same interface. Choose a complete design system. Make it yours.</p></div></div></section>
  <section class="collection" aria-labelledby="collection-title"><div class="section-label"><h2 id="collection-title">Explore the families</h2><span>${String(families.length).padStart(2,'0')} collections</span></div><div class="family-grid">${families.map((f,i)=>`<a class="family-card" href="/styles/${esc(f.slug)}" data-route><div class="family-art art-${esc(f.slug)}"><span class="art-index">0${i+1} / ${esc(f.name.toUpperCase())}</span><div class="mini-browser"><div class="mini-nav"><strong>Common Ground</strong><span>Workspace &nbsp; About</span></div><div class="mini-hero"><span class="mini-eyebrow">A PLACE FOR YOUR NEXT IDEA</span><strong>Good work.<br>Made together.</strong><p>Bring your notes, plans, and people into one place.</p><span class="mini-button">Start a project <b>↗</b></span></div><div class="mini-blocks"><span>01<br><b>Make room.</b></span><span>02<br><b>Find focus.</b></span><span>03<br><b>Move forward.</b></span></div></div></div><div class="family-info"><div class="family-title"><h3>${esc(f.name)}</h3><span>${f.variants.length} variants</span></div><p>${esc(f.description)}</p><div class="variant-names">${f.variants.map(v=>`<span>${esc(label(v.slug))}</span>`).join('')}</div><div class="explore-link">Explore ${esc(f.name)} <span aria-hidden="true">↗</span></div></div></a>`).join('')}</div></section>
