...
description: Optimistic instrument-like modules with warm panels, rounded windows, and clear analog cues.
tags:
  - retro-futurism
  - orbital-console
  - complete-system
...
Optimistic instrument-like modules with warm panels, rounded windows, and clear analog cues. This is one original interpretation of the retro futurism family, not a definitive rulebook for the entire aesthetic. It is intended for complete websites and application surfaces that benefit from this visual character. Its layout, material choices, and control conventions below define why it exists as a separate system.

Use equal hero halves and two-column feature modules. The project pane may be a rounded instrument window; the rest of the page stays in a stable modular grid. Align data readouts in a single clear band.
...
- Pair warm housings with clear readouts.
...
| `muted` | `#5c6b70` | Secondary text on surface |
...
Use broad bold sans headings and monospaced technical labels. Keep the display expressive but readable; body is 17px. Tabular statistics evoke instruments without recreating an existing device interface.

| Role | Stack | Size / line height | Weight and tracking |
|---|---|---|---|
| Display / H1 | `system-ui, sans-serif` | fluid 36–62px / 1.08 | 750; -0.03em |
...
Use warm opaque housings with 28px corners, inset reading windows, and dark seams. Restrained copper actions and pale blue secondary fields create optimism. Avoid glossy chrome and literal spacecraft controls.

Reference card boundary: 2px where a frame is used; reference corner radius: 28px. Surface shadow: `0 6px 0 #a4aaa5`. Controls generally use 12px corners unless their component treatment specifies a more precise shape. Content stays at layer 0, navigation at 10 when sticky, menus at 20, backdrop at 40, dialog at 50, and transient feedback at 60. Avoid creating stacking contexts on every card. Overlays must remain above all decorative treatments.

## Components

Primary buttons are copper with white labels and 12px corners. Secondary controls use outlined cream. Inputs are shallow inset rectangles with 8px corners and strong boundaries. Cards have rounded housing edges and a distinct title area. Pricing recommendation uses a short status label. Navigation forms a rounded rail; tabs appear as inset windows with a dark selected fill. Menus and dialogs are opaque, calmer versions of the same housing.
...
Use original optimistic technical illustration or abstract orbital geometry outside controls. Avoid trademarked spacecraft, badges, and film interfaces. Charts are flat and factual within readout windows, with units and table alternatives; do not replace quantities with decorative gauges.
...
Use 140ms ease-out for button color and shallow press depth. No radar sweep, moving star fields, or automatic instrument simulation. Reduced motion uses static labels and instant state changes.
...
```css
.design-retro-futurism-orbital-console {
  --canvas: #e7dfcc;
  --surface: #fff3dc;
  --ink: #39444a;
  --muted: #5c6b70;
...
.design-retro-futurism-orbital-console * { box-sizing: border-box; }
```
...
- Pair warm housings with clear readouts.
- retain conventional controls.
- keep the mood optimistic.
- Preserve the readable control and feedback behavior when adapting the visual composition.

## Don't

- Do not imitate a particular film console or use fake indicators that appear functional.
