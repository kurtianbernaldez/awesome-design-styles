---
name: Annotated Manuscript Dark Academia
slug: annotated-manuscript
family: dark-academia
description: "A manuscript-led interpretation placing cream reading leaves inside a dark scholarly environment, with marginal notes and literary hierarchy."
tags:
  - dark-academia
...
# Annotated Manuscript Dark Academia

## Overview and Design Philosophy

A manuscript-led interpretation placing cream reading leaves inside a dark scholarly environment, with marginal notes and literary hierarchy.
...
Use a 3:2 opening and a 66px italic serif title. Workspace content occupies a pale manuscript leaf. Features stack as reading entries with a narrow annotation rail.

The distinction from **Scholarly Journal Dark Academia** is structural: that interpretation follows this direction: Use a centered 64px upright serif title above a broad journal abstract panel. Feature entries form two columns with fine rules and clear section numbering. Choose this variant when the composition above suits the content. A palette substitution alone does not establish either interpretation.

## Visual Character

Use dark walnut surroundings, warm paper interiors, fine brown rules, and small burgundy index marks. Keep all text on paper dark and high contrast.
...
- Actions are dark stamped labels. Tabs resemble manuscript index headings. Fields are pale writable areas, and dialogs use an opaque paper sheet.
...
Use original archival-object or book-detail photography. Transcribe any meaningful handwriting and avoid unlicensed manuscript scans.
...
Use 180ms color changes and no simulated page turns.
...
- **Active / pressed:** Use 180ms color changes and no simulated page turns.
...
.design .hero{display:grid;grid-template-columns:3fr 2fr;gap:48px;align-items:start}.design .hero h1{font:400 italic 66px/1.12 Georgia,serif}
.design .panel{background:#e8ddc6;color:#332d26;border:3px double #8d7a5d;border-radius:0;box-shadow:5px 6px #110e0b55;padding:28px}
.design .panel :is(p,small,.feature-index,.card-top>span,.card-bottom){color:#625749}.design .panel a{color:#332d26}
.design .features{grid-template-columns:1fr;gap:32px}.design .feature-card:nth-child(n){display:grid;grid-template-columns:100px 1fr;gap:8px 24px}
.design .feature-index{grid-row:1/4;font-style:italic}.design .feature-card h3{margin:0;font:400 28px/1.3 Georgia,serif}
.design .project-rows>div{border-bottom:1px solid #ad9d80}.design .stats{border-block:3px double var(--line);padding:28px 0}
.design .testimonial{text-align:left;font:italic 28px/1.7 Georgia,serif;border-left:3px solid var(--line);padding:32px}
.design .testimonial blockquote{font:italic 28px/1.7 Georgia,serif}
.design .demo-form :is(input,textarea,select){background:#faf0db;color:#332d26;border-color:#887557}
.design .panel{--surface:#e8ddc6;--ink:#332d26;--muted:#625749;--accent:#76552e;--on-accent:#ffffff;--line:#ad9d80;--control-line:#887557;color:var(--ink)}
