---
name: Scholarly Journal Dark Academia
slug: scholarly-journal
family: dark-academia
description: "A formal publication interpretation with restrained dark paper, small scholarly indexes, and strict typographic columns instead of manuscript-like leaves."
tags:
  - dark-academia
  - scholarly-journal
  - complete-system
...
# Scholarly Journal Dark Academia

## Overview and Design Philosophy

A formal publication interpretation with restrained dark paper, small scholarly indexes, and strict typographic columns instead of manuscript-like leaves.
...
Use a centered 64px upright serif title above a broad journal abstract panel. Feature entries form two columns with fine rules and clear section numbering.

The distinction from **Annotated Manuscript Dark Academia** is structural: that interpretation follows this direction: Use a 3:2 opening and a 66px italic serif title. Workspace content occupies a pale manuscript leaf. Features stack as reading entries with a narrow annotation rail. Choose this variant when the composition above suits the content. A palette substitution alone does not establish either interpretation.

## Visual Character

Use near-black brown, muted parchment text, fine brass rules, and no ornamental frames or distressed texture. Typography and proportions carry the academic atmosphere.
...
- Actions are modest outlined rectangles. Tabs use a selected double underline. Fields and dialogs are opaque dark reading panels with clear boundaries.
...
| Token | Value | Role |
|---|---|---|
| `canvas` | `#262722` | Page environment |
| `surface` | `#34372e` | Opaque reading surface |
| `ink` | `#eee6cb` | Primary text on surface |
| `muted` | `#c3bea6` | Secondary text on surface |
| `accent` | `#dec78d` | Action and selection cue |
| `on-accent` | `#303327` | Text on filled accent |
| `line` | `#96957b` | Grouping rules |
| `control-line` | `#96957b` | Control boundary |
...
Use original documentary imagery with formal captions. Charts should follow the same thin-rule scholarly treatment and include sources.
...
Use immediate selection and 160ms overlay opacity.
...
**Pointer policy: none.** Long-form reading and scholarly annotations should remain visually stable.
...
- **Active / pressed:** Use immediate selection and 160ms overlay opacity.
...
.design .hero{display:block;border-block:3px double var(--line);padding:48px 0;margin:32px 0}.design .hero-copy{text-align:center;max-width:760px;margin:auto}
.design .hero h1{font:400 64px/1.12 Georgia,serif;letter-spacing:.02em}.design .hero-copy p{margin-inline:auto}.design .actions{justify-content:center}
.design .project-card{max-width:820px;margin:40px auto 0;background:none;border:0;border-block:1px solid var(--line);border-radius:0;box-shadow:none;padding:28px 0}
.design .features{grid-template-columns:1fr 1fr;gap:40px}.design .feature-card:nth-child(n){background:none;border:0;border-top:1px solid var(--line);border-radius:0;box-shadow:none;padding:28px 0}
.design .feature-index{font:12px/1.5 ui-monospace,monospace;letter-spacing:.12em}.design .feature-card h3{font:400 28px/1.3 Georgia,serif}
.design .stats{border-block:3px double var(--line);padding:28px 0}.design .stats strong{font:400 42px/1.2 Georgia,serif}
.design .testimonial{border:0;font:italic 26px/1.7 Georgia,serif}
.design .testimonial blockquote{font:italic 26px/1.7 Georgia,serif}.design .demo-form{background:none;border:0;border-top:3px double var(--line);border-radius:0;padding:32px 0;box-shadow:none}
.design .project-card{--surface:inherit;--ink:inherit;--muted:inherit;--accent:inherit;--line:inherit;--control-line:inherit;color:var(--ink)}
