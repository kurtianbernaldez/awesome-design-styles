---
name: Assembly Line Industrial
slug: assembly-line
family: industrial
description: "A factory-floor interpretation that separates operations into large labeled stations with conspicuous safety bands and mechanical controls."
tags:
  - industrial
  - assembly-line
  - complete-system
...
A factory-floor interpretation that separates operations into large labeled stations with conspicuous safety bands and mechanical controls.
...
Use a broad 72px condensed heading above a full-width workstation. Features form equal station columns; statistics appear as individual readout banks.

The distinction from **Technical Manual Industrial** is structural: that interpretation follows this direction: Use a compact 4:5 opening, 48px condensed heading, and tabular workspace. Stack feature entries with an 88px drawing-number rail. Keep all prose upright and left aligned. Choose this variant when the composition above suits the content. A palette substitution alone does not establish either interpretation.

## Visual Character

Use dark steel panels, thick seams, amber safety bands at outer edges, and shallow inset work areas. Avoid distressing labels or rendering every surface as shiny chrome.
...
- Buttons are large mechanical keys with a pressed bevel. Tabs select a station with a solid amber marker. Fields remain opaque and dialogs use a bolted-looking outer frame without obstructing text.
...
| Token | Value | Role |
|---|---|---|
| `canvas` | `#20292d` | Page environment |
| `surface` | `#303c40` | Opaque reading surface |
| `ink` | `#edf1e7` | Primary text on surface |
| `muted` | `#c0cbc7` | Secondary text on surface |
| `accent` | `#e0c16d` | Action and selection cue |
| `on-accent` | `#20292d` | Text on filled accent |
| `line` | `#8f9e9c` | Grouping rules |
| `control-line` | `#8f9e9c` | Control boundary |
...
| Section heading | `'Barlow Condensed','Arial Narrow',sans-serif` | 30px / 1.2; 28px on phones |
| Card heading | `'Barlow Condensed','Arial Narrow',sans-serif` | 22px / 1.3; variant overrides below take precedence |
...
Use original workshop photography and process diagrams. Safety striping is decorative here and must not replace actual warning words.
...
Use immediate mechanical depression; no automatic blinking indicators.
...
**Pointer policy: none.** Technical information and station hierarchy need stable reading surfaces.
...
- **Active / pressed:** Use immediate mechanical depression; no automatic blinking indicators.
...
.design .hero{display:block;padding:36px 0}.design .hero h1{font:700 72px/.98 'Barlow Condensed',Impact,sans-serif;text-transform:uppercase}
.design .hero-copy{border-left:16px solid var(--accent);padding-left:24px}.design .project-card{margin:36px 0 0;border:3px solid var(--control-line);border-top:20px solid var(--accent);padding:32px;border-radius:4px}
.design .project-rows{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:24px}.design .project-rows>div{display:block;border:0;border-left:2px solid var(--line);padding-left:16px}
.design .features{gap:24px}.design .feature-card:nth-child(n){border:3px solid var(--control-line);border-top:16px solid var(--accent);border-radius:4px}
.design .stats{background:none;border:0;gap:24px}.design .stats>div{background:var(--surface);border:3px solid var(--control-line);padding:24px}
.design .stats strong{font:700 40px/1.1 ui-monospace,monospace}.design .primary{border-radius:3px;padding:16px 28px}
.design .testimonial{text-align:left;border-left:12px solid var(--accent);padding:32px}.design .demo-form{border-top:20px solid var(--accent)}
