---
name: Op Art Psychedelic
slug: op-art
family: psychedelic
description: "An optical-geometry interpretation using disciplined black-and-cream line fields and striking scale rather than flowing multicolor organic forms."
tags:
  - psychedelic
  - op-art
  - complete-system
...
An optical-geometry interpretation using disciplined black-and-cream line fields and striking scale rather than flowing multicolor organic forms.
...
Use a centered 80px condensed title inside a broad optical border. Keep the workspace and all text on plain opaque planes. Feature sections use a measured two-column grid.

The distinction from **Flower Power Psychedelic** is structural: that interpretation follows this direction: Use a broad 76px heavy serif heading beside a rounded workspace. Features form two large organically framed sections, with generous text interiors and repeated curved edges. Choose this variant when the composition above suits the content. A palette substitution alone does not establish either interpretation.

## Visual Character

Use monochrome concentric or striped abstract patterns only at section edges. Large quiet interiors prevent visual vibration from interfering with reading.
...
- Actions are solid dark rectangles with clear outlines. Tabs use a thick inverse selection. Inputs and dialogs remain pattern-free, with a plain modal backdrop.
...
| Token | Value | Role |
|---|---|---|
| `canvas` | `#e8e5d9` | Page environment |
| `surface` | `#f5f2e6` | Opaque reading surface |
| `ink` | `#30302b` | Primary text on surface |
| `muted` | `#5e5d54` | Secondary text on surface |
| `accent` | `#30302b` | Action and selection cue |
| `on-accent` | `#ffffff` | Text on filled accent |
| `line` | `#99968a` | Grouping rules |
| `control-line` | `#77756b` | Control boundary |
| `error` | `#822d38` | Error text with written explanation |
| `success` | `#355b3d` | Success text with written confirmation |
| `warning` | `#70520b` | Warning text with written next step |
...
| Section heading | `ui-serif, Georgia, serif` | 42px / 1.2; 28px on phones |
| Card heading | `ui-serif, Georgia, serif` | 27px / 1.3; variant overrides below take precedence |
...
Use a 4px base with 8/12/16/24/32/48/64/96px intervals. The wide-screen section interval is 104px; the default module gap is 32px and outer gutter is 36px. Follow the role-specific interiors in the construction rules rather than forcing every region to have identical padding.
...
Use original optical studies sparingly. Provide a reduced-decoration mode and never animate high-contrast patterns.
...
Use immediate state changes; all optical patterns remain static.
...
**Pointer policy: page.** A small static-pattern aperture lets pointer movement explore the optical geometry.

### Optical aperture

A small static striped aperture follows the pointer; its pattern never scrolls or flashes. This is optional decorative feedback, not a status indicator.

**Construction.** Clip static 3px ink stripes, separated by 5px clear gaps and inclined 80 degrees, into a circle. Limit paint opacity to 12%; the stripes must never scroll, pulse, invert, or flash. Unless a material-specific size above overrides it, use a 144px-wide region, height equal to width / 1.45, and a 2px base stroke. Sparse ink geometry defaults to 16% paint opacity; minimalist work uses 12%, brutalist work 20%, Art Deco 28%, and Wabi-Sabi 10%. Explicit recipe opacity takes precedence. Apply a further 70% opacity to the page-level geometry; glass uses its explicit light values. Use this document’s accent and ink tokens.
...
- **Active / pressed:** Use immediate state changes; all optical patterns remain static.
...
.design {
  ...
  --dialog-padding: 32px;
  --backdrop: #4b294d99;
  --dialog-shadow: 8px 8px 0 #b77e9e;
  --stat: 36px;
  --scheme: light;
}
...
.design .demo-shell{ background:#f3dfb7; border-inline:8px solid #6d397e; border-radius:100px 0 100px 0 / 40px 0 40px 0; }

.design .hero{ gap:42px; border-top:20px solid; border-image:repeating-linear-gradient(110deg,#a05c96 0 18px,#df9868 18px 36px,#e4ca74 36px 54px,#755387 54px 72px) 20; margin-top:20px; }

.design h1{ font:900 clamp(48px,7.5vw,88px)/.95 Georgia,serif; letter-spacing:-.075em; font-style:italic; color:#6d397e; text-shadow:2px 2px #e48f65; }

.design .project-card{ background:#eed18f; border:5px solid #6d397e; border-radius:44% 28% 38% 25% / 35px 28px 45px 38px; box-shadow:8px 8px #c06b93,14px 14px #6d397e; }

.design .feature-card{ border:4px solid #6d397e; border-radius:35px 10px 35px 10px; box-shadow:5px 5px #c06b93; }

.design .feature-card:nth-child(2){ background:#e5b2c7; border-radius:10px 35px; }

.design .stats{ background:#e5b2c7; padding:28px; border:4px solid #6d397e; border-radius:50% / 20px; }

.design :is(button,.badge){ border-radius:50% / 25%; }

.design .testimonial{ border:5px double #6d397e; border-radius:50% / 12%; }
...
.design body{background:#e8e5d9}.design .hero{display:block;padding:40px!important;margin:32px 0;border:12px double #30302b;background:repeating-linear-gradient(90deg,#30302b 0 3px,#f5f2e6 3px 12px)}
.design .hero-copy{text-align:center;max-width:820px;margin:auto;background:#f5f2e6;padding:28px}.design .hero h1{font:700 80px/1 'Barlow Condensed',Impact,sans-serif;text-shadow:none;transform:none}
.design .hero-copy p{margin-inline:auto}.design .actions{justify-content:center}.design .project-card{max-width:820px;margin:32px auto 0;border:6px double #30302b;border-radius:0;background:#f5f2e6;box-shadow:none;transform:none}
.design .features{grid-template-columns:1fr 1fr;gap:32px}.design .feature-card:nth-child(n){border:6px double #30302b;border-radius:0;background:#f5f2e6;box-shadow:none;transform:none}
.design .stats{background:#f5f2e6;border-block:12px double #30302b;padding:28px}.design .stats strong{font:700 52px/1.2 'Barlow Condensed',Impact,sans-serif}
.design .testimonial{border:0;font:italic 26px/1.6 Georgia,serif}
.design .testimonial blockquote{font:italic 26px/1.6 Georgia,serif}.design .demo-form{border:6px double #30302b;border-radius:0;background:#f5f2e6;box-shadow:none}
@media(prefers-contrast:more){.design .hero{background:#f5f2e6}}
...
.design .demo-shell{border-inline:0}.design .demo-nav{background:none;border:0;border-bottom:2px solid var(--ink);border-radius:0}.design .demo-nav::after{display:none}.design .primary,.design .secondary,.design .primary:hover:not(:disabled){background:var(--accent);color:var(--on-accent);border:2px solid var(--ink);border-radius:0;box-shadow:none}.design .badge{background:var(--surface);color:var(--ink);border:1px solid var(--ink);border-radius:0}
...
.design .hero h1{letter-spacing:-.015em}
