...
description: Tactile panels and inset controls that use physical affordances to clarify digital actions.
tags:
  - skeuomorphism
  - instrument-panel
  - complete-system
...
Tactile panels and inset controls that use physical affordances to clarify digital actions. This is one original interpretation of the skeuomorphism family, not a definitive rulebook for the entire aesthetic. It is intended for complete websites and application surfaces that benefit from this visual character. Its layout, material choices, and control conventions below define why it exists as a separate system.

Use a 3:2 hero and regular control modules. Align instruments to a straight grid and separate readouts from actions. A product page may feel like a desktop instrument, but reading and tab order remain ordinary document order.
...
Use sturdy sans-serif labels and tabular monospaced readouts. Headings remain moderate so the material hierarchy, rather than giant typography, defines the interface.

| Role | Stack | Size / line height | Weight and tracking |
|---|---|---|---|
| Display / H1 | `system-ui, sans-serif` | fluid 36–52px / 1.08 | 650; -0.03em |
| H2 | `system-ui, sans-serif` | 30px / 1.2 | 650; -0.02em |
| H3 | `system-ui, sans-serif` | 22px / 1.3 | 650; normal |
...
The outer container is at most 1120px wide, with 32px gutters and 24px gaps. Use CSS grid with `minmax(0, 1fr)` tracks so long content cannot enlarge the page. Use flex wrapping for action groups and navigation. Cards grow with content; do not enforce fixed text heights. Comparable pricing keeps the same benefit order and aligned actions even if decorative feature blocks have unequal spans.

## Surfaces, Borders, Radius, Shadows, and Depth

Use a warm matte housing, softly raised panel edges, and inset field wells. Light comes consistently from above. Borders remain visible independently of shading. Avoid imitation leather, photographed screws, or proprietary device shapes.

Reference card boundary: 1px where a frame is used; reference corner radius: 12px. Surface shadow: `inset 0 1px 0 #fff9, 0 5px 12px #332e2729`. Controls generally use 12px corners unless their component treatment specifies a more precise shape. Content stays at layer 0, navigation at 10 when sticky, menus at 20, backdrop at 40, dialog at 50, and transient feedback at 60. Avoid creating stacking contexts on every card. Overlays must remain above all decorative treatments.

## Components

Buttons have a slightly raised top edge and a firm dark accent fill; pressed state removes the outer elevation. Inputs use an inset shadow plus a solid boundary, never shadow alone. Cards resemble shallow trays with a title strip and clear content area. Pricing compares equal trays rather than physical objects at different scales. Navigation is a horizontal rail; menus are flat opaque sheets. Selected tabs appear connected to their panel. Dialogs use an opaque housing and contain standard text actions; decorative knobs must not replace accessible controls.
...
Use original close-up material imagery only outside text areas. Interface material cues should be CSS shading, not image dependencies. Icons use familiar literal shapes at 24px. Charts are flat and precise inside inset readout areas; offer a table and visible units rather than relying on dial angles.
...
Press depth changes over 100ms ease-out; menus fade in 140ms. No mechanical sound, automatic needle sweep, or spring-loaded page transitions. Reduced motion changes elevation immediately.
...
```css
.design-skeuomorphism-instrument-panel {
  --canvas: #d0cbc1;
  --surface: #eee9df;
  --ink: #332e27;
  --muted: #625b50;
...
.design-skeuomorphism-instrument-panel * { box-sizing: border-box; }
```
...
- Keep one light direction.
- distinguish raised actions from inset fields.
- retain explicit labels.
- Preserve the readable control and feedback behavior when adapting the visual composition.

## Don't

- Do not recreate a specific hardware product or make tactile depth the only state cue.
