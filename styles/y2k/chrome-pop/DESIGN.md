...
slug: chrome-pop
family: y2k
description: Sleek pill geometry, cool metallic accents, and compact expressive panels with readable interiors.
tags:
  - y2k
  - chrome-pop
  - complete-system
...
Sleek pill geometry, cool metallic accents, and compact expressive panels with readable interiors. This is one original interpretation of the y2k family, not a definitive rulebook for the entire aesthetic. It is intended for complete websites and application surfaces that benefit from this visual character. Its layout, material choices, and control conventions below define why it exists as a separate system.

Use a 7:5 hero and three compact feature panels. A rounded project window is the main focal surface. Keep navigation compact and section spacing regular; novelty comes from edge finish and shape rather than layout chaos.
...
- Keep shiny accents outside the reading plane.
...
| Token | Value | Role |
|---|---|---|
| `canvas` | `#dce0f1` | Page environment; ink text |
| `surface` | `#f5f5ff` | Opaque reading and control plane |
| `ink` | `#35334f` | Main text on canvas or surface |
| `muted` | `#62617d` | Secondary text on surface |
| `accent` | `#655390` | Primary action and selected state |
| `on-accent` | `#ffffff` | Text on the accent fill |
| `line` | `#827fa0` | Grouping rules |
| `control-line` | `#827fa0` | Visible input and control boundaries |
...
Use heavy, tightly tracked sans display type and small monospaced metadata. Keep body and controls plain. Do not use illegible liquid lettering or artificially stretched fonts.

| Role | Stack | Size / line height | Weight and tracking |
|---|---|---|---|
| Display / H1 | `system-ui, sans-serif` | fluid 36–68px / 1.08 | 800; -0.03em |
| H2 | `system-ui, sans-serif` | 34px / 1.2 | 800; -0.02em |
...
Use 4/8/12/16/24/32/48/72px. The hero is compact and expressive; supporting panels keep regular gaps and 24px padding.
...
Use cool opaque panels and rounded pill controls. Metallic gradients are limited to non-text edge strips and secondary button surfaces with tested dark labels. The reading plane is solid, not reflective.

Reference card boundary: 1px where a frame is used; reference corner radius: 22px. Surface shadow: `inset 0 2px 0 #ffffff,0 5px 14px #4f487529`. Controls generally use 12px corners unless their component treatment specifies a more precise shape. Content stays at layer 0, navigation at 10 when sticky, menus at 20, backdrop at 40, dialog at 50, and transient feedback at 60. Avoid creating stacking contexts on every card. Overlays must remain above all decorative treatments.

## Components

Primary buttons are solid violet with white labels and pill corners. Secondary controls have a pale metallic gradient and dark text. Inputs are opaque, 10px rounded rectangles. Cards use cool panels with a reflective top seam, not mirrored content. Pricing displays clear numeric values. Navigation forms a slim rounded bar, tabs use inset pill selection, and dialogs are opaque with a restrained metallic border strip.
...
Use original abstract digital imagery or crisp product crops. Avoid copied operating-system chrome, logos, or recognizable era-specific mascots. Icons use clear 2px strokes. Data charts remain flat with high-contrast labels; no metallic bars or distorted perspective. Supply table alternatives.
...
Use 140ms ease-out color changes. No animated chrome sweep, flashing stars, or cursor trails. Reduced motion removes fades and uses static loading text.
...
.design-y2k-chrome-pop * { box-sizing: border-box; }
```
...
- Keep shiny accents outside the reading plane.
- pair compact panels with clear controls.
- test dark labels on metallic fills.
- Preserve the readable control and feedback behavior when adapting the visual composition.

## Don't

- Do not use mirrored text, animated sparkles, or copied software chrome.
