---
name: Hypertext Index Web 1.0
slug: hypertext-index
family: web-1.0
description: "A document-network interpretation where concise headings, visible links, and ordered reference sections take precedence over decorative homepage boxes."
tags:
  - web-1.0
  - hypertext-index
  - complete-system
...
# Hypertext Index Web 1.0

## Overview and Design Philosophy

A document-network interpretation where concise headings, visible links, and ordered reference sections take precedence over decorative homepage boxes.
...
Use a single left-aligned 44px serif heading and a narrow reading measure. Place the workspace below as a simple ruled list. Stack all feature entries as document sections.

The distinction from **Personal Homepage Web 1.0** is structural: that interpretation follows this direction: Center a 56px serif welcome heading above a broad bordered workspace. Arrange feature sections in two simple columns and retain visibly separate navigation links. Choose this variant when the composition above suits the content. A palette substitution alone does not establish either interpretation.

## Visual Character

Use plain warm white, dark text, blue underlined links, and thin horizontal rules. No shadows, colored cards, or rounded panel shells.
...
- Use conventional native-looking controls and always-visible link underlines. Tabs are clearly labeled section selectors. Dialogs use a simple bordered white document.
...
Use a 4px base with 8/12/16/24/32/48/64/96px intervals. The wide-screen section interval is 48px; the default module gap is 20px and outer gutter is 24px. Follow the role-specific interiors in the construction rules rather than forcing every region to have identical padding.
...
Use a flexible container around 900px, with minmax(0,1fr) tracks and content-sized cards. Keep navigation, opening, workspace, statistics, features, testimonial, pricing, contact, and footer in a coherent source order. A visually offset panel must not change the keyboard order. Allow action rows to wrap and avoid fixed heights around text.
...
Use diagrams only when they explain the text. The page should remain complete and coherent as a text-only document.
...
Use immediate state changes; no transition is required.
...
- **Active / pressed:** Use immediate state changes; no transition is required.
...
.design .demo-shell{max-width:900px}.design .hero{display:block;padding:28px 0}.design .hero h1{font:700 44px/1.15 'Times New Roman',serif}
.design .project-card{margin:32px 0 0;border:0;border-block:1px solid #888;border-radius:0;background:none;box-shadow:none;padding:24px 0}
.design .features{grid-template-columns:1fr;gap:28px}.design .feature-card:nth-child(n){border:0;border-top:1px solid #888;background:none;padding:24px 0;box-shadow:none}
.design .feature-index{font:14px/1.5 ui-monospace,monospace}.design .feature-card h3{font:700 26px/1.3 'Times New Roman',serif}
.design .stats{border-block:1px solid #888;padding:24px 0;background:none}.design .stats strong{font:700 30px/1.2 'Times New Roman',serif}
.design .testimonial{text-align:left;border:0;border-left:3px solid #888;padding:24px;font:italic 22px/1.6 'Times New Roman',serif}
.design .testimonial blockquote{font:italic 22px/1.6 'Times New Roman',serif}
.design .demo-form{border:0;border-top:1px solid #888;background:none;padding:24px 0}.design .demo-nav{border-bottom:1px solid #888}
