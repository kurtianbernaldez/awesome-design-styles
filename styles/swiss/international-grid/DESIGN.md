...
family: swiss
description: A disciplined asymmetric grid with precise sans-serif hierarchy and content-led order.
tags:
  - swiss
  - international-grid
  - complete-system
...
A disciplined asymmetric grid with precise sans-serif hierarchy and content-led order. This is one original interpretation of the swiss family, not a definitive rulebook for the entire aesthetic. It is intended for complete websites and application surfaces that benefit from this visual character. Its layout, material choices, and control conventions below define why it exists as a separate system.

Use a 12-column grid with an 8:4 hero and a 3:6:3 content rhythm. Headlines align left; captions align with the image or table they explain. Never justify body copy into uneven word spaces. Unequal columns must reflect information priority.
...
Use one neutral sans stack, flush-left ragged-right text, and a clear size contrast. Avoid italics and ornamental capitals. Labels and numerals share the same family; tabular figures support comparisons.

| Role | Stack | Size / line height | Weight and tracking |
|---|---|---|---|
| Display / H1 | `system-ui, sans-serif` | fluid 36–72px / 1.08 | 650; -0.03em |
| H2 | `system-ui, sans-serif` | 36px / 1.2 | 650; -0.02em |
...
The outer container is at most 1200px wide, with 40px gutters and 24px gaps. Use CSS grid with `minmax(0, 1fr)` tracks so long content cannot enlarge the page. Use flex wrapping for action groups and navigation. Cards grow with content; do not enforce fixed text heights. Comparable pricing keeps the same benefit order and aligned actions even if decorative feature blocks have unequal spans.

## Surfaces, Borders, Radius, Shadows, and Depth

Use a continuous pale page and thin gray rules. Cards do not need individual background fills. Red is an action signal, not an alternating decoration. No shadows, rounded corners, or material simulation.
...
Buttons are compact red rectangles with white labels and a 44px target. Secondary actions are outlined or underlined. Forms use crisp rectangular fields and left-aligned labels. Feature cards are ruled columns rather than elevated boxes. Pricing presents aligned baselines and identical benefit ordering. Navigation is a text index; current links have a 2px red underline. Tabs share one baseline. Menus and dialogs use plain white, a thin dark border, and no decorative elevation.
...
Use documentary photographs with uncropped meaning, rectangular edges, and careful captions. Diagrams and charts have consistent label placement. Tables align decimals and units, with row rules only when needed. Provide a textual conclusion and accessible data alternative for every chart.
...
Use 100ms color changes only. Reading content appears immediately; no animated headline, masonry shuffle, or parallax. Reduced motion removes transitions.
...
```css
.design-swiss-international-grid {
  --canvas: #f8f8f5;
  --surface: #ffffff;
  --ink: #222b30;
  --muted: #52616a;
...
.design-swiss-international-grid * { box-sizing: border-box; }
```
...
- Align across sections.
- distinguish hierarchy through scale and space.
- edit labels for clarity.
- Preserve the readable control and feedback behavior when adapting the visual composition.

## Don't

- Do not center long text, decorate every module, or introduce arbitrary offsets.
