---
name: Typographic Plane De Stijl
slug: typographic-plane
family: de-stijl
description: "A print-led interpretation that leaves most content unboxed and uses asymmetric typography with a few decisive orthogonal color planes."
tags:
  - de-stijl
  - typographic-plane
  - complete-system
...
# Typographic Plane De Stijl

## Overview and Design Philosophy

A print-led interpretation that leaves most content unboxed and uses asymmetric typography with a few decisive orthogonal color planes.
...
Use a 90px left-aligned headline with an offset workspace below. Place one short colored rule against each long reading baseline. Feature content forms unboxed horizontal bands.

The distinction from **Architectural Grid De Stijl** is structural: that interpretation follows this direction: Use adjoining 5:4 hero compartments divided by an 8px black beam. Features share boundaries with no gaps, forming unequal 2:1:1 rooms. Maintain clear rectangular reading zones. Choose this variant when the composition above suits the content. A palette substitution alone does not establish either interpretation.

## Visual Character

Use a nearly white field, black type, and small red, yellow, or blue planes located at grid intersections. Color becomes a counterweight to type rather than a full card fill.
...
- Buttons remain flat rectangles; links are underlined. Tabs use a thick selected underline. Inputs have clear thin frames, and dialogs are white sheets with one colored edge.
...
Use pure rectilinear diagrams and sparse original photography. Keep the composition legible with all images removed.
...
Use immediate underline and fill changes with no animated geometry.
...
- **Active / pressed:** Use immediate underline and fill changes with no animated geometry.
...
.design .hero{display:block;padding:40px 0}.design .hero-copy{border-left:24px solid #c33b2d;padding-left:28px;background:none}
.design .hero h1{font:750 90px/.98 Arial,sans-serif;max-width:11ch}.design .project-card{margin:40px 0 0 34%;border:0;border-top:12px solid #34588c;padding:24px 0;background:none;box-shadow:none}
.design .features{grid-template-columns:1fr;gap:32px}.design .feature-card:nth-child(n){border:0;border-top:3px solid;padding:28px 0;background:none}
.design .feature-index{display:block;border-left:24px solid #e4bc38;padding-left:16px;font-weight:700}
.design .stats{background:none;border:0;border-block:3px solid;padding:28px 0}.design .stats strong{font-size:56px}
.design .demo-form{border:0;border-top:12px solid #34588c;padding:32px 0;background:none}
.design .testimonial{border:0;border-left:24px solid #c33b2d;padding:32px;text-align:left}
