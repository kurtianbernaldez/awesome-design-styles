...
slug: fluid-poster
family: psychedelic
description: Expressive flowing frames and saturated accents balanced by calm, opaque reading areas.
tags:
  - psychedelic
  - fluid-poster
  - complete-system
...
Expressive flowing frames and saturated accents balanced by calm, opaque reading areas. This is one original interpretation of the psychedelic family, not a definitive rulebook for the entire aesthetic. It is intended for complete websites and application surfaces that benefit from this visual character. Its layout, material choices, and control conventions below define why it exists as a separate system.

Use a centered display introduction followed by a contained project frame and two-column features. Alternate expressive framing with quiet reading sections. Flow comes from large contours and spacing, not moving or overlapping content.
...
| `muted` | `#785379` | Secondary text on surface |
...
Use a large expressive serif display with ordinary sans body and controls. Curved energy comes from framing and scale, not warped glyphs. Never distort essential text or reduce word spacing to imitate a poster.

| Role | Stack | Size / line height | Weight and tracking |
|---|---|---|---|
| Display / H1 | `ui-serif, Georgia, serif` | fluid 36–80px / 1.08 | 700; -0.03em |
| H2 | `ui-serif, Georgia, serif` | 42px / 1.2 | 700; -0.02em |
| H3 | `ui-serif, Georgia, serif` | 27px / 1.3 | 650; normal |
...
The outer container is at most 1180px wide, with 36px gutters and 32px gaps. Use CSS grid with `minmax(0, 1fr)` tracks so long content cannot enlarge the page. Use flex wrapping for action groups and navigation. Cards grow with content; do not enforce fixed text heights. Comparable pricing keeps the same benefit order and aligned actions even if decorative feature blocks have unequal spans.

## Surfaces, Borders, Radius, Shadows, and Depth

Use opaque warm reading planes with purple outlines and broad asymmetric corner pairs. A static saturated margin may frame a section. Keep patterns away from text and never use optical vibration as a background effect.

Reference card boundary: 2px where a frame is used; reference corner radius: 32px. Surface shadow: `8px 8px 0 #b77e9e`. Controls generally use 12px corners unless their component treatment specifies a more precise shape. Content stays at layer 0, navigation at 10 when sticky, menus at 20, backdrop at 40, dialog at 50, and transient feedback at 60. Avoid creating stacking contexts on every card. Overlays must remain above all decorative treatments.

## Components

Primary actions are solid purple with white labels and modest rounded corners. Secondary controls use clear outlined surfaces. Inputs are plain opaque rectangles; never warp their boundaries. Cards use broad organic outer corners and stable interiors. Pricing stays equally aligned. Navigation is simple text, tabs use a clear filled selection, and dialogs discard decorative complexity in favor of a calm opaque frame and visible close control.
...
Use original abstract illustration with bounded color fields. Avoid copied concert posters, illegible lettering, and repetitive high-frequency patterns. Charts must remain flat and accurate with ordinary axes, direct labels, and data tables; never bend scales for visual flow.
...
Use 180ms color changes only. No rotating patterns, pulsing saturation, zoom tunnels, or flashing complementary colors. Reduced motion makes states immediate.
...
```css
.design-psychedelic-fluid-poster {
  --canvas: #efe0c7;
  --surface: #fff1da;
  --ink: #4b294d;
  --muted: #785379;
...
.design-psychedelic-fluid-poster * { box-sizing: border-box; }
```
...
- Put expression in framing.
- keep text and data calm.
- use low-frequency static decoration.
- Preserve the readable control and feedback behavior when adapting the visual composition.

## Don't

- Do not warp words, create vibrating patterns, or animate the entire environment.
