---
name: Civic Infrastructure Solarpunk
slug: civic-infrastructure
family: solarpunk
description: "A public-service interpretation focused on legible shared systems, ecological data, and durable civic interfaces rather than pastoral ornament."
tags:
  - solarpunk
...
# Civic Infrastructure Solarpunk

## Overview and Design Philosophy

A public-service interpretation focused on legible shared systems, ecological data, and durable civic interfaces rather than pastoral ornament.
...
Use a compact 4:5 opening and a 48px medium sans heading. Give statistics and workspace rows a measured civic-report rhythm. Feature rows pair a narrow index with clear explanations.

The distinction from **Community Garden Solarpunk** is structural: that interpretation follows this direction: Center a 64px serif title above a wide communal workspace. Use spacious two-column feature beds with one full-width introduction. Keep labels conversational and reading groups generous. Choose this variant when the composition above suits the content. A palette substitution alone does not establish either interpretation.

## Visual Character

Use chalk, mineral green, thin structural rules, and restrained wood-toned accents. Geometry is mostly rectangular with modest 8px corners; nature is contextual rather than a frame around every control.
...
- Actions are durable rectangular signs. Tabs use a solid green baseline. Forms have clearly grouped service fields; dialogs are opaque civic information sheets.
...
Crop the original garden image into a narrow contextual strip. Use accessible charts and labeled infrastructure diagrams, with explicit units and data sources.
...
Use 120ms state changes and stable data updates.
...
- **Active / pressed:** Use 120ms state changes and stable data updates.
...
.design .hero{display:grid;grid-template-columns:4fr 5fr;gap:32px;align-items:start;padding:32px 0}.design .hero h1{font:600 48px/1.1 system-ui,sans-serif}
.design .panel{border:1px solid #779076;border-radius:8px;background:#f3f3e5;box-shadow:none;padding:24px}
.design .style-artwork{height:100px;border-radius:4px;background-position:center 65%}.design .features{grid-template-columns:1fr;gap:20px}
.design .feature-card:nth-child(n){display:grid;grid-template-columns:100px 1fr;gap:8px 20px}.design .feature-index{grid-row:1/4;font:600 13px/1.5 ui-monospace,monospace}.design .feature-card h3{margin:0}
.design .stats{border-block:2px solid #779076;padding:24px 0;background:none}.design .stats strong{font:600 34px/1.2 system-ui,sans-serif}
.design :is(button,input,select,textarea){border-radius:6px}.design .testimonial{text-align:left;font:20px/1.6 system-ui,sans-serif;border-left:4px solid #779076;padding:24px}
.design .testimonial blockquote{font:20px/1.6 system-ui,sans-serif}
