---
name: Production Bulletin Constructivism
slug: production-bulletin
family: constructivism
description: "A utilitarian bulletin interpretation where compressed headings, serial indexes, and strong rules organize operational information rather than a dramatic poster."
tags:
  - constructivism
  - production-bulletin
  - complete-system
...
# Production Bulletin Constructivism

## Overview and Design Philosophy

A utilitarian bulletin interpretation where compressed headings, serial indexes, and strong rules organize operational information rather than a dramatic poster.
...
Use a compact 4:5 opening and a 50px condensed heading. Align workspace rows and feature indexes to shared rails. Reading groups form a stacked bulletin with clear column labels.

The distinction from **Diagonal Poster Constructivism** is structural: that interpretation follows this direction: Place an 100px condensed headline in a broad opening. Offset the workspace below and to the right. Diagonal bands cross empty margins; feature blocks step in scale without rotating their text. Choose this variant when the composition above suits the content. A palette substitution alone does not establish either interpretation.

## Visual Character

Use cream paper, graphite text, red serial markers, and heavy horizontal rules. Avoid diagonal decoration; energy comes from compression and repetition.
...
- Actions resemble stamped work orders. Tabs are compact labeled index cells. Form fields use untextured paper with strong boundaries; dialogs resemble a single task bulletin.
...
Use original technical photographs, simple process diagrams, and explicit captions. Numbering must correspond to actual content order.
...
Use immediate state changes; asynchronous status includes text and a stable progress position.
...
- **Active / pressed:** Use immediate state changes; asynchronous status includes text and a stable progress position.
...
.design .hero{display:grid;grid-template-columns:4fr 5fr;gap:32px;align-items:start;padding:32px 0}.design .hero h1{font:700 50px/1 'Barlow Condensed',Impact,sans-serif;text-transform:uppercase}
.design .hero-copy{border-top:8px solid;padding-top:24px}.design .project-card{border:2px solid;border-radius:0;transform:none;box-shadow:none;background:var(--surface);padding:24px}
.design .project-rows>div{border-bottom:2px solid;padding:16px 0}.design .features{grid-template-columns:1fr;gap:0}
.design .feature-card:nth-child(n){display:grid;grid-template-columns:96px 1fr;gap:8px 20px;border:0;border-top:4px solid;padding:24px 0;transform:none;background:none}
.design .feature-index{grid-row:1/4;font-weight:800;color:#a22b24}.design .feature-card h3{margin:0}
.design .stats{border-block:4px solid;padding:24px 0}.design .stats strong{font-size:36px}
.design .testimonial{text-align:left;border:2px solid;padding:24px;font-size:22px}
.design .testimonial blockquote{font-size:22px}.design .demo-form{border:2px solid;padding:24px}
