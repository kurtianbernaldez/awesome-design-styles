...
family: grunge
description: Rough-edged editorial framing and forceful type around clean, usable content areas.
tags:
  - grunge
  - weathered-print
  - complete-system
...
Rough-edged editorial framing and forceful type around clean, usable content areas. This is one original interpretation of the grunge family, not a definitive rulebook for the entire aesthetic. It is intended for complete websites and application surfaces that benefit from this visual character. Its layout, material choices, and control conventions below define why it exists as a separate system.

Use a 2:1 hero and two-column features with varied but aligned section rules. The headline is forceful; body stays orderly. A quote can read like a pasted print block without rotation or overlapping content.
...
- Keep distress outside information.
...
| `muted` | `#6a5d4e` | Secondary text on surface |
...
Use heavy display headings and ordinary sans body. Distress is not applied to text glyphs. Uppercase is limited to short headings; controls remain sentence case and legible.

| Role | Stack | Size / line height | Weight and tracking |
|---|---|---|---|
| Display / H1 | `Impact, Haettenschweiler, sans-serif` | fluid 36–80px / 1.08 | 900; -0.03em |
| H2 | `Impact, Haettenschweiler, sans-serif` | 42px / 1.2 | 900; -0.02em |
...
The outer container is at most 1180px wide, with 28px gutters and 24px gaps. Use CSS grid with `minmax(0, 1fr)` tracks so long content cannot enlarge the page. Use flex wrapping for action groups and navigation. Cards grow with content; do not enforce fixed text heights. Comparable pricing keeps the same benefit order and aligned actions even if decorative feature blocks have unequal spans.

## Surfaces, Borders, Radius, Shadows, and Depth

Use warm paper planes and uneven-looking border treatments such as dashed secondary rules or stacked lines. Any grain is decorative and outside reading areas. Opaque clean fields are mandatory; never distress controls or reduce text contrast.

Reference card boundary: 2px where a frame is used; reference corner radius: 0px. Surface shadow: `4px 4px 0 #8b7d68`. Controls generally use 0px corners unless their component treatment specifies a more precise shape. Content stays at layer 0, navigation at 10 when sticky, menus at 20, backdrop at 40, dialog at 50, and transient feedback at 60. Avoid creating stacking contexts on every card. Overlays must remain above all decorative treatments.

## Components

Primary actions are earthy brown with white labels and a hard edge. Secondary controls are clean paper rectangles with strong borders. Forms have pristine opaque interiors, clear errors, and normal native behavior. Cards use rough outer rules but stable padding. Pricing is equally aligned. Navigation uses a heavy masthead rule; tabs use a solid selection marker. Menus and dialogs remove decorative distress and keep a plain dark frame.
...
Use original scanned textures or documentary imagery only in non-text margins or separate crops. Avoid copied music posters and distressed brand marks. Icons remain crisp. Charts use clean plotting areas with direct labels and data tables; noise must not encode or obscure values.
...
Use immediate states or 80ms color changes. No shaking, flickering film grain, or aggressive entry motion. Reduced motion is identical to the static default.
...
.design-grunge-weathered-print * { box-sizing: border-box; }
```
