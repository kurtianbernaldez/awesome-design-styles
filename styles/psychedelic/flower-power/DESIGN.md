---
name: Flower Power Psychedelic
slug: flower-power
family: psychedelic
description: "A warm organic interpretation using swollen letterforms, petal-like containers, and flat interlocking color fields instead of optical monochrome geometry."
tags:
  - psychedelic
  - flower-power
  - complete-system
...
A warm organic interpretation using swollen letterforms, petal-like containers, and flat interlocking color fields instead of optical monochrome geometry.
...
Use a broad 76px heavy serif heading beside a rounded workspace. Features form two large organically framed sections, with generous text interiors and repeated curved edges.

The distinction from **Op Art Psychedelic** is structural: that interpretation follows this direction: Use a centered 80px condensed title inside a broad optical border. Keep the workspace and all text on plain opaque planes. Feature sections use a measured two-column grid. Choose this variant when the composition above suits the content. A palette substitution alone does not establish either interpretation.

## Visual Character

Use warm orange, plum, butter, and pale rose in flat fields. Broad asymmetrical radii suggest organic growth; avoid gradients that wash out text.
...
- Actions are thick rounded labels. Tabs are distinct petal-like keys. Inputs remain simple pale wells, and dialogs use one large organic frame with a plain interior.
...
| Token | Value | Role |
|---|---|---|
| `canvas` | `#efe0c7` | Page environment |
| `surface` | `#fff1da` | Opaque reading surface |
| `ink` | `#4b294d` | Primary text on surface |
| `muted` | `#6c4a6d` | Secondary text on surface |
| `accent` | `#794584` | Action and selection cue |
| `on-accent` | `#ffffff` | Text on filled accent |
| `line` | `#96729a` | Grouping rules |
| `control-line` | `#96729a` | Control boundary |
...
Use original abstract floral illustration in margins. Avoid decorative flowers covering labels or controls.
...
Use 160ms color changes; no pulsing forms or swirling background animation.
...
**Pointer policy: none.** The flower and color composition already provides an expressive static experience.
...
- **Active / pressed:** Use 160ms color changes; no pulsing forms or swirling background animation.
...
.design .hero{display:grid;grid-template-columns:3fr 2fr;gap:40px;align-items:start}.design .hero h1{font:800 76px/1.02 Georgia,serif;letter-spacing:-.06em;text-shadow:3px 3px #dfb068;transform:none}
.design .hero-copy{background:#edc9a4;border-radius:60px 20px;padding:32px}.design .project-card{margin-top:40px;background:#f3dfb0;border:4px solid #715070;border-radius:60px 24px 52px 20px;box-shadow:none;transform:none}
.design .features{grid-template-columns:1fr 1fr;gap:36px}.design .feature-card:nth-child(n){background:#e4cadc;border:4px solid #715070;border-radius:48px 16px;box-shadow:none;transform:none;padding:32px}
.design .feature-card:first-child{grid-column:1/-1;background:#f3dfb0}.design .feature-index{font:700 18px/1.5 Georgia,serif}
.design .stats{background:#edc9a4;border:4px solid #715070;border-radius:48px 20px;padding:28px}.design .stats strong{font:800 48px/1.2 Georgia,serif}
.design .demo-tabs button{border-radius:28px 8px}.design .testimonial{border:0;font:italic 28px/1.6 Georgia,serif}
.design .testimonial blockquote{font:italic 28px/1.6 Georgia,serif}
.design .demo-form{background:#f3dfb0;border:4px solid #715070;border-radius:48px 20px;box-shadow:none}
