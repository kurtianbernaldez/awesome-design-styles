---
name: Toy Workshop Claymorphism
slug: toy-workshop
family: claymorphism
description: "A playful construction-set interpretation that treats controls as chunky separate pieces and emphasizes compact, colorful task modules."
tags:
  - claymorphism
  - toy-workshop
  - complete-system
...
# Toy Workshop Claymorphism

## Overview and Design Philosophy

A playful construction-set interpretation that treats controls as chunky separate pieces and emphasizes compact, colorful task modules.
...
Use a 1:1 opening with a heavy 64px rounded sans title. The workspace is assembled from visibly separate colored task pieces. Features become a compact board with large numbered tiles.

The distinction from **Ceramic Studio Claymorphism** is structural: that interpretation follows this direction: Use a 3:2 opening with a regular 66px serif title. The workspace becomes a wide shallow vessel. Feature groups use one generous column with a sculpted outer edge, leaving broad space between reading groups. Choose this variant when the composition above suits the content. A palette substitution alone does not establish either interpretation.

## Visual Character

Keep the inflated inner highlight and lower inner shade, but vary component color deliberately: peach, mint, butter, and lavender pieces sit on a pale backdrop. Use 20–24px rounded-square forms instead of one oversized pillow.
...
- Buttons are thick toy keys with clear labels. Selected tools compress into their place. Tabs are separate molded keys; form fields sit inside shallow cutouts. Avoid giving passive badges a button-like cast shadow.
...
| Token | Value | Role |
|---|---|---|
| `canvas` | `#eee4f4` | Page environment |
| `surface` | `#fbf2ff` | Opaque reading surface |
| `ink` | `#3d2b50` | Primary text on surface |
| `muted` | `#69517c` | Secondary text on surface |
| `accent` | `#74508b` | Action and selection cue |
| `on-accent` | `#ffffff` | Text on filled accent |
| `line` | `#91779f` | Grouping rules |
| `control-line` | `#91779f` | Control boundary |
...
Use original matte toy-like objects only when useful. Functional icons should be chunky and simple enough to read at small sizes.
...
Use 120ms compression to .96 scale; reduced motion uses an immediate shadow change.
...
A rounded-square molded highlight steps between toy-sized cells. This is decorative material feedback, not a status indicator.

**Construction.** Use a molded highlight with a 24px corner radius, a white rim, and a softly blurred 5px shadow. Preserve the surrounding toy material and its deeper pressed state. Use this document’s accent and surface tokens, with a 144px-wide region, height equal to width / 1.45, and a 2px base rim.

**Movement and coverage.** React on only the active material surface. Snap local coordinates to a 8px grid. Measure coordinates relative to that surface, keep the reflection behind its content, and clip it to the material radius. Leave page margins, section gaps, and unrelated reading regions still. The material host, labels, and hit targets never tilt or move.
...
- **Active / pressed:** Use 120ms compression to .96 scale; reduced motion uses an immediate shadow change.
...
.design .hero{display:grid;grid-template-columns:1fr 1fr;gap:36px;padding:40px 0}.design .hero h1{font-size:64px;font-weight:900}
.design .project-card{background:#e5daf4;border-radius:24px;padding:28px}
.design .project-rows>div{border:0;border-radius:16px;background:#f3d7ca;padding:14px;margin:16px 0;box-shadow:inset 4px 4px 7px #fff9,inset -4px -4px 8px #ad7e6955,4px 5px 8px #6b487933}
.design .project-rows>div:nth-child(2){background:#d5e8e5}.design .project-rows>div:nth-child(3){background:#f2e3b9}
.design .features{grid-template-columns:1fr 1fr;gap:24px}.design .feature-card:nth-child(n){border-radius:24px}
.design .feature-index{display:grid;place-items:center;width:88px;min-height:52px;background:#f3d7ca;border-radius:16px;box-shadow:inset 3px 3px 5px #fff9,inset -3px -3px 6px #ad7e6955}
.design .primary{border-radius:16px;padding:16px 24px}.design .demo-tabs{border:0;gap:16px;padding:12px}
.design .demo-tabs button{border-radius:16px;background:#f2e3b9}.design .demo-tabs button[aria-selected=true]{background:#d5e8e5;box-shadow:inset 3px 4px 8px #51797155}
.design .stats{gap:24px}.design .stats>div{background:#f2e3b9;border-radius:20px;padding:24px;box-shadow:inset 4px 4px 8px #fff9,inset -4px -4px 8px #aa925b44}
