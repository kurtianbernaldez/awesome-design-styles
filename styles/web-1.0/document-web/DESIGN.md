...
slug: document-web
family: web-1.0
description: A direct link-led document aesthetic with exposed rules and readable native controls.
tags:
  - web-1.0
  - document-web
  - complete-system
...
A direct link-led document aesthetic with exposed rules and readable native controls. This is one original interpretation of the web 1.0 family, not a definitive rulebook for the entire aesthetic. It is intended for complete websites and application surfaces that benefit from this visual character. Its layout, material choices, and control conventions below define why it exists as a separate system.

Use a single main reading column for the opening and two simple feature columns below. Keep all navigation visible as a wrapping link list. Tables are used for data only, never layout. Content adapts to the viewport rather than imitating a fixed historical canvas.
...
- Prioritize links and document hierarchy.
...
Use serif document headings and body with sans controls. Body remains 17px and comfortable to read. Native-looking does not mean tiny default text or poorly scaled heading jumps.

| Role | Stack | Size / line height | Weight and tracking |
|---|---|---|---|
| Display / H1 | `ui-serif, Georgia, serif` | fluid 36–48px / 1.08 | 700; -0.03em |
| H2 | `ui-serif, Georgia, serif` | 30px / 1.2 | 700; -0.02em |
| H3 | `ui-serif, Georgia, serif` | 23px / 1.3 | 650; normal |
| Body | `ui-serif, Georgia, serif` | 17px / 1.65 | 400; normal |
...
The outer container is at most 1080px wide, with 24px gutters and 20px gaps. Use CSS grid with `minmax(0, 1fr)` tracks so long content cannot enlarge the page. Use flex wrapping for action groups and navigation. Cards grow with content; do not enforce fixed text heights. Comparable pricing keeps the same benefit order and aligned actions even if decorative feature blocks have unequal spans.

## Surfaces, Borders, Radius, Shadows, and Depth

Use a plain light page, visible horizontal rules, and opaque rectangular fields. No gradients, shadows, pills, or decorative image buttons. The character comes from document structure and links, not inaccessible nostalgia.

Reference card boundary: 1px where a frame is used; reference corner radius: 0px. Surface shadow: `none`. Controls generally use 0px corners unless their component treatment specifies a more precise shape. Content stays at layer 0, navigation at 10 when sticky, menus at 20, backdrop at 40, dialog at 50, and transient feedback at 60. Avoid creating stacking contexts on every card. Overlays must remain above all decorative treatments.

## Components

Primary buttons are solid blue rectangles with white labels; secondary controls are plain outlined native-like buttons. Links are underlined at rest. Fields use ordinary labels, borders, and native validation. Cards are open document sections with headings and rules. Pricing can be a simple equal-column comparison. Tabs are clear rectangular choices. Menus and dialogs use plain cream frames and modern keyboard/focus behavior despite their simple appearance.
...
Use original small illustrations or photographs only when informative. Never copy old badges, browser logos, or animated GIF identities. Alt text and dimensions are required. Charts remain simple labeled plots with table alternatives; do not use images of tables.
...
No animation, marquees, flashing text, or animated banners. Loading uses a written status. Reduced motion is naturally satisfied by the static presentation.
...
.design-web-1.0-document-web * { box-sizing: border-box; }
```
...
- Do not reproduce inaccessible fixed-width layouts, blinking text, or image-only navigation.
