---
name: Document Web 1.0
slug: document-web
family: web-1.0
description: "A linked document with ordinary browser-like typography, horizontal rules, and minimal visual machinery."
tags:
  - web-1.0
  - document-web
  - complete-system
...
Use a single flowing opening, a 36px serif H1, and a document-wide text measure constrained only for readability. Navigation behaves like a short index. The workspace follows the introduction; features are stacked sections, not equal marketing cards.
...
- Use a plain light document canvas, no shadows or panel containers, and groove-like horizontal rules.
- Let the control construction explain state and hierarchy.
- Do not add a SaaS split hero, three rounded feature cards, or novelty errors to simulate age.
...
| Token | Value | Role |
|---|---|---|
| `canvas` | `#f1eee1` | Page environment; ink text |
| `surface` | `#fffbee` | Opaque reading and control plane |
| `ink` | `#30372d` | Main text on canvas or surface |
| `muted` | `#606958` | Secondary text on surface |
| `accent` | `#344f99` | Primary action and selected state |
| `on-accent` | `#ffffff` | Text on the accent fill |
| `line` | `#7c8772` | Grouping rules |
| `control-line` | `#7c8772` | Visible input and control boundaries |
...
Use a plain light document canvas, no shadows or panel containers, and groove-like horizontal rules. A compact statistics table-like region may have a simple frame. The lack of decorative UI is intentional.
...
|---|---|---|
| Display | `ui-serif, Georgia, serif` | Use the display scale and weight described above; 34–40px on phones unless specified below |
| Section heading | `ui-serif, Georgia, serif` | 32px / 1.2; 28px on small screens |
| Card heading | `ui-serif, Georgia, serif` | 22px / 1.3; wrap naturally |
...
Use 4/8/12/16/20/32/48/56px. Keep content groups compact and separate sections with clear horizontal rules. Avoid large decorative hero whitespace.
...
Links are persistently underlined with a distinct visited color. Buttons are small conventional gray outset controls with 44px hit areas. Forms use native semantics and obvious labels. Tabs and dialogs remain usable modern controls inside the document aesthetic.
...
Use only necessary inline images, figures, and captions. Do not copy historical site text or add fake broken-image icons and decorative counters.
...
Charts remain simple labeled plots with table alternatives; do not use images of tables.
...
- **Active / pressed:** Immediate state changes; no animation.
...
Background reference: [CERN: first web document](https://info.cern.ch/hypertext/WWW/TheProject.html). The rules in this document are an original interface interpretation, not a reproduction of a source artifact or a claim that this variant exhausts the family.
