---
name: Diagonal Poster Constructivism
slug: diagonal-poster
family: constructivism
description: "A poster-led interpretation using diagonal compositional tension and enormous compressed type, while keeping the actual reading and keyboard order straight."
tags:
  - constructivism
  - diagonal-poster
  - complete-system
...
# Diagonal Poster Constructivism

## Overview and Design Philosophy

A poster-led interpretation using diagonal compositional tension and enormous compressed type, while keeping the actual reading and keyboard order straight.
...
Place an 100px condensed headline in a broad opening. Offset the workspace below and to the right. Diagonal bands cross empty margins; feature blocks step in scale without rotating their text.

The distinction from **Production Bulletin Constructivism** is structural: that interpretation follows this direction: Use a compact 4:5 opening and a 50px condensed heading. Align workspace rows and feature indexes to shared rails. Reading groups form a stacked bulletin with clear column labels. Choose this variant when the composition above suits the content. A palette substitution alone does not establish either interpretation.

## Visual Character

Use paper, black, and red planes with hard edges. A clipped red corner and diagonal stripe provide directional energy. Content surfaces remain opaque and square.
...
- Actions are black rectangular labels with hard red offsets. Selected tabs have a thick red rule. Forms and dialogs use square paper sheets without decorative rotation.
...
| Token | Value | Role |
|---|---|---|
| `canvas` | `#eee3d0` | Page environment |
| `surface` | `#fff5e4` | Opaque reading surface |
| `ink` | `#322b28` | Primary text on surface |
| `muted` | `#665a50` | Secondary text on surface |
| `accent` | `#a23527` | Action and selection cue |
| `on-accent` | `#ffffff` | Text on filled accent |
| `line` | `#71645a` | Grouping rules |
| `control-line` | `#71645a` | Control boundary |
...
| Section heading | `'Barlow Condensed','Arial Narrow',sans-serif` | 44px / 1.2; 28px on phones |
...
Use a 4px base with 8/12/16/24/32/48/64/96px intervals. The wide-screen section interval is 88px; the default module gap is 24px and outer gutter is 32px. Follow the role-specific interiors in the construction rules rather than forcing every region to have identical padding.
...
Use original photomontage-style crops or industrial detail. Never use political propaganda text or borrowed historical slogans as interface content.
...
Use immediate stamp-like selection and no continuously moving diagonals.
...
**Pointer policy: none.** The directional print composition already establishes movement through static geometry.
...
- **Active / pressed:** Use immediate stamp-like selection and no continuously moving diagonals.
...
.design .hero{display:block;padding:36px 0;background:linear-gradient(155deg,transparent 68%,#b62c2520 68% 75%,transparent 75%)}
.design .hero h1{font:700 100px/.92 'Barlow Condensed',Impact,sans-serif;text-transform:uppercase;max-width:12ch}
.design .hero-copy{border-left:16px solid #b62c25;padding-left:24px}.design .project-card{margin:40px 0 0 28%;transform:none;border:3px solid;border-top:24px solid #b62c25;background:var(--surface)}
.design .features{grid-template-columns:2fr 1fr;gap:24px}.design .feature-card:nth-child(n){transform:none;border:3px solid;background:var(--surface)}
.design .feature-card:first-child{grid-row:span 2;border-left:20px solid #b62c25}.design .feature-index{font-size:22px;font-weight:800}
.design .stats strong{font-family:'Barlow Condensed',Impact,sans-serif;font-size:64px}.design .primary{box-shadow:5px 5px #b62c25}
.design .testimonial{text-align:left;border-left:16px solid #b62c25;padding:32px;font-weight:700}
.design .testimonial blockquote{font-weight:700}.design .demo-form{border:3px solid;border-top:20px solid #b62c25}
.design .hero-copy{transform:none}
