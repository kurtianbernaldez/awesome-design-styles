---
name: Mall Directory Vaporwave
slug: mall-directory
family: vaporwave
description: "A spatial nostalgia interpretation using monumental signage, tiled public-space geometry, and pastel directory bands instead of desktop windows."
tags:
  - vaporwave
  - mall-directory
  - complete-system
...
# Mall Directory Vaporwave

## Overview and Design Philosophy

A spatial nostalgia interpretation using monumental signage, tiled public-space geometry, and pastel directory bands instead of desktop windows.
...
Center a widely tracked 72px title above a broad directory panel. Feature entries form a full-width index with large ordinal rails. Preserve large empty plazas between sections.

The distinction from **Pastel Desktop Vaporwave** is structural: that interpretation follows this direction: Use a compact 4:5 opening with a 54px serif title. Workspace and features become layered desktop windows with restrained title strips and generous text interiors. Choose this variant when the composition above suits the content. A palette substitution alone does not establish either interpretation.

## Visual Character

Use pale stone, pink and aqua tile bands, thin grid lines, and flat dark lettering. Avoid beveled window chrome; surfaces resemble architectural signage.
...
- Actions are long directory plaques. Selected tabs have a dark baseline. Inputs and dialogs use plain pale slabs with a tile-colored edge.
...
Use original empty architectural scenes or abstract tiled perspectives. Essential navigation must remain live HTML, not signage baked into an image.
...
Use 180ms color changes and no simulated camera drift.
...
**Pointer policy: none.** Directory labels and their fixed spatial arrangement should remain the focal point.
...
- **Active / pressed:** Use 180ms color changes and no simulated camera drift.
...
.design .hero{display:block;padding:56px 0;border-block:12px solid #b9d9db}.design .hero-copy{text-align:center;max-width:800px;margin:auto}
.design .hero h1{font:400 72px/1.1 Georgia,serif;letter-spacing:.08em;text-transform:uppercase;text-shadow:none}.design .hero-copy p{margin-inline:auto}.design .actions{justify-content:center}
.design .project-card{max-width:840px;margin:44px auto 0;border:2px solid #9c86af;border-radius:0;box-shadow:none;background:#eee5ed;border-top:20px solid #d3acc4}
.design .features{grid-template-columns:1fr;gap:0}.design .feature-card:nth-child(n){display:grid;grid-template-columns:120px 1fr;gap:8px 24px;border:0;border-bottom:2px solid #9c86af;background:none;padding:32px 0}
.design .feature-index{grid-row:1/4;font-size:18px;letter-spacing:.1em}.design .feature-card h3{margin:0}
.design .stats{border-block:12px solid #b9d9db;padding:32px;background:#eee5ed}.design .stats strong{font:400 48px/1.2 Georgia,serif}
.design .testimonial{border:0;font:italic 28px/1.6 Georgia,serif}
.design .testimonial blockquote{font:italic 28px/1.6 Georgia,serif}.design .demo-form{border:2px solid #9c86af;border-top:20px solid #d3acc4;border-radius:0;box-shadow:none;background:#eee5ed}
.design .feature-card:nth-child(n){transform:none}
.design body{background:linear-gradient(#ffffff44 1px,transparent 1px),linear-gradient(90deg,#ffffff44 1px,transparent 1px),#e2d4e2;background-size:48px 48px}.design .card-top{background:none;margin:0 0 20px;padding:0;border-bottom:2px solid #9c86af}.design .card-top .eyebrow{color:var(--muted)}.design .hero::before{display:none}.design .feature-card:nth-child(n){box-shadow:none}
