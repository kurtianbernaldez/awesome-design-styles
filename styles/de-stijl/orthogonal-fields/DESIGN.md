---
name: Orthogonal Fields De Stijl
slug: orthogonal-fields
family: de-stijl
description: Strict horizontal and vertical divisions with purposeful colored fields and an ordered reading path.
tags:
  - de-stijl
  - orthogonal-fields
  - complete-system
...
# Orthogonal Fields De Stijl

## Overview and Design Philosophy

Strict horizontal and vertical divisions with purposeful colored fields and an ordered reading path. This is one original interpretation of the de stijl family, not a definitive rulebook for the entire aesthetic. It is intended for complete websites and application surfaces that benefit from this visual character. Its layout, material choices, and control conventions below define why it exists as a separate system.

Use a 2:1 hero and asymmetric feature spans of 1:2:1. Align all boundaries horizontally and vertically. Keep a single logical source order despite unequal visual spans. Avoid recreating any recognizable existing painting or composition.
...
- Let content determine field size.
...
| Token | Value | Role |
|---|---|---|
| `canvas` | `#f2f0e7` | Page environment; ink text |
| `surface` | `#fffdf5` | Opaque reading and control plane |
| `ink` | `#272c31` | Main text on canvas or surface |
| `muted` | `#535d67` | Secondary text on surface |
| `accent` | `#a8372d` | Primary action and selected state |
| `on-accent` | `#ffffff` | Text on the accent fill |
| `line` | `#272c31` | Grouping rules |
| `control-line` | `#272c31` | Visible input and control boundaries |
...
Use plain sans-serif type with a limited 400/600/700 range. Headings remain horizontal and sentence case. Color fields and grid proportions create identity; lettering should not become another competing shape.

| Role | Stack | Size / line height | Weight and tracking |
|---|---|---|---|
| Display / H1 | `system-ui, sans-serif` | fluid 36–64px / 1.08 | 700; -0.03em |
| H2 | `system-ui, sans-serif` | 36px / 1.2 | 700; -0.02em |
| H3 | `system-ui, sans-serif` | 24px / 1.3 | 650; normal |
...
Use 8/16/24/32/48/80px. Internal field padding is 24px. Dark divisions are part of the structure and need no extra decorative outline.
...
The outer container is at most 1200px wide, with 32px gutters and 24px gaps. Use CSS grid with `minmax(0, 1fr)` tracks so long content cannot enlarge the page. Use flex wrapping for action groups and navigation. Cards grow with content; do not enforce fixed text heights. Comparable pricing keeps the same benefit order and aligned actions even if decorative feature blocks have unequal spans.

## Surfaces, Borders, Radius, Shadows, and Depth

Use opaque cream, restrained red, pale yellow, and blue-tinted fields divided by dark orthogonal rules. No radius, diagonals, gradients, or shadows. Every color field contains a clear content group or remains deliberately empty.

Reference card boundary: 3px where a frame is used; reference corner radius: 0px. Surface shadow: `none`. Controls generally use 0px corners unless their component treatment specifies a more precise shape. Content stays at layer 0, navigation at 10 when sticky, menus at 20, backdrop at 40, dialog at 50, and transient feedback at 60. Avoid creating stacking contexts on every card. Overlays must remain above all decorative treatments.

## Components

Primary actions are red rectangles with white labels; secondary actions use cream and dark frames. Inputs use the same orthogonal boundaries and persistent labels. Feature cards can occupy distinct flat fields; pricing uses equal-width cream panels for comparison. Navigation is a strongly divided strip. Tabs are rectangular cells with a selected lower rule. Menus and dialogs use plain cream, strong dark edges, and no colored distraction around task content.
...
Use sparse rectangular photos or original explanatory geometric diagrams. Decorative fields are not clickable unless they contain a real action. Charts use rectangular bars, accurate scales, dark labels, and patterns for series; never use field area decoratively in a way that implies data. Include tabular alternatives.
...
Charts use rectangular bars, accurate scales, dark labels, and patterns for series; never use field area decoratively in a way that implies data. Include tabular alternatives.
...
Use immediate state changes, with no translation or animated rearrangement of color fields. Loading is written progress. Reduced-motion behavior is identical to the default.
...
```css
.design-de-stijl-orthogonal-fields {
  --canvas: #f2f0e7;
  --surface: #fffdf5;
  --ink: #272c31;
  --muted: #535d67;
...
.design-de-stijl-orthogonal-fields * { box-sizing: border-box; }
```
...
- Let content determine field size.
- keep divisions orthogonal.
- maintain a linear reading path.
- Preserve the readable control and feedback behavior when adapting the visual composition.

## Don't

- Do not copy a recognizable composition, rotate text, or make every field a separate action.
