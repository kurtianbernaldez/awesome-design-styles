...
slug: graphic-impact
family: pop-art
description: Bold outlines and bright flat emphasis with an unmistakable hierarchy and clear practical controls.
tags:
  - pop-art
  - graphic-impact
  - complete-system
...
Bold outlines and bright flat emphasis with an unmistakable hierarchy and clear practical controls. This is one original interpretation of the pop art family, not a definitive rulebook for the entire aesthetic. It is intended for complete websites and application surfaces that benefit from this visual character. Its layout, material choices, and control conventions below define why it exists as a separate system.

Use a 2:1 hero and three equal feature blocks. One large declarative headline creates impact; supporting details stay quiet. Keep pricing and forms conventional rather than turning every section into a comic panel.
...
- Use a single emphatic focal point.
...
| `muted` | `#685870` | Secondary text on surface |
...
Use heavy sans display headings with plain body text. Short uppercase titles can be emphatic; paragraphs and controls remain sentence case. Avoid comic lettering that sacrifices legibility.

| Role | Stack | Size / line height | Weight and tracking |
|---|---|---|---|
| Display / H1 | `system-ui, sans-serif` | fluid 36–76px / 1.08 | 900; -0.03em |
| H2 | `system-ui, sans-serif` | 40px / 1.2 | 900; -0.02em |
| H3 | `system-ui, sans-serif` | 26px / 1.3 | 650; normal |
...
The outer container is at most 1180px wide, with 32px gutters and 28px gaps. Use CSS grid with `minmax(0, 1fr)` tracks so long content cannot enlarge the page. Use flex wrapping for action groups and navigation. Cards grow with content; do not enforce fixed text heights. Comparable pricing keeps the same benefit order and aligned actions even if decorative feature blocks have unequal spans.

## Surfaces, Borders, Radius, Shadows, and Depth

Use warm white reading panels, dark 3px outlines, and selected flat red, pale yellow, or blue fields. Halftone-like dots may occupy an empty edge band only. No patterned text backgrounds or speech-bubble hit areas.

Reference card boundary: 3px where a frame is used; reference corner radius: 0px. Surface shadow: `6px 6px 0 #352d41`. Controls generally use 0px corners unless their component treatment specifies a more precise shape. Content stays at layer 0, navigation at 10 when sticky, menus at 20, backdrop at 40, dialog at 50, and transient feedback at 60. Avoid creating stacking contexts on every card. Overlays must remain above all decorative treatments.

## Components

Primary buttons are deep red with white labels and a dark outline; secondary controls are cream with a strong frame. Inputs remain clean and opaque. Feature cards have flat fields and a clear headline/body/action sequence. Recommended pricing uses an explicit badge, not a covering starburst. Navigation has a heavy lower rule; tabs invert clearly. Menus and dialogs are framed opaque panels with plain task labels and no decorative speech tails.
...
Use original bold illustration or photographs with deliberate crops. Avoid copying famous artworks, comic characters, speech text, or brand packaging. Charts use flat dark-outlined bars, accurate baselines, direct labels, and data tables; dot patterns must not distort perceived values.
...
Use 100ms color changes and immediate press feedback. No flashing bursts, zooming exclamations, or comic-style screen shake. Reduced motion removes all translation.
...
.design-pop-art-graphic-impact * { box-sizing: border-box; }
```
...
- Use a single emphatic focal point.
- keep flat fields readable.
- preserve conventional controls.
- Preserve the readable control and feedback behavior when adapting the visual composition.

## Don't

- Do not copy recognizable artwork or let bursts and dot patterns cover content.
