---
name: Fluid Poster Psychedelic
slug: fluid-poster
family: psychedelic
description: "Optical rhythm and swelling display forms organized into a readable fluid poster."
tags:
  - psychedelic
  - fluid-poster
  - complete-system
...
# Fluid Poster Psychedelic

## Overview and Design Philosophy

Optical rhythm and swelling display forms organized into a readable fluid poster.

Use a two-column opening with a 42px gap, tightly packed 88px italic serif display, and a bold multi-color rhythm band. The workspace has an asymmetric undulating contour. Feature frames alternate their corner rhythm.
...
- Use ochre, plum, coral, and pink with layered hard outlines and visibly varied curvatures.
- Let the control construction explain state and hierarchy.
- Do not reduce the style to a purple palette, or distort all typography until the interface becomes unreadable.
...
| Token | Value | Role |
|---|---|---|
| `canvas` | `#efe0c7` | Page environment; ink text |
| `surface` | `#fff1da` | Opaque reading and control plane |
| `ink` | `#4b294d` | Main text on canvas or surface |
| `muted` | `#614161` | Secondary text on surface |
| `accent` | `#794584` | Primary action and selected state |
| `on-accent` | `#ffffff` | Text on the accent fill |
| `line` | `#96729a` | Grouping rules |
| `control-line` | `#96729a` | Visible input and control boundaries |
...
Use ochre, plum, coral, and pink with layered hard outlines and visibly varied curvatures. Repeating optical bands stay at section edges; reading areas remain opaque. Large type may be expressive, but body text is stable and unwarped.
...
Buttons use lens-like rounded forms with clear labels. Forms retain regular text and fixed boundaries. Tabs distinguish selection without optical animation. Dialogs calm the surrounding pattern with an opaque surface.
...
Use original fluid poster art and nonrepresentational optical rhythm. Avoid copying concert posters or deforming essential text into an image.
...
Charts must remain flat and accurate with ordinary axes, direct labels, and data tables; never bend scales for visual flow.
...
Keep optical patterns static. Use at most 140ms color feedback and honor reduced motion. No strobing or continuous color cycling.
...
**Pointer policy: page.** Bounded organic rings support the expressive fluid composition.

### Liquid color rings

Two broad organic color rings turn around the pointer without animating the page background. This is optional decorative feedback, not a status indicator.

**Construction.** Draw an asymmetric closed curve with alternating broad and narrow bends; use an accent stroke three times the nominal weight and one fine offset ink echo. Nouveau uses fine tracery; psychedelic work uses a 15px stroke. Turn the complete shape gently without changing its topology. Unless a material-specific size above overrides it, use a 96px-wide region, height equal to width / 1, and a 1px base stroke. Sparse ink geometry defaults to 16% paint opacity; minimalist work uses 12%, brutalist work 20%, Art Deco 28%, and Wabi-Sabi 10%. Explicit recipe opacity takes precedence. Apply a further 70% opacity to the page-level geometry; glass uses its explicit light values. Use this document’s accent and ink tokens.

**Movement and coverage.** Follow both pointer coordinates. Derive a bounded rotation from normalized position: 70 × (x / width − 0.5) + 35 × (y / height − 0.5) degrees. Stop moving when input stops. Position the page-level decoration in a fixed, viewport-sized layer: it follows the mouse through the whole page, including margins, the hero, footer, and the gaps between columns. Never clip this layer to a card or grid column.
...
- **Active / pressed:** Keep optical patterns static. Use at most 140ms color feedback and honor reduced motion. No strobing or continuous color cycling.
...
.design body{ background:repeating-radial-gradient(ellipse at 100% 0,#a05c96 0 18px,#df9868 18px 36px,#e4ca74 36px 54px,#755387 54px 72px); }
...
Background reference: [V&A: poster history](https://www.vam.ac.uk/articles/a-short-history-of-the-poster). The rules in this document are an original interface interpretation, not a reproduction of a source artifact or a claim that this variant exhausts the family.
