---
name: Vector Arcade Synthwave
slug: vector-arcade
family: synthwave
description: "A vector-display interpretation using hard grid geometry, indexed panels, and precise neon linework instead of soft sunset nostalgia."
tags:
  - synthwave
  - vector-arcade
  - complete-system
...
# Vector Arcade Synthwave

## Overview and Design Philosophy

A vector-display interpretation using hard grid geometry, indexed panels, and precise neon linework instead of soft sunset nostalgia.
...
Use a compact 4:5 opening and a 56px monospace heading. Workspace rows resemble a vector control board. Features stack with numbered rails and firm rectangular boundaries.

The distinction from **Sunset Radio Synthwave** is structural: that interpretation follows this direction: Center a 72px italic title above a long radio-like workspace. Features form two broad program panels. Keep generous horizontal bands and a clear dial-like control row. Choose this variant when the composition above suits the content. A palette substitution alone does not establish either interpretation.

## Visual Character

Use near-black violet, thin cyan and magenta lines, and sparse perspective grids outside text. Eliminate rounded glossy shells and blurred text glow.
...
- Buttons are outlined vector keys. Selected tabs use a solid bright marker. Inputs and dialogs are opaque dark rectangles with a double-line border.
...
| Token | Value | Role |
|---|---|---|
| `canvas` | `#1e1933` | Page environment |
| `surface` | `#302443` | Opaque reading surface |
| `ink` | `#f7edf9` | Primary text on surface |
| `muted` | `#d0b8da` | Secondary text on surface |
| `accent` | `#edb6db` | Action and selection cue |
| `on-accent` | `#35213e` | Text on filled accent |
| `line` | `#a085af` | Grouping rules |
| `control-line` | `#a085af` | Control boundary |
...
Use a 4px base with 8/12/16/24/32/48/64/96px intervals. The wide-screen section interval is 56px; the default module gap is 28px and outer gutter is 32px. Follow the role-specific interiors in the construction rules rather than forcing every region to have identical padding.
...
Use original wireframe diagrams and geometric game-like scenes. Grid decoration must not create moiré behind paragraphs.
...
Use immediate key feedback and no flashing score or moving grid.
...
- **Active / pressed:** Use immediate key feedback and no flashing score or moving grid.
...
.design .hero{display:grid;grid-template-columns:4fr 5fr;gap:32px;align-items:start}.design .hero h1{font:600 56px/1.05 ui-monospace,monospace;text-shadow:none}
.design .panel{border:3px double var(--control-line);border-radius:0;box-shadow:none;padding:24px;background:var(--surface)}
.design .project-rows>div{border-bottom:1px solid var(--line);font:14px/1.5 ui-monospace,monospace}
...
.design .stats{border:3px double var(--control-line);padding:24px}.design .stats strong{font:500 34px/1.2 ui-monospace,monospace}
.design :is(button,.badge){border-radius:0}.design .testimonial{text-align:left;font:20px/1.6 ui-monospace,monospace;border-left:3px solid var(--accent);padding:24px}
.design .testimonial blockquote{font:20px/1.6 ui-monospace,monospace}
