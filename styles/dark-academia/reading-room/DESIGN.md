...
| Token | Value | Role |
|---|---|---|
| `canvas` | `#262722` | Page environment; ink text |
| `surface` | `#34372e` | Opaque reading and control plane |
| `ink` | `#eee6cb` | Main text on canvas or surface |
| `muted` | `#c3bea6` | Secondary text on surface |
| `accent` | `#dec78d` | Primary action and selected state |
| `on-accent` | `#303327` | Text on the accent fill |
| `line` | `#96957b` | Grouping rules |
| `control-line` | `#96957b` | Visible input and control boundaries |
...
Use 4/8/12/20/28/40/64/96px. Give reading passages generous margins and keep catalog metadata close to its item. Cards use 28px interiors.
...
Charts use clear warm labels and solid/dashed series, with table alternatives; aesthetic mood cannot justify vague values.
