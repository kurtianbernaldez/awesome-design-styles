---
name: Arcade Scoreboard Pixel Art
slug: arcade-scoreboard
family: pixel-art
description: "A public arcade-display interpretation using large block lettering, bright framed score regions, and high-contrast segmented panels."
tags:
  - pixel-art
  - arcade-scoreboard
  - complete-system
...
# Arcade Scoreboard Pixel Art

## Overview and Design Philosophy

A public arcade-display interpretation using large block lettering, bright framed score regions, and high-contrast segmented panels.
...
Center a 52px pixel title above a broad scoreboard-like workspace. Features use two large framed columns with a full-width lead stage. Increase spacing around pixel typography.

The distinction from **Handheld RPG Pixel Art** is structural: that interpretation follows this direction: Use a compact 4:5 opening with a 40px pixel heading. Workspace tasks form inventory rows. Features stack as dialogue panels with a narrow index slot. Choose this variant when the composition above suits the content. A palette substitution alone does not establish either interpretation.

## Visual Character

Use deep navy, pale readable text, bright cyan and amber frame accents, and hard 4px offsets. No blurred neon glow or smooth rounded corners.
...
- Buttons are large block keys with mechanical offsets. Tabs use an inverse highlight. Forms and dialogs are opaque dark panels with a stepped frame.
...
| Token | Value | Role |
|---|---|---|
| `canvas` | `#1c2337` | Page environment |
| `surface` | `#303950` | Opaque reading surface |
| `ink` | `#eff1d7` | Primary text on surface |
| `muted` | `#c5ceb9` | Secondary text on surface |
| `accent` | `#cde889` | Action and selection cue |
| `on-accent` | `#25321c` | Text on filled accent |
| `line` | `#98aa91` | Grouping rules |
| `control-line` | `#98aa91` | Control boundary |
| `error` | `#ffc0ca` | Error text with written explanation |
| `success` | `#b9e5c6` | Success text with written confirmation |
| `warning` | `#f0d799` | Warning text with written next step |
...
Use original pixel graphics and integer scaling. Avoid borrowed game sprites or tiny image-based text.
...
Use immediate key depression, no flashing scores, and no unnecessary animation loops.
...
Large hard scoreboard brackets snap to 24px grid cells. This is optional decorative feedback, not a status indicator.

**Construction.** Construct four separated square brackets with the specified stroke at 50% paint opacity. Leave the central 40% of each edge open. In pixel contexts, keep integer coordinates and square ends. Unless a material-specific size above overrides it, use a 72px-wide region, height equal to width / 0.78, and a 4px base stroke. Sparse ink geometry defaults to 16% paint opacity; minimalist work uses 12%, brutalist work 20%, Art Deco 28%, and Wabi-Sabi 10%. Explicit recipe opacity takes precedence. Apply a further 70% opacity to the page-level geometry; glass uses its explicit light values. Use this document’s accent and ink tokens.
...
- **Active / pressed:** Use immediate key depression, no flashing scores, and no unnecessary animation loops.
...
.design .hero{display:block;padding:40px 0}.design .hero-copy{text-align:center;max-width:820px;margin:auto}.design .hero h1{font:400 52px/1.2 Silkscreen,monospace;text-shadow:4px 4px #604a7c}
.design .hero-copy p{margin-inline:auto}.design .actions{justify-content:center}.design .project-card{max-width:840px;margin:40px auto 0;border:4px solid var(--accent);border-radius:0;box-shadow:8px 8px #604a7c;padding:32px}
.design .features{grid-template-columns:1fr 1fr;gap:32px}.design .feature-card:nth-child(n){border:4px solid var(--control-line);box-shadow:6px 6px #604a7c;padding:28px}
.design .feature-card:first-child{grid-column:1/-1}.design .feature-index{font:14px/1.5 Silkscreen,monospace}
.design .stats{border:4px solid var(--accent);padding:32px;background:var(--surface)}.design .stats strong{font:400 36px/1.3 Silkscreen,monospace}
.design .testimonial{border:0;font:20px/1.7 ui-monospace,monospace}
.design .testimonial blockquote{font:20px/1.7 ui-monospace,monospace}.design .demo-form{border:4px solid var(--accent);box-shadow:8px 8px #604a7c}
