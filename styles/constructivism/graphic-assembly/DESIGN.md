...
description: Forceful type and assembled blocks with diagonal accents kept outside the reading plane.
tags:
  - constructivism
  - graphic-assembly
  - complete-system
...
Forceful type and assembled blocks with diagonal accents kept outside the reading plane. This is one original interpretation of the constructivism family, not a definitive rulebook for the entire aesthetic. It is intended for complete websites and application surfaces that benefit from this visual character. Its layout, material choices, and control conventions below define why it exists as a separate system.

Use an 8:4 hero and feature spans of 2:1:1. Build the page from clear headline, evidence, and action blocks. Diagonal accents sit behind empty margin space. Avoid ideological emblems or reproducing existing propaganda posters.
...
- Assemble a clear argument through layout.
...
Use heavy display type for short declarations and plain sans for body. Uppercase is limited to headings under eight words. Labels remain sentence case so actions can be scanned quickly.

| Role | Stack | Size / line height | Weight and tracking |
|---|---|---|---|
| Display / H1 | `Impact, Haettenschweiler, sans-serif` | fluid 36–84px / 1.08 | 900; -0.03em |
| H2 | `Impact, Haettenschweiler, sans-serif` | 44px / 1.2 | 900; -0.02em |
| H3 | `Impact, Haettenschweiler, sans-serif` | 27px / 1.3 | 650; normal |
...
Use cream planes, dark structural rules, and an earthy red action accent. A diagonal stripe can mark a margin, never a text baseline or hit target. No material depth, rounded panels, or blended gradients.
...
Primary buttons are emphatic red rectangles; secondary buttons are dark outlines. Forms live on a calm cream block with strong labels and complete rectangular fields. Feature cards begin with a large ordinal and heavy rule. Pricing uses equal spans despite the surrounding asymmetry. Navigation is a narrow bold register; tabs use a thick red baseline. Menus and dialogs are square cream panels with quiet body text and explicit action names.
...
Use original documentary photographs or abstract diagrams with direct captions. Avoid historical slogans and copied poster imagery. Icons are blunt strokes with recognizable silhouettes. Charts use heavy baselines and directly labeled values; dramatic graphic treatment must not exaggerate scales. Supply data tables.
...
Use 100ms linear color changes. No rotating panels, text scramble, or sliding headlines. Reduced motion makes all changes immediate.
...
```css
.design-constructivism-graphic-assembly {
  --canvas: #eee3d0;
  --surface: #fff5e4;
  --ink: #322b28;
  --muted: #665a50;
...
.design-constructivism-graphic-assembly * { box-sizing: border-box; }
```
...
- Assemble a clear argument through layout.
- keep diagonals decorative.
- separate headline and action hierarchy.
- Preserve the readable control and feedback behavior when adapting the visual composition.

## Don't

- Do not rotate meaningful text or borrow political symbols to create visual intensity.
