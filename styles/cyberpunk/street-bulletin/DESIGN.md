---
name: Street Bulletin Cyberpunk
slug: street-bulletin
family: cyberpunk
description: "A street-level interpretation combining public notices, oversized warning typography, and layered information blocks rather than a polished terminal."
tags:
  - cyberpunk
  - street-bulletin
  - complete-system
...
# Street Bulletin Cyberpunk

## Overview and Design Philosophy

A street-level interpretation combining public notices, oversized warning typography, and layered information blocks rather than a polished terminal.
...
Use an 96px condensed heading in a full-width notice band. Offset the workspace below. Feature notices form unequal columns, with large index stamps and hard dividers.

The distinction from **Diagnostic Console Cyberpunk** is structural: that interpretation follows this direction: Use a compact 4:5 opening, 44px monospace heading, and narrow spacing. Workspace rows align like diagnostic entries. Feature blocks form a single column with serial labels. Choose this variant when the composition above suits the content. A palette substitution alone does not establish either interpretation.

## Visual Character

Use near-black walls, acid-yellow notice strips, magenta edge marks, and hard rectangular panels. Noise is confined to outer margins; text remains sharp.
...
- Buttons are conspicuous labeled activation blocks. Tabs resemble notice indexes. Fields are opaque black wells with bright boundaries; dialogs use one dominant framed notice.
...
| Token | Value | Role |
|---|---|---|
| `canvas` | `#151721` | Page environment |
| `surface` | `#232636` | Opaque reading surface |
| `ink` | `#f0eef8` | Primary text on surface |
| `muted` | `#bebdd2` | Secondary text on surface |
| `accent` | `#cee957` | Action and selection cue |
| `on-accent` | `#151d29` | Text on filled accent |
| `line` | `#85839e` | Grouping rules |
| `control-line` | `#85839e` | Control boundary |
...
| Section heading | `'Barlow Condensed','Arial Narrow',sans-serif` | 34px / 1.2; 28px on phones |
| Card heading | `'Barlow Condensed','Arial Narrow',sans-serif` | 23px / 1.3; variant overrides below take precedence |
...
Use a 4px base with 8/12/16/24/32/48/64/96px intervals. The wide-screen section interval is 64px; the default module gap is 24px and outer gutter is 28px. Follow the role-specific interiors in the construction rules rather than forcing every region to have identical padding.
...
Use original night-street imagery or abstract signage. Avoid fake foreign-language glyphs and borrowed branded signage.
...
Use immediate selection and restrained 120ms color changes; no flicker, glitching copy, or flashing warnings.
...
**Pointer policy: none.** Layered notices already supply visual noise; a tracking signal would reduce readability.
...
- **Active / pressed:** Use immediate selection and restrained 120ms color changes; no flicker, glitching copy, or flashing warnings.
...
.design .hero{display:block;padding:36px 0}.design .hero-copy{border-left:16px solid var(--accent);padding:24px;background:#141b23}
.design .hero h1{font:700 96px/.94 'Barlow Condensed',Impact,sans-serif;text-transform:uppercase;text-shadow:none}
.design .project-card{margin:36px 0 0 24%;border:3px solid var(--accent);border-radius:0;box-shadow:8px 8px #854177;clip-path:none}
.design .features{grid-template-columns:2fr 1fr;gap:24px}.design .feature-card:nth-child(n){border:2px solid var(--control-line);border-radius:0;clip-path:none;box-shadow:none}
.design .feature-card:first-child{grid-row:span 2;border-top:24px solid var(--accent)}.design .feature-index{font-size:18px;font-weight:800}
.design .stats{border-block:6px solid var(--accent);padding:28px 0}.design .stats strong{font:700 56px/1 'Barlow Condensed',Impact,sans-serif}
.design .testimonial{text-align:left;border-left:12px solid #b777aa;padding:32px}.design .demo-form{border:3px solid var(--accent);border-radius:0;clip-path:none}
