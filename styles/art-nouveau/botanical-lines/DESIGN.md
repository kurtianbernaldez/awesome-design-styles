...
family: art-nouveau
description: "Flowing line and integrated framing: organic curvature connects typography and surfaces into one ornamental structure."
tags:
  - art-nouveau
  - botanical-lines
  - complete-system
...
Pair a 1:1-ish opening with a 54px gap. A regular italic serif headline reaches 80px. Give the text a bowed left rule and the workspace an arched, asymmetric lower contour. Keep text blocks rectangular inside the curved frames.
...
- Use warm parchment, olive linework, and nested curved borders.
- Let the control construction explain state and hierarchy.
- Do not equate the family with green colors and a serif alone, or put flowing ornament through editable text.
...
Use warm parchment, olive linework, and nested curved borders. The workspace has a 5px double outline and a secondary inset curve, while feature cards use shallow arched caps. Organic radius is unequal, not a generic pill repeated everywhere.
...
Buttons use alternating curved corners while their labels and hit areas remain stable. Forms use persistent labels and simple inset boundaries. Tabs and dialogs repeat the flowing frame at the container scale, not around every letter.
...
Use original botanical line illustration where appropriate; the live reference expresses the style through flowing structural curves without borrowing historical artwork. Avoid unrelated photographic collage.
...
Use a gentle 160ms color transition, not continuous vine growth. Reduced motion is immediate.
...
- **Active / pressed:** Use a gentle 160ms color transition, not continuous vine growth. Reduced motion is immediate.
...
Background reference: [V&A: Art Nouveau](https://www.vam.ac.uk/articles/art-nouveau-an-international-style). The rules in this document are an original interface interpretation, not a reproduction of a source artifact or a claim that this variant exhausts the family.
