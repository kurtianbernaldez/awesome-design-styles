---
name: Warm Order Japandi
slug: warm-order
family: japandi
description: Warm neutral planes, precise alignment, and gently softened practical components.
tags:
  - japandi
  - warm-order
  - complete-system
...
Warm neutral planes, precise alignment, and gently softened practical components. This is one original interpretation of the japandi family, not a definitive rulebook for the entire aesthetic. It is intended for complete websites and application surfaces that benefit from this visual character. Its layout, material choices, and control conventions below define why it exists as a separate system.

Use balanced hero halves and three equal feature columns. Align titles and action baselines. Calm repetition and softened precision distinguish this from the intentionally less regular framing of Quiet Imperfection.
...
| Token | Value | Role |
|---|---|---|
| `canvas` | `#eae6dc` | Page environment; ink text |
| `surface` | `#faf6ed` | Opaque reading and control plane |
| `ink` | `#3c4940` | Main text on canvas or surface |
| `muted` | `#627065` | Secondary text on surface |
| `accent` | `#4e6a55` | Primary action and selected state |
| `on-accent` | `#ffffff` | Text on the accent fill |
| `line` | `#819185` | Grouping rules |
| `control-line` | `#819185` | Visible input and control boundaries |
...
Use an understated sans stack with a moderate size range. Avoid decorative scripts and oversized luxury serifs. Body is 17px with 1.7 leading and a 62ch measure.

| Role | Stack | Size / line height | Weight and tracking |
|---|---|---|---|
| Display / H1 | `system-ui, sans-serif` | fluid 36–58px / 1.08 | 550; -0.03em |
| H2 | `system-ui, sans-serif` | 32px / 1.2 | 550; -0.02em |
...
Use 4/8/12/20/28/40/64/96px. Keep outer whitespace generous and component interiors efficient. Consistent alignment matters more than empty space for its own sake.
...
Use warm opaque light planes, desaturated green actions, and subtle 8px corners. No visible material grain behind text. Cards may be separated by open space and a fine rule rather than outlined boxes.

Reference card boundary: 1px where a frame is used; reference corner radius: 8px. Surface shadow: `none`. Controls generally use 8px corners unless their component treatment specifies a more precise shape. Content stays at layer 0, navigation at 10 when sticky, menus at 20, backdrop at 40, dialog at 50, and transient feedback at 60. Avoid creating stacking contexts on every card. Overlays must remain above all decorative treatments.

## Components

Primary buttons use deep green with white labels and 6px corners. Secondary actions are outlined. Fields are clean opaque rectangles with full labels. Feature cards are quiet ruled groups; pricing is aligned with minimal recommendation emphasis. Navigation is a simple 72px row, tabs have a 2px underline, and menus/dialogs use opaque warm surfaces with restrained borders. Avoid ornamental ceremony around common tasks.
...
Use original natural-light interiors or material imagery sparingly, without copying recognizable furniture or cultural symbols. Icons are fine but readable 1.75px outlines. Data views are flat and practical with clear units, direct labels, and accessible tables.
...
Use 160ms ease-out color changes and a subtle dialog fade. No floating objects, parallax, or slow ceremonial transitions. Reduced motion makes state changes immediate.
...
.design-japandi-warm-order * { box-sizing: border-box; }
```
...
- Keep rhythm consistent.
- balance warm surfaces with precise controls.
- use content-driven space.
- Preserve the readable control and feedback behavior when adapting the visual composition.

## Don't

- Do not replace usability with excessive emptiness or culturally stereotyped ornaments.
