...
slug: pattern-play
family: memphis
description: Playful geometric accents, contrasting fields, and orderly controls on a cheerful canvas.
tags:
  - memphis
  - pattern-play
  - complete-system
...
Playful geometric accents, contrasting fields, and orderly controls on a cheerful canvas. This is one original interpretation of the memphis family, not a definitive rulebook for the entire aesthetic. It is intended for complete websites and application surfaces that benefit from this visual character. Its layout, material choices, and control conventions below define why it exists as a separate system.

Use balanced hero halves and three equal feature blocks. The visual rhythm comes from alternating pale peach, mint, and lilac fields rather than random positioning. Keep all action positions and reading order predictable.
...
- Concentrate pattern in margins.
...
| `muted` | `#625c75` | Secondary text on surface |
...
Use friendly bold sans headings with ordinary readable body. Short monospaced indexes add rhythm. Avoid novelty fonts or mixing too many type voices with the already varied geometry.
...
Use pale color fields, contrasting 2px outlines, and a small hard offset. Reserve dots and stripes for a header margin or card edge. Do not texture input interiors. Combine 12px cards with square title rules and circular passive indexes.

Reference card boundary: 2px where a frame is used; reference corner radius: 12px. Surface shadow: `5px 5px 0 #353147`. Controls generally use 12px corners unless their component treatment specifies a more precise shape. Content stays at layer 0, navigation at 10 when sticky, menus at 20, backdrop at 40, dialog at 50, and transient feedback at 60. Avoid creating stacking contexts on every card. Overlays must remain above all decorative treatments.

## Components

Primary buttons are deep plum with white labels; secondary buttons use a cream fill and dark boundary. Fields remain plain and rectangular with modest corners. Feature cards may have a short patterned top band, but text stays on a flat field. Pricing is equally sized and recommendation is written. Navigation is plain text beneath a patterned edge. Tabs use a filled selected segment. Menus and dialogs strip away patterns and keep only the outline and small hard shadow.
...
Use original simple geometric illustration with a limited shape vocabulary. Avoid copying recognizable furniture patterns or designer prints. Images use rectangular crops and captions. Charts use flat shapes with accurate scales, explicit labels, and a quiet background; never substitute a decorative squiggle for a data line.
...
Use 120ms color changes and a 1px press offset. Decorative dots and stripes never move. Reduced motion removes translation and changes the border or shadow instantly.
...
.design-memphis-pattern-play * { box-sizing: border-box; }
```
...
- Concentrate pattern in margins.
- keep pastel text planes flat.
- preserve stable controls.
- Preserve the readable control and feedback behavior when adapting the visual composition.

## Don't

- Do not pattern every card, rotate controls, or use decorative geometry as an unlabeled icon.
