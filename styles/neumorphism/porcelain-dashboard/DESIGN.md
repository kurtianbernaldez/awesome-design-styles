---
name: Porcelain Dashboard Neumorphism
slug: porcelain-dashboard
family: neumorphism
description: "An airy dashboard interpretation where separate raised islands emerge from one pale plane, with generous spacing and softly molded edges."
tags:
  - neumorphism
  - porcelain-dashboard
  - complete-system
...
# Porcelain Dashboard Neumorphism

## Overview and Design Philosophy

An airy dashboard interpretation where separate raised islands emerge from one pale plane, with generous spacing and softly molded edges.
...
Center the opening headline above a broad raised workspace. Statistics form three individual relief islands. Features use a spacious two-column grid, emphasizing quiet, readable modules rather than a compact console.

The distinction from **Recessed Console Neumorphism** is structural: that interpretation follows this direction: Use a compact 4:5 opening, a 46px medium sans heading, and a 28px gap. The workspace is a recessed basin; feature rows are shallow inset channels. The raised/inset hierarchy distinguishes tools from information. Choose this variant when the composition above suits the content. A palette substitution alone does not establish either interpretation.

## Visual Character

Keep canvas and surface identical. Use a broad 16px opposed shadow pair for the workspace, 10px relief for smaller islands, and 36px rounded corners. Inputs remain inset so depth has a consistent meaning.
...
- Primary text buttons are long raised pills. Circular tools sit in a roomy row. Selected tabs are inset capsules, while menus and dialogs become a single lifted porcelain slab. Use visible focus outlines even when resting borders are absent.
...
Use a 4px base with 8/12/16/24/32/48/64/96px intervals. The wide-screen section interval is 88px; the default module gap is 40px and outer gutter is 36px. Follow the role-specific interiors in the construction rules rather than forcing every region to have identical padding.
...
Keep icons simple and use diagrams sparingly. The pale material and opposing light should be recognizable without any image.
...
Use 160ms color changes and immediate press-depth changes. Do not float islands continuously.
...
A broad porcelain highlight follows the pointer while the outer relief light shifts gently. This is decorative material feedback, not a status indicator.

**Construction.** Paint a broad 240 × 80px elliptical white glaze highlight fading to transparent by 70%. Keep it low-opacity and smooth. Move the active panel’s opposing gray and white relief shadows within 3–9px offsets while preserving the raised and recessed hierarchy. Use this document’s surface tokens; the glaze has no hard outline.

**Movement and coverage.** React on only the active material surface. Follow local coordinates and rotate by at most 52.5 degrees as the pointer crosses the surface. Measure coordinates relative to that surface, keep the reflection behind its content, and clip it to the material radius. Leave page margins, section gaps, and unrelated reading regions still. The material host, labels, and hit targets never tilt or move.
...
- **Active / pressed:** Use 160ms color changes and immediate press-depth changes. Do not float islands continuously.
...
.design .hero{display:block;padding:56px 0}.design .hero-copy{text-align:center;max-width:760px;margin:auto}.design .hero-copy p{margin-inline:auto}.design .actions{justify-content:center}
.design .hero h1{font-size:64px;font-weight:450;letter-spacing:-.05em}
.design .project-card{max-width:800px;margin:44px auto 0;padding:40px;border-radius:36px;box-shadow:16px 16px 32px #bdc7d8,-16px -16px 32px #fff}
.design .project-rows{display:grid;grid-template-columns:repeat(3,minmax(0,1fr));gap:24px}.design .project-rows>div{display:block;border:0}
.design .control-tools{justify-content:center;gap:24px}.design .switch-label{margin-left:12px}
.design .features{grid-template-columns:1fr 1fr;gap:40px}.design .feature-card:nth-child(n){padding:36px;border-radius:32px;box-shadow:10px 10px 22px #bdc7d8,-10px -10px 22px #fff}
.design .stats{box-shadow:none;padding:12px 0;gap:28px}.design .stats>div{box-shadow:8px 8px 18px #bdc7d8,-8px -8px 18px #fff;border-radius:28px;padding:28px}
.design .primary{padding:16px 28px;border-radius:32px}.design .demo-form{border-radius:32px;padding:36px}
.design .testimonial{max-width:760px;margin-inline:auto;border:0}
