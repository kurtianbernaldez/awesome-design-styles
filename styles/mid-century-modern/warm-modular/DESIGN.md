...
description: Warm color blocks, balanced proportions, and understated editorial detail in a practical modular system.
tags:
  - mid-century-modern
  - warm-modular
  - complete-system
...
Warm color blocks, balanced proportions, and understated editorial detail in a practical modular system. This is one original interpretation of the mid century modern family, not a definitive rulebook for the entire aesthetic. It is intended for complete websites and application surfaces that benefit from this visual character. Its layout, material choices, and control conventions below define why it exists as a separate system.

Use a 3:2 hero and equal three-column modules. Balance a large typographic area against a contained project pane. Keep the composition calm and horizontal, with a spacious quote separating features and pricing.
...
- Balance warm fields with readable neutral areas.
...
| `muted` | `#616957` | Secondary text on surface |
...
Use a modest serif display and clean sans body. Headings have natural proportions and restrained weight. Small labels may use uppercase tracking, but body copy stays relaxed and clear.

| Role | Stack | Size / line height | Weight and tracking |
|---|---|---|---|
| Display / H1 | `ui-serif, Georgia, serif` | fluid 36–60px / 1.08 | 500; -0.03em |
| H2 | `ui-serif, Georgia, serif` | 34px / 1.2 | 500; -0.02em |
...
Use warm cream panels, subtle 4px corners, olive dividers, and restrained walnut-colored actions. Supporting blocks may be pale ochre or blue-green. Avoid fake wood grain beneath text and heavy material simulation.

Reference card boundary: 1px where a frame is used; reference corner radius: 4px. Surface shadow: `0 3px 0 #c3baa6`. Controls generally use 4px corners unless their component treatment specifies a more precise shape. Content stays at layer 0, navigation at 10 when sticky, menus at 20, backdrop at 40, dialog at 50, and transient feedback at 60. Avoid creating stacking contexts on every card. Overlays must remain above all decorative treatments.

## Components

Primary buttons are walnut-colored rectangles with small corners and white labels. Secondary buttons use a visible olive outline. Forms are straightforward cream fields, with readable labels and native choices. Cards use flat warm fills and a small lower seam. Pricing remains aligned; recommendation uses a simple text badge. Navigation is a quiet horizontal row, tabs have an accent underline, and dialogs are opaque cream with restrained depth.
...
Use original warm documentary photography, simple line illustration, and geometric accents outside text. Avoid copying furniture silhouettes or vintage advertising layouts. Charts use muted but distinguishable fills with dark labels and patterns; provide data tables. Never let period mood reduce readability.
...
Use 160ms ease-out color changes. Dialogs fade without scale. No swinging ornaments, animated starbursts, or bouncing cards. Reduced motion uses immediate updates.
...
```css
.design-mid-century-modern-warm-modular {
  --canvas: #eee6d5;
  --surface: #fff7e8;
  --ink: #3b423b;
  --muted: #616957;
...
.design-mid-century-modern-warm-modular * { box-sizing: border-box; }
```
...
- Balance warm fields with readable neutral areas.
- keep modules proportional.
- use small restrained accents.
- Preserve the readable control and feedback behavior when adapting the visual composition.

## Don't

- Do not turn the interface into a catalog of vintage objects or apply wood texture to forms.
